...
#[allow(dead_code)]
pub async fn get_experiment_iterations(pool: &DbPool, experiment_id: i64)
...
pub async fn get_recent_iterations(
	pool: &DbPool,
	experiment_id: i64,
	limit: i32,
) -> Result<Vec<Iteration>>
...
fn row_to_iteration(row: &sqlx::sqlite::SqliteRow) -> Result<Iteration>
{
	Ok(Iteration {
		...
		patience_counter: row
			...
			.or_else(|_| {
				row
					.try_get::<Option<i64>, _>("patience_counter")
...
pub async fn get_genome_evaluations(
	pool: &DbPool,
	iteration_id: i64,
) -> Result<Vec<GenomeEvaluation>>
{
	let rows = sqlx::query(
