...
pub mod queries {
    ...
    pub async fn list_best_genomes(
        ...
        .bind(ids_dataset).bind(ids_dataset)
        .bind(ids_split).bind(ids_split)
        .bind(limit)
        .bind(offset)
        .fetch_all(pool)
        .await?;
    ...
    pub async fn submit_best_genome(
        ...
        let genome_id = if let Some(data) = genome_data {
            let config_hash = data.get("config_hash")
                .and_then(|v| v.as_str())
                .unwrap_or(genome_hash);
            let tiers_json = data.get("tiers_json")
                .and_then(|v| v.as_str())
                .unwrap_or("[]");
            let total_clusters = data.get("total_clusters")
                .and_then(|v| v.as_i64())
                .unwrap_or(0) as i32;
            let total_neurons = data.get("total_neurons")
                .and_then(|v| v.as_i64())
                .unwrap_or(0) as i32;
            let architecture_type = data.get("architecture_type")
                .and_then(|v| v.as_str())
                .unwrap_or("bitwise");
            let connections_json = data.get("connections_json")
                .and_then(|v| v.as_str());
    ...
    pub async fn update_best_genome_hf(
        pool: &DbPool,
        id: i64,
        hf_repo_id: &str,
    ) -> Result<bool> {
