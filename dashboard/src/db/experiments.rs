...
pub async fn delete_experiment(pool: &DbPool, exp_id: i64) -> Result<bool>
{
	...
	{
		anyhow::bail!(
			"Can only delete pending experiments (current status: {})",
			status
		);
	}
	...
	if let Some(fid) = flow_id
	{
		let remaining_ids: Vec<i64> =
			sqlx::query_scalar("SELECT id FROM experiments WHERE flow_id = ? ORDER BY sequence_order")
				.bind(fid)
				.fetch_all(pool)
				.await?;
...
pub async fn reorder_experiments(
	pool: &DbPool,
	flow_id: i64,
	experiment_ids: &[i64],
) -> Result<bool>
...
) -> Result<i64>
{
	...
	let fitness_calculator = config
		.get("fitness_calculator")
		.or_else(|| config.get("fitness_calculator_type"))
		.and_then(|v| v.as_str())
		.unwrap_or("normalized");
	let fitness_weight_ce = config
		.get("fitness_weight_ce")
		.and_then(|v| v.as_f64())
		.unwrap_or(1.0);
	let fitness_weight_acc = config
		.get("fitness_weight_acc")
		.and_then(|v| v.as_f64())
		.unwrap_or(1.0);
	let tier_config = config.get("tier_config").map(|v| v.to_string());
	...
	E: sqlx::SqliteExecutor<'e>,
{
	...
	let tier_config = flow_config
		.params
		.get("tier_config")
		.and_then(|v| v.as_str())
		.map(|s| s.to_string());
	let fitness_calculator = flow_config
		.params
		.get("fitness_calculator")
		.and_then(|v| v.as_str())
		.unwrap_or("harmonic_rank");
	let fitness_weight_ce = flow_config
		.params
		.get("fitness_weight_ce")
		.and_then(|v| v.as_f64())
		.unwrap_or(1.0);
	let fitness_weight_acc = flow_config
		.params
		.get("fitness_weight_acc")
		.and_then(|v| v.as_f64())
		.unwrap_or(1.0);
	let context_size = flow_config
		.params
		.get("context_size")
		.and_then(|v| v.as_i64())
		.unwrap_or(4) as i32;
	let population_size = flow_config
		.params
		.get("population_size")
		.and_then(|v| v.as_i64())
		.unwrap_or(50) as i32;

	let architecture_type = flow_config
		.params
		.get("architecture_type")
		.and_then(|v| v.as_str())
		.unwrap_or("tiered");
	...
	if let Some(s) = status
	{
		...
		match s
		{
			"running" =>
			{
				...
				let prior_iter: i32 =
					sqlx::query_scalar("SELECT COALESCE(current_iteration, 0) FROM experiments WHERE id = ?")
						.bind(id)
						.fetch_optional(pool)
						.await?
						.unwrap_or(0);
				let has_iteration_rows: i32 =
					sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM iterations WHERE experiment_id = ?)")
						.bind(id)
						.fetch_one(pool)
						.await?;
...
{
	let result = sqlx::query("UPDATE experiments SET flow_id = ?, sequence_order = ? WHERE id = ?")
		.bind(flow_id)
		.bind(sequence_order)
		.bind(experiment_id)
		.execute(pool)
...
fn row_to_experiment(row: &sqlx::sqlite::SqliteRow) -> Result<Experiment>
{
	...
	let gating_status: Option<GatingStatus> = row
		.try_get::<Option<String>, _>("gating_status")
		.ok()
		.flatten()
		.map(|s| parse_gating_status(&s));
	let gating_results: Option<GatingResults> = row
		.try_get::<Option<String>, _>("gating_results")
		.ok()
		.flatten()
		.and_then(|s| serde_json::from_str(&s).ok());

	let architecture_type: ArchitectureType = row
		.try_get::<Option<String>, _>("architecture_type")
		.ok()
		.flatten()
	...
	Ok(Experiment {
		...
		status_message: row
			.try_get::<Option<String>, _>("status_message")
			.ok()
			.flatten(),
		architecture_type,
		gating_status,
		gating_results,
		params: row
			.try_get::<Option<String>, _>("params_json")
			.ok()
			.flatten()
			.and_then(|s| serde_json::from_str(&s).ok()),
		extra_metrics: row
			.try_get::<Option<String>, _>("extra_metrics_json")
			.ok()
			.flatten()
			.and_then(|s| serde_json::from_str(&s).ok()),
	})
