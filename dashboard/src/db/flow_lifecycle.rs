...
{
	// Get the flow's PID
	let pid: Option<i64> = sqlx::query_scalar("SELECT pid FROM flows WHERE id = ?")
		.bind(flow_id)
		.fetch_optional(pool)
		.await?
		.flatten();
	...
	{
		tracing::warn!(
			"No PID registered for flow {}, marking as cancelled (worker will check status)",
			flow_id
		);
	}
...
async fn delete_flow_data(pool: &DbPool, flow_id: i64) -> Result<()>
{
	...
	let exp_ids: Vec<i64> = sqlx::query_scalar("SELECT id FROM experiments WHERE flow_id = ?")
		.bind(flow_id)
		.fetch_all(pool)
		.await?;
...
pub(crate) async fn delete_experiment_data(pool: &DbPool, exp_id: i64) -> Result<()>
{
	...
	let checkpoint_paths: Vec<String> =
		sqlx::query_scalar("SELECT file_path FROM checkpoints WHERE experiment_id = ?")
			.bind(exp_id)
			.fetch_all(pool)
			.await?;
...
) -> Result<bool>
{
	...
	if clear_seed.is_some()
	{
		...
		if !saved_experiments.is_empty()
		{
			...
			let config_json: String = sqlx::query_scalar("SELECT config_json FROM flows WHERE id = ?")
				.bind(id)
				.fetch_one(pool)
				.await?;
	...
	else
	{
		...
		let non_completed_exp_ids: Vec<i64> =
			sqlx::query_scalar("SELECT id FROM experiments WHERE flow_id = ? AND status != 'completed'")
				.bind(id)
				.fetch_all(pool)
				.await?;
		...
		let all_experiments: Vec<(i64, Option<String>)> =
			sqlx::query_as("SELECT id, phase_type FROM experiments WHERE flow_id = ?")
				.bind(id)
				.fetch_all(pool)
				.await?;
