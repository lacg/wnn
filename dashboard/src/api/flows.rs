...
) -> impl IntoResponse
{
	...
	.await
	{
		...
		Ok(false) => (
			StatusCode::NOT_FOUND,
			Json(
				serde_json::json!({"error": "Flow not found (or a concurrent status change won the race)"}),
			),
		)
			.into_response(),
		Err(e) =>
	...
	let max_iterations = exp_spec
		...
		.or_else(|| {
			match exp_spec.experiment_type
			{
				...
				ExperimentType::Ga => flow
					.config
					.params
					.get("ga_generations")
					.and_then(|v| v.as_i64())
					.map(|v| v as i32),
				ExperimentType::Ts => flow
					.config
					.params
					.get("ts_iterations")
					.and_then(|v| v.as_i64())
				...
				| ExperimentType::Axonogenesis => exp_spec
					.params
					.get("iterations")
					.and_then(|v| v.as_i64())
					.map(|v| v as i32),
