...
pub fn routes(state: Arc<AppState>) -> Router
{
	Router::new()
		...
		.route(
			"/api/experiments/:id/iterations",
			get(get_experiment_iterations),
		)
		.route(
			"/api/experiments/:id/summaries",
			get(get_validation_summaries).post(create_validation_summary),
		)
		// Gating runs
		.route(
			"/api/experiments/:id/gating",
			get(list_gating_runs).post(create_gating_run),
		)
		.route(
			"/api/experiments/:id/gating/:gating_id",
			get(get_gating_run).patch(update_gating_run),
		)
		...
		.route(
			"/api/flows/:id/experiments",
			get(list_flow_experiments).post(add_experiment_to_flow),
		)
		.route(
			"/api/flows/:id/experiments/link",
			post(link_experiment_to_flow),
		)
		.route(
			"/api/flows/:id/experiments/reorder",
			axum::routing::put(reorder_flow_experiments),
		)
		...
		.route(
			"/api/flows/:id/combined-validations",
			get(get_combined_validations).post(create_combined_validation),
		)
		...
		.route(
			"/api/best-genomes",
			get(list_best_genomes).post(submit_best_genomes),
		)
		.route(
			"/api/best-genomes/recalculate",
			post(recalculate_best_genomes),
		)
		.route(
			"/api/best-genomes/:id",
			get(get_best_genome).delete(delete_best_genome),
		)
		.route("/api/best-genomes/:id/download", get(download_best_genome))
		.route(
			"/api/best-genomes/:id/export-hf",
			post(export_best_genome_hf),
		)
		// Live progress (in-memory, no DB)
		.route(
			"/api/experiments/:id/live-progress",
			get(get_live_progress)
				.post(post_live_progress)
				.delete(clear_live_progress),
		)
		// Worker log watching
