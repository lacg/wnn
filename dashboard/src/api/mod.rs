...
async fn get_best_genome(
    State(state): State<Arc<AppState>>,
    Path(id): Path<i64>,
) -> impl IntoResponse {
    match crate::db::queries::get_best_genome(&state.db, id).await {
        Ok(Some(genome)) => (StatusCode::OK, Json(genome)).into_response(),
        Ok(None) => (StatusCode::NOT_FOUND, Json(serde_json::json!({"error": "Best genome not found"}))).into_response(),
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))).into_response(),
    }
}

async fn download_best_genome(
    State(state): State<Arc<AppState>>,
    Path(id): Path<i64>,
) -> impl IntoResponse {
    match crate::db::queries::get_best_genome_data(&state.db, id).await {
        Ok(Some(data)) => (StatusCode::OK, Json(data)).into_response(),
        Ok(None) => (StatusCode::NOT_FOUND, Json(serde_json::json!({"error": "Best genome data not found"}))).into_response(),
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))).into_response(),
    }
}

#[derive(Debug, Deserialize)]
struct SubmitBestGenomeEntry {
    task_type: String,
    stage: String,
    metric: String,
    genome_hash: String,
...
#[derive(Debug, Deserialize)]
struct SubmitBestGenomesRequest {
    genomes: Vec<SubmitBestGenomeEntry>,
}

async fn submit_best_genomes(
    State(state): State<Arc<AppState>>,
    ...
    for entry in &req.genomes {
        match crate::db::queries::submit_best_genome(
            ...
            Ok(Some(id)) => accepted.push(serde_json::json!({"genome_hash": entry.genome_hash, "id": id})),
            Ok(None) => rejected.push(serde_json::json!({"genome_hash": entry.genome_hash, "reason": "genome not found"})),
            Err(e) => rejected.push(serde_json::json!({"genome_hash": entry.genome_hash, "reason": e.to_string()})),
        }
    }
...
fn default_max_entries() -> i32 { 150 }

async fn recalculate_best_genomes(
    State(state): State<Arc<AppState>>,
...
async fn delete_best_genome(
    State(state): State<Arc<AppState>>,
    Path(id): Path<i64>,
) -> impl IntoResponse {
    match crate::db::queries::delete_best_genome(&state.db, id).await {
        Ok(true) => StatusCode::NO_CONTENT.into_response(),
        Ok(false) => (StatusCode::NOT_FOUND, Json(serde_json::json!({"error": "Best genome not found"}))).into_response(),
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))).into_response(),
    }
}

#[derive(Debug, Deserialize)]
struct BestGenomeExportHfRequest {
    repo_id: String,
}

async fn export_best_genome_hf(
    State(state): State<Arc<AppState>>,
    Path(id): Path<i64>,
    ...
    match crate::db::queries::update_best_genome_hf(&state.db, id, &req.repo_id).await {
        Ok(true) => {
            match crate::db::queries::get_best_genome(&state.db, id).await {
                Ok(Some(genome)) => (StatusCode::OK, Json(genome)).into_response(),
                _ => (StatusCode::OK, Json(serde_json::json!({"id": id, "hf_repo_id": req.repo_id}))).into_response(),
            }
        }
        Ok(false) => (StatusCode::NOT_FOUND, Json(serde_json::json!({"error": "Best genome not found"}))).into_response(),
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))).into_response(),
