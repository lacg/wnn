...
class RecurrentArchGenome:
	...
	def _remap_state_neuro(self, k: int, sw: int, ow: int, removed_floor: int,
		...
		`widths` = (state bits before, after, output bits before, after): a layer
		wider than 64 bits is hashed and is DROPPED to relearn instead (Rust)."""
		# Rust compound op: grow both layers' prefixes, or drop-removed + shrink.
		sb0, sb1, ob0, ob1 = widths
	...
	def set_state_neurons(self, target: int, rng: np.random.Generator) -> None:
		...
		old_sb, old_ob = self.state_bits_per_neuron, self.output_bits_per_neuron
		self._resize_state_neurons(target, rng)
		if self.cells is not None:
			self._remap_state_neuro(k, sw, ow, removed_floor=target,
			                        widths=(old_sb, self.state_bits_per_neuron,
			                                old_ob, self.output_bits_per_neuron))
	...
	def set_state_suffix(self, target: int, rng: np.random.Generator) -> None:
		...
		if target == old:
			return
		old_sb = self.state_bits_per_neuron
		for suffix in self.state_sampled:
			_resize_suffix(suffix, self.shape.state_input_space, target, rng)
		if self.cells is not None:
			# Widths before/after: a layer wider than 64 bits is hashed and cannot be
			# remapped — the Rust side drops it to relearn (genome_cells.rs, 29/08/2026).
			self.cells.remap_bits_state(target - old, old_sb, self.state_bits_per_neuron)
	...
	def set_output_suffix(self, target: int, rng: np.random.Generator) -> None:
		...
		if self.cells is not None:
			self.cells.remap_bits_output(target - old, old_ob, self.output_bits_per_neuron)
	...
	def remove_state_neuron(self, k: int, rng: np.random.Generator) -> None:
		...
		if self.cells is not None:
			...
			# Widths BEFORE the excision; a hashed (>64-bit) layer drops instead.
