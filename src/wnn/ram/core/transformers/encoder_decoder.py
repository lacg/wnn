...
class RAMEncoderDecoder(Module):
	...
	def decode(
		...
		"""
		Greedy decoding given encoder output.

		Args:
			encoder_output: Encoded source sequence
			start_token: Initial decoder token
		...
		def decoder_model(tokens, enc_out):
			return self.decode_step(tokens, enc_out)
	...
	def search(
		...
		"""
		Beam search decoding given encoder output.
		...
		Returns:
			GenerationResult with best sequence and candidates
		"""
