...
) -> (f64, f64, f64, f64, f64)
{
	...
	let (packed_eval, words_per_example) =
		ram_core::neuron_memory::pack_packed_to_u64(eval_input_bits);
	...
	if num_clusters == 1
	{
		...
		let threshold = override_threshold.unwrap_or_else(|| {
			let (t, _f1, _fpr) =
				find_optimal_threshold_auto(&flat_scores, eval_targets, settings.fitness_weights);
			t
		});
		...
		for ex_idx in 0..num_eval
		{
			let pred = if all_scores[ex_idx][0] >= threshold
			{
				1u32
			}
			else
			...
			if pred as i64 == eval_targets[ex_idx]
			{
				correct += 1;
			}
		}
		...
		let (f1, fpr) =
			compute_f1_fpr_with_normal_class(&predictions, eval_targets, 2, settings.normal_class);
	...
	if export.group_info.len() == 1 && export.sparse_exports.len() == 1
	{
		...
		if *is_sparse && cluster_ids.len() == num_clusters
		{
			...
			if is_contiguous
			{
				...
				static CE_EVALUATOR: std::sync::OnceLock<
					Option<crate::metal_genome_eval::MetalSparseCEEvaluator>,
				> = std::sync::OnceLock::new();
				let ce_eval =
					CE_EVALUATOR.get_or_init(|| crate::metal_genome_eval::MetalSparseCEEvaluator::new().ok());
	...
	#[cfg(target_os = "macos")]
	{
		let use_full_gpu = std::env::var("WNN_GPU_CE")
			.map(|v| v != "0")
			.unwrap_or(true);
		...
		if use_full_gpu
		{
			if let Some(group_eval) = get_group_evaluator()
			{
				...
				let scores_buffer = CACHED_SCORES_BUFFER.with(|cache| {
					...
					{
						if cached_gen == current_reset_gen
							&& cached_eval == num_eval
							&& cached_clusters == num_clusters
						{
				...
				let zero_time_ms = if phase_timing
				{
					phase_start.elapsed().as_micros() as f64 / 1000.0
				}
				else
				{
					0.0
				...
				let input_time_ms = if phase_timing
				{
					phase_start.elapsed().as_micros() as f64 / 1000.0
				}
				else
					...
					if *is_sparse
					{
						...
						sparse_groups.push(crate::metal_genome_eval::SparseGroupData {
							connections: &export.connections
								[group.conn_offset..group.conn_offset + group.conn_size()],
				...
				if all_groups_success
				{
					...
					let result =
						group_eval.compute_ce_from_buffer(&scores_buffer, eval_targets, num_eval, num_clusters);

					let ce_time_ms = if phase_timing
					{
						ce_start.elapsed().as_micros() as f64 / 1000.0
					}
					else
					{
						0.0
	...
	let predictions: Vec<u32> = all_scores
		.par_iter()
		.map(|scores| {
			scores
				.iter()
...
) -> Vec<i64>
{
	...
	)
	{
		...
		{
			eprintln!(
				"[PATH2_FALLBACK] train_and_predict_single → dense: {}",
				reason
			);
...
) -> Vec<f64>
{
	...
	)
	{
		...
		{
			eprintln!(
				"[PATH2_FALLBACK] train_and_score_single → dense: {}",
				reason
			);
			let mut cluster_to_group: Vec<(usize, usize)> = vec![(0, 0); num_clusters];
			for (group_idx, group) in groups.iter().enumerate()
...
) -> (Vec<f64>, Vec<f64>, Option<Vec<f64>>)
{
	...
	)
	{
		...
		{
			eprintln!(
				"[PATH2_FALLBACK] train_and_score_eval_and_train → dense: {}",
				reason
			);
			let mut cluster_to_group: Vec<(usize, usize)> = vec![(0, 0); num_clusters];
			for (group_idx, group) in groups.iter().enumerate()
...
) -> (Vec<f64>, Vec<f64>, Option<Vec<f64>>)
{
	...
	)
	{
		...
		{
			eprintln!(
				"[PATH2_FALLBACK] train_and_score_sets_flat → dense: {}",
				reason
			);
			let mut cluster_to_group: Vec<(usize, usize)> = vec![(0, 0); num_clusters];
			for (group_idx, group) in groups.iter().enumerate()
