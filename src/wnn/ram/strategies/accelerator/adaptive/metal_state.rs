...
pub fn find_optimal_threshold_auto(
	scores: &[f64],
	labels: &[i64],
	fitness_weights: Option<(f32, f32, f32, f32)>,
) -> (f64, f64, f64)
{
	if let Some((w_ce, w_f1, w_fpr, w_acc)) = fitness_weights
	{
		let (t, f1, fpr, _acc, _fitness) =
			find_optimal_threshold_fitness(scores, labels, w_ce, w_f1, w_fpr, w_acc);
		(t, f1, fpr)
	}
	else
	{
...
static METAL_EVALUATOR: RwLock<Option<Arc<crate::metal_ramlm::MetalRAMLMEvaluator>>> =
	RwLock::new(None);
static SPARSE_METAL_EVALUATOR: RwLock<Option<Arc<ram_core::metal_sparse::MetalSparseEvaluator>>> =
	RwLock::new(None);
static GROUP_EVALUATOR: RwLock<Option<Arc<crate::metal_genome_eval::MetalGroupEvaluator>>> =
	RwLock::new(None);
...
{
	let nonempty = |name: &str| std::env::var(name).map(|v| !v.is_empty()).unwrap_or(false);
