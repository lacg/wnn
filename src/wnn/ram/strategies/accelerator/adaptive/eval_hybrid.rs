...
fn log_eval_env_once()
{
	...
	LOGGED.call_once(|| {
		let flags = [
			"WNN_BATCH_SIZE",
			"WNN_HYBRID",
			"WNN_HYBRID_SPEED_RATIO",
			"WNN_OPTION_B",
			"WNN_GPU_BATCHED_TRAIN",
			"WNN_GPU_AFFINITY_RATIO",
			"WNN_SHAPE_GROUP",
			"WNN_COALESCE_GROUPS",
			"WNN_NO_METAL",
			"WNN_ORDER_INDEPENDENT_TRAIN",
			"WNN_TIMING",
			"WNN_GROUP_LOG",
			"WNN_SPARSE_THRESHOLD",
			"WNN_ATOMIC_SPARSE",
		];
...
) -> (usize, GenomeExport, Option<f64>)
{
	...
	let neurons_per_cluster =
		&cfg.genomes_neurons_flat[genome_offset..genome_offset + cfg.num_clusters];
	...
	)
	{
		...
		Err(reason) =>
		{
			...
			else
			{
				...
				let oi_chunked = ram_core::neuron_memory::order_independent_training_active(cfg.memory_mode);
				if oi_chunked
				{
...
) -> Option<Vec<(usize, f64, f64, f64, f64, f64, u32)>>
{
	...
	// The probe merges OI counters — only meaningful when this mode trains
	// through them (env AND `CellMode::uses_oi_counters`).
	if !ram_core::neuron_memory::order_independent_training_active(cfg.memory_mode)
	{
		return None;
	}
	...
	for g in batch_start..batch_end
	{
		...
		if !crate::marker_train::single_genome_exceeds_batch_budget(
			bits,
			cfg.num_train,
			cfg.neuron_sample_rate,
		)
	...
	let mut out: Vec<(usize, f64, f64, f64, f64, f64, u32)> =
	...
	for g in batch_start..batch_end
	{
		...
		let bits = &cfg.genomes_bits_flat[cfg.genome_bpn_offsets[g]..cfg.genome_bpn_offsets[g + 1]];
		let conns =
			&cfg.genomes_connections_flat[cfg.conn_offsets[g]..cfg.conn_offsets[g] + cfg.conn_sizes[g]];
		...
		let votes = match crate::marker_train::train_single_genome_chunked_scored(
			...
			cfg.empty_value,
			cfg.neuron_sample_rate,
			cfg.rng_seed.wrapping_add(g as u64),
			cfg.class_weights,
			eval_input_bits,
			num_eval,
			score_train,
		...
		)
		{
			...
			{
				eprintln!(
					"[EVAL_IN_PLACE] g={} fused path failed ({}); falling back to export path",
					g, e
				);
				return None;
			}
...
) -> (f64, f64, f64, f64, f64)
{
	...
	let to_scores =
		|v: &[u32]| -> Vec<f64> { v.iter().map(|&x| ((x as f32 / 4.0) / n) as f64).collect() };
	...
	let (ce, acc, f1, fpr) = compute_binary_metrics_at_threshold(
		&eval_scores,
		eval_targets,
		threshold,
		settings.normal_class,
	);
...
{
	let mut results: Vec<(f64, f64, f64, f64, f64, u32)> =
		vec![(0.0, 0.0, 0.0, 0.0, 0.5, 0u32); num_genomes];
...
) -> BatchRouting
{
	...
	let gpu_override = std::env::var("WNN_GPU_BATCHED_TRAIN")
		.ok()
		.unwrap_or_default();
	let force_cpu = matches!(gpu_override.as_str(), "off" | "0" | "false" | "no");
	let force_gpu = matches!(gpu_override.as_str(), "force" | "always");
	let legacy_off = std::env::var("WNN_OPTION_B")
		.map(|v| matches!(v.as_str(), "off" | "0"))
		.unwrap_or(false);
	...
	let total_neurons_in_batch: usize = genomes_neurons_flat
		[batch_start * num_clusters..batch_end * num_clusters]
		.iter()
		.sum();
	let avg_n_per_genome =
		(total_neurons_in_batch as f64 / current_batch_size.max(1) as f64) as usize;
	...
	let affinity_ratio_threshold: f64 = std::env::var("WNN_GPU_AFFINITY_RATIO")
		...
		.unwrap_or(100.0);
	let gpu_wins_here =
		max_bits_in_batch > SPARSE_THRESHOLD && parallelism_ratio >= affinity_ratio_threshold;
	...
	let batch_is_homogeneous = (batch_start..batch_end).all(|g| {
		...
		&genomes_neurons_flat[off..off + num_clusters]
			== &genomes_neurons_flat[batch_start * num_clusters..(batch_start + 1) * num_clusters]
	});
	...
	{
		eprintln!(
			"[GPU_BATCHED_TRACE] B12/B13: hybrid eligible (speed_ratio={:.2}, batch={})",
			speed_ratio, current_batch_size
		);
	}
	...
	let shape_grouping_enabled = std::env::var("WNN_SHAPE_GROUP")
		.map(|v| !matches!(v.as_str(), "0" | "off" | "false" | "no"))
		.unwrap_or(true);
	let do_shape_grouping =
		shape_grouping_enabled && use_gpu_batched && !batch_is_homogeneous && !want_hybrid;
...
) -> Vec<(usize, GenomeExport, Option<f64>)>
{
	...
	let shape_group_result: Option<Vec<(usize, GenomeExport, Option<f64>)>> = if do_shape_grouping
	{
		#[cfg(target_os = "macos")]
		{
			...
			let mut per_local_export: Vec<Option<GenomeExport>> =
				(0..current_batch_size).map(|_| None).collect();
			...
			for (shape_neurons, locals) in shape_to_locals.iter()
			{
				...
				let g_conns_slice: &[i64] = if use_provided_connections
				{
					&g_conns
				}
				else
			...
			{
				let mut merged: Vec<(usize, GenomeExport, Option<f64>)> =
	...
	let option_b_result: Option<Vec<GenomeExport>> =
		if use_gpu_batched && !want_hybrid && shape_group_result.is_none() && batch_is_homogeneous
		{
			...
			let neurons_slice =
				&genomes_neurons_flat[batch_start * num_clusters..batch_end * num_clusters];
	...
	else
	{
		...
		if want_hybrid
		{
			...
			let gpu_neurons_slice =
				&genomes_neurons_flat[gpu_batch_start * num_clusters..batch_end * num_clusters];
			...
			let (cpu_results, gpu_result_or_err, cpu_elapsed, gpu_elapsed) = std::thread::scope(|scope| {
				...
				let cpu_res: Vec<(usize, GenomeExport, Option<f64>)> =
					(0..k_cpu).into_par_iter().map(&cpu_one_genome).collect();
			...
			match gpu_result_or_err
			{
				Ok(gpu_exports) =>
				{
					...
					batch_exports.extend(
						gpu_exports
							.into_iter()
							.enumerate()
							.map(|(local_idx, export)| (gpu_batch_start + local_idx, export, None)),
					);
				}
				Err(e) =>
				{
					eprintln!(
						"[GPU_BATCHED] B12 hybrid GPU arm failed (reason: {}); recomputing on CPU",
						e
					);
					// Recompute GPU half on CPU.
					let cpu_fallback: Vec<_> = (k_cpu..current_batch_size)
						.into_par_iter()
						.map(&cpu_one_genome)
						.collect();
					batch_exports = cpu_results;
...
) -> Vec<(f64, f64, f64, f64, f64, u32)>
{
	...
	let GenomeOffsets {
		bpn_offsets: genome_bpn_offsets,
		conn_offsets,
		conn_sizes,
	} = compute_genome_offsets(
		genomes_bits_flat,
		genomes_neurons_flat,
		num_genomes,
		num_clusters,
	...
	for batch_idx in 0..num_batches
	{
		...
		if num_clusters == 1
		{
			...
			else
			{
				...
				for (_, export, threshold) in batch_exports.iter_mut()
				{
					...
					let (t, _f1, _fpr) =
						find_optimal_threshold_auto(&flat_scores, train_targets, settings.fitness_weights);
					*threshold = Some(t);
				}
			}
		}
		...
		{
			((batch_total_secs * 1000.0) / current_batch_size as f64)
				.round()
				.clamp(0.0, u32::MAX as f64) as u32
		}
		else
