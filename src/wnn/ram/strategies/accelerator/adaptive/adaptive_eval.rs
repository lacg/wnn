...
) -> Vec<crate::adaptation::NeuronStats>
{
	...
	let sample_indices =
		crate::adaptation::build_sample_indices(num_train, config.stats_sample_size, 42);
...
{
	let sample_indices =
		crate::adaptation::build_sample_indices(num_train, config.stats_sample_size, 43);
	let n_sample = sample_indices.len();
	...
	(0..num_clusters)
		...
		.map(|cluster| {
			...
			for &ex in &sample_indices
			{
				...
				if majority_true != target_is_this_cluster
				{
					cluster_errors += 1;
				}
			}
			...
			if n_sample > 0
			{
				for local_n in 0..n_neurons
				{
					...
					for s in 0..n_sample
					{
						...
						if my_vote == target_is_this_cluster
						{
							correct += 1;
						}
					}
		...
		for &(local_idx, _old_entropy) in &weak_conns
		{
			...
			for &(cand_conn, _cand_entropy) in &candidates
			{
				...
				for (si, &ex) in sample_indices.iter().enumerate()
				{
					...
					let old_cell = export.read_cell_at(
						group_idx,
						neuron_in_group,
						old_addr as u64,
						ram_core::metal_sparse::default_cell_for_mode(memory_mode) as u8,
					...
					// New prediction
					let new_cell = export.read_cell_at(
						group_idx,
						neuron_in_group,
						new_addr as u64,
						ram_core::metal_sparse::default_cell_for_mode(memory_mode) as u8,
...
) -> Vec<AdaptiveGenomeResult>
{
	...
	if rate == 0.0 || (!adapt_config.synaptogenesis_enabled && !adapt_config.neurogenesis_enabled)
	{
		...
		for g in 0..num_genomes
		{
			...
			let total_n: usize = genomes_neurons_flat[nc_base..nc_base + num_clusters]
				.iter()
				.sum();
		...
		return standard
			.into_iter()
			.enumerate()
			.map(|(g, (ce, acc, f1, fpr, _threshold, _ms))| {
				...
				AdaptiveGenomeResult {
					...
					fpr,
					adapted_bits: bits,
					adapted_neurons: neurons,
					adapted_connections: conns,
					pruned: 0,
					grown: 0,
					added: 0,
					removed: 0,
	...
	for batch_idx in 0..num_batches
	{
		...
		let trained_states: Vec<TrainedState> = (0..current_batch_size)
			...
			.map(|local_idx| {
				...
				let neurons_per_cluster =
					genomes_neurons_flat[genome_offset..genome_offset + num_clusters].to_vec();
				let bpn_start = genome_bpn_offsets[genome_idx];
				let bpn_end = genome_bpn_offsets[genome_idx + 1];
				...
				else
				{
					...
					(0..total_conn)
						.map(|_| rng.gen_range(0..total_input_bits as i64))
						.collect()
				};
		...
		for state in &trained_states
		{
			...
			let neuron_stats = compute_neuron_stats_adaptive(
				&state.per_neuron_bits,
				&state.neurons_per_cluster,
				&state.connections,
				&state.export,
				&state.cluster_to_group,
				&state.cluster_neuron_starts,
				&state.neuron_conn_offsets,
				&packed_train_input,
				words_per_example,
				train_targets,
				num_train,
			...
			adapted_states.push(AdaptedState {
				bits: adapt_bits,
				neurons: adapt_neurons,
				connections: adapt_conns,
				changed,
				pruned: total_pruned,
				grown: total_grown,
				added: total_added,
				removed: total_removed,
				rewired: total_rewired,
				genome_idx: state.genome_idx,
			});
		}

		// Phase 3: Retrain changed genomes + export all (parallel)
		let batch_exports: Vec<(usize, GenomeExport, Option<f64>)> = adapted_states
			.par_iter()
			.enumerate()
	...
	all_adapted
		.into_iter()
		.map(
			|(idx, bits, neurons, conns, pruned, grown, added, removed, rewired)| {
