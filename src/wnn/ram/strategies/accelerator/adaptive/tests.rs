...
mod tests
{
	...
	fn test_build_config_groups()
	{
		...
		// Find the (5,8) group
		let group_5_8 = groups
			.iter()
			.find(|g| g.neurons == 5 && g.bits == 8)
			.unwrap();
		assert_eq!(group_5_8.cluster_ids, vec![0, 1, 4]);

		// Find the (3,10) group
		let group_3_10 = groups
			.iter()
			.find(|g| g.neurons == 3 && g.bits == 10)
			.unwrap();
	...
	fn oi_dense_permutation_invariance()
	{
		...
		for seed in 0..10u64
		{
			...
			assert_eq!(
				snap, baseline,
				"permutation {} produced a different snapshot",
				seed
			);
		}
	}
	...
	fn oi_dense_bin_oracle()
	{
		...
		use ram_core::neuron_memory::{QUAD_FALSE, QUAD_TRUE, QUAD_WEAK_FALSE, QUAD_WEAK_TRUE};
	...
	fn oi_dense_concurrent_nudges_match_serial()
	{
		...
		// Commit and snapshot.
		let mut mem = Arc::try_unwrap(mem)
			.map_err(|_| "Arc still has refs")
			.unwrap();
		mem.commit_oi();
		...
		assert_eq!(
			serial, parallel,
			"concurrent OI nudges produced different cell states than serial"
		);
	}
	...
	{
		use ram_core::neuron_memory::{QUAD_TRUE, QUAD_WEAK_FALSE, QUAD_WEAK_TRUE};
		...
		let expected: Vec<(usize, u64, u8)> =
			vec![(0, 300, QUAD_WEAK_TRUE as u8), (0, 400, QUAD_TRUE as u8)];
	...
	fn oi_sparse_concurrent_match_serial()
	{
		...
		assert_eq!(
			serial, parallel,
			"concurrent OI sparse nudges diverged from serial"
		);
	}
...
mod flat_genome_validation_tests
{
	...
	fn golden_hybrid_inputs() -> (
		...
		Vec<usize>,
		Vec<i64>,
		ram_core::packed_bits::PackedBits,
		Vec<i64>,
		Vec<i64>,
		ram_core::packed_bits::PackedBits,
		Vec<i64>,
	)
	{
		...
		let train_input =
			ram_core::packed_bits::PackedBits::from_bool_bytes(&train_bytes, total_input_bits);
	...
	fn run_golden_hybrid() -> Vec<(f64, f64, f64, f64, f64)>
	{
		...
		res
			.into_iter()
			.map(|(ce, acc, f1, fpr, th, _ms)| (ce, acc, f1, fpr, th))
			.collect()
	}
	...
	fn golden_hybrid_dense_cpu_snapshot()
	{
		...
		const EXPECTED_LEGACY: (f64, f64, f64, f64, f64) = (0.38687095046043396, 1.0, 1.0, 0.0, 0.5);
		const EXPECTED_OI: (f64, f64, f64, f64, f64) = (0.4740769565105438, 1.0, 1.0, 0.0, 0.5);
		...
		for (i, g) in got.iter().enumerate()
		{
			...
			assert_eq!(
				g.3.to_bits(),
				expected.3.to_bits(),
				"genome {i} fpr drift: {g:?}"
			);
			assert_eq!(
				g.4.to_bits(),
				expected.4.to_bits(),
				"genome {i} threshold drift: {g:?}"
			);
		}
	}
	...
	fn eval_in_place_vote_normalization_is_bitwise_exact()
	{
		...
		for &n in &[1usize, 8, 53, 250, 500]
		{
			for _ in 0..200
			{
				...
				assert_eq!(
					export_score.to_bits(),
					fused_score.to_bits(),
					"normalization diverged at n={} votes={}",
					n,
					votes
				);
			}
