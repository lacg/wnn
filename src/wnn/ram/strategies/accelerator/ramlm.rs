...
use ram_core::neuron_memory::{
	BITS_PER_CELL, CELLS_PER_WORD, CELL_MASK, EMPTY, FALSE, QUAD_FALSE, QUAD_TRUE, QUAD_WEAK_TRUE,
	QUAD_WEIGHTS, TRUE,
};
...
) -> usize
{
	...
	for cluster in 0..num_clusters
	{
		...
		let cluster_modified: usize = (0..num_examples)
			.into_par_iter()
			.map(|ex_idx| {
				let input_start = ex_idx * total_input_bits;
				let input_bits = &input_bits_flat[input_start..input_start + total_input_bits];
...
) -> usize
{
	...
	let total_modified: usize = memory_words
		...
		.map(|(neuron_idx, neuron_mem)| {
			...
			let mut rng_state = (rng_seed as u32).wrapping_add(neuron_idx as u32 * 1000003);
			if rng_state == 0
			{
				rng_state = 1;
			}
...
) -> (f64, f64, Vec<f32>)
{
	...
	let per_bit_accuracy: Vec<f32> = per_bit_correct
		.iter()
