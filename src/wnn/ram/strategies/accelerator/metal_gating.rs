...
impl MetalGatingEvaluator
{
	...
	) -> Result<Vec<f32>, String>
	{
		...
		GATING_BUFFER_CACHE.with(|cache_cell| {
			...
			let params_buffer = get_or_create_buffer(
				&self.device,
			...
			let output_buffer = get_or_create_output_buffer(
				&self.device,
...
) -> Result<Vec<f32>, String>
{
	...
	let cpu_batch_size = ((batch_size as f32 * cpu_fraction) as usize)
		.max(1)
		.min(batch_size);
	...
	if gpu_batch_size == 0
	{
		return Ok(gating.forward_batch(inputs, batch_size));
	}

	// Split input as packed row slices
	let cpu_inputs = inputs.slice_rows(0..cpu_batch_size);
	...
	let (cpu_results, gpu_results) = rayon::join(
		|| gating.forward_batch(&cpu_inputs, cpu_batch_size),
...
mod tests
{
	...
	fn test_metal_cpu_equivalence()
	{
		...
		let input_flat: Vec<bool> = (0..batch_size * total_input_bits)
			...
			.collect();
		let pb_batch =
			ram_core::packed_bits::PackedBits::from_bool_slice(&input_flat, total_input_bits);
		...
		let metal_eval = MetalGatingEvaluator::new().unwrap();
		let gpu_gates = metal_eval
			.forward_batch(&gating, &packed, batch_size, wpe)
			.unwrap();
	...
	fn test_metal_gpu_training()
	{
		...
		for b in 0..batch_size
		{
			...
			let gpu_memory = metal_eval
				.train_batch(&gating_gpu, &packed, &target_bools, 1, wpe)
				.unwrap();
		...
		for b in 0..batch_size
		{
			...
			let gpu_gates = gating_gpu.forward_single(row);
