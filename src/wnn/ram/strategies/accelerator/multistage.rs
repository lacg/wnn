...
use crate::adaptive::{
	build_groups, build_neuron_metadata, evaluate_group_metal, evaluate_group_sparse_gpu,
	get_metal_evaluator, get_sparse_metal_evaluator, per_cluster_max_bits,
	reorganize_connections_for_gpu, train_genome_in_slot, try_gpu_addresses_adaptive, GroupMemory,
};
use crate::bitwise_ramlm::{
	bits_needed, compute_genome_layout, evaluate_genomes_with_params, forward_eval_into,
	forward_scores_on_eval, forward_scores_on_subset, pack_input_bits, train_and_get_scores,
	train_and_get_scores_with_model, train_into, train_only, BitwiseEvalSubset, BitwiseSubset,
};
use ram_core::neuron_memory::{cell_to_weight, pack_bools_to_u64, ClusterStorage};
...
impl MultiStageTokenCache
{
	...
	) -> Self
	{
		...
		assert_eq!(
			stage_ctx.len(),
			num_stages,
			"stage_context_sizes length {} != num_stages {}",
			stage_ctx.len(),
			num_stages
		);
		let max_context_size = *stage_ctx.iter().max().unwrap();
		let stage_context_input_bits: Vec<usize> =
			stage_ctx.iter().map(|&cs| cs * bits_per_token).collect();
		...
		let stage_types =
			stage_cluster_types.unwrap_or_else(|| vec!["bitwise".to_string(); num_stages]);
		let stage_is_tiered: Vec<bool> = stage_types
			.iter()
		...
		if let Some(ref custom) = custom_cluster_of
		{
			...
			assert_eq!(
				custom.len(),
				vocab_size,
				"custom_cluster_of length {} != vocab_size {}",
				custom.len(),
				vocab_size
			);
		...
		else if any_tiered
		{
			...
			sorted_tokens.sort_by(|&a, &b| token_freq[b].cmp(&token_freq[a]).then_with(|| a.cmp(&b)));
		...
		{
			stage_input_bits_raw[s] =
				stage_context_input_bits[s] + stage_output_bits_raw[..s].iter().sum::<usize>();
		}

		let stage_num_output_clusters: Vec<usize> = (0..num_stages)
			.map(|s| {
				if stage_is_tiered[s]
				{
					stage_vocab[s]
				}
				else
				{
					stage_output_bits_raw[s]
				}
			})
			.collect();
		...
		for s in 0..num_stages
		{
			...
			// Full train
			let (input_bits, _targets, target_bits, n) = Self::encode_bitwise_stage_sequence(
				&train_tokens,
				s_ctx,
				max_context_size,
				bits_per_token,
			...
			// Full eval
			let (eval_input, eval_targets, _, eval_n) = Self::encode_bitwise_stage_sequence(
				&eval_tokens,
				s_ctx,
				max_context_size,
				bits_per_token,
		...
		let mut tiered_train_subsets: Vec<Option<Vec<TieredSubset>>> =
			(0..num_stages).map(|_| None).collect();
		let mut tiered_eval_subsets: Vec<Option<Vec<TieredEvalSubset>>> =
			(0..num_stages).map(|_| None).collect();
		let mut tiered_full_train: Vec<Option<TieredSubset>> = (0..num_stages).map(|_| None).collect();
		let mut tiered_full_eval: Vec<Option<TieredEvalSubset>> =
			(0..num_stages).map(|_| None).collect();
		...
		for stage in 0..num_stages
		{
			...
			let train_input_pb =
				ram_core::packed_bits::PackedBits::from_bool_slice(&train_input, stage_input);
			drop(train_input);
			...
			// Encode full eval
			let (eval_input, eval_targets_i64, _, eval_n) = Self::encode_tiered_stage_sequence(
				&eval_tokens,
				t_ctx,
				max_context_size,
				bits_per_token,
			...
			let eval_input_pb =
				ram_core::packed_bits::PackedBits::from_bool_slice(&eval_input, stage_input);
			drop(eval_input);
		...
		// ── Logging ──────────────────────────────────────────────────
		let mapping_type = if any_tiered
		{
			"freq-interleaved"
		}
		else
		...
		// Build KN bigram tables from training tokens
		{
			...
			// P_cont(cur)
			let kn_pcont: Vec<f64> = continuation_count
				.iter()
				.map(|&c| {
					if total_unique_bigrams > 0
					{
						c as f64 / total_unique_bigrams as f64
					}
					else
					{
			...
			let kn_lambda: Vec<f64> = (0..vocab_size)
				.map(|prev| {
			...
			let kn_first_term: Vec<HashMap<u32, f64>> = (0..vocab_size)
				.map(|prev| {
			...
			let kn_bigram_argmax: Vec<u32> = (0..vocab_size)
				.map(|prev| {
					...
					else
					{
						...
						let pcont_argmax = kn_pcont
							.iter()
							.enumerate()
							.max_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap())
							.map(|(idx, _)| idx)
							.unwrap_or(0);
						let p_pcont = kn_first_term[prev]
							.get(&(pcont_argmax as u32))
							.copied()
							.unwrap_or(0.0)
							+ lam * kn_pcont[pcont_argmax];
						if p_pcont > best_p
						{
							best_tok = pcont_argmax as u32;
						}
						best_tok
					}
				})
		...
		input_bits
			...
			.for_each(|(i, ((inp, tgt), tb))| {
				...
				else
				{
					...
					{
						inp[context_input_bits + b] = ((cluster_id >> (bits_per_cluster_id - 1 - b)) & 1) == 1;
					}
		...
		(0..num_parts)
			.map(|i| {
				...
				let part_preds = predicted_clusters.map(|p| {
					let num_ex = if end > start + max_context_size
					{
						end - start - max_context_size
					}
					else
					{
						0
				...
				let (input_bits, _, target_bits, num_ex) = Self::encode_bitwise_stage_sequence(
					part,
					context_size,
					max_context_size,
					bits_per_token,
	...
	) -> Vec<BitwiseEvalSubset>
	{
		...
		(0..num_parts)
			.map(|i| {
				...
				let (input_bits, targets, _, num_ex) = Self::encode_bitwise_stage_sequence(
					part,
					context_size,
					max_context_size,
					bits_per_token,
	...
	) -> (BitwiseSubset, BitwiseEvalSubset)
	{
		...
		let group_indices: Vec<usize> = (0..total_ex)
			.filter(|&i| match predicted_clusters
			{
	...
	) -> Vec<TieredSubset>
	{
		...
		(0..num_parts)
			.map(|i| {
				...
				let (input_bits, targets, negatives, num_ex) = Self::encode_tiered_stage_sequence(
					part,
					context_size,
					max_context_size,
					bits_per_token,
				...
				let input_bits_pb =
					ram_core::packed_bits::PackedBits::from_bool_slice(&input_bits, total_input_bits);
				drop(input_bits);
				TieredSubset {
	...
	) -> Vec<TieredEvalSubset>
	{
		...
		(0..num_eval_parts)
			.map(|i| {
				...
				let (input_bits, targets, _, num_ex) = Self::encode_tiered_stage_sequence(
					part,
					context_size,
					max_context_size,
					bits_per_token,
	...
	)
	{
		assert!(
			target_stage > 0,
			"Cannot recompute stage 0 — it has no previous stage"
		);
		let s = target_stage;
		...
		let (input_bits, _targets, target_bits, n) = Self::encode_bitwise_stage_sequence(
			...
			total_input,
			target_bits_count,
			prev_output_bits_slice,
			&self.cluster_of,
			&self.index_in_cluster,
		...
		if self.stage_is_tiered[s]
		{
			...
			// Full eval
			let (eval_input_t, eval_targets_i64, _, eval_n_t) = Self::encode_tiered_stage_sequence(
				&self.eval_tokens,
				s_ctx,
				self.max_context_size,
				self.bits_per_token,
			...
			let eval_input_t_pb =
				ram_core::packed_bits::PackedBits::from_bool_slice(&eval_input_t, stage_input);
			drop(eval_input_t);
		...
		eprintln!("[recompute_stage_data] Re-encoded S{s}: bitwise train={n}, eval={eval_n}");
	}
	...
	if reweight_rounds > 0
	{
		...
		for round in 0..reweight_rounds
		{
			...
			let weights = compute_reweight_weights(
				&train_scores,
				&cache.bitwise_full_train[stage].target_bits,
			...
			for cs in cluster_storage.iter_mut()
			{
				cs.reset();
			}
			train_into(
				...
				memory_mode,
				neuron_sample_rate,
				rng_seed.wrapping_add(round as u64 + 1),
			);
...
) -> (f64, f64, f64, f64, f64, f64)
{
	...
	for s in 0..num_stages
	{
		if cache.stage_is_tiered[s]
		{
			...
			stage_tiered_scores[s] = train_and_get_tiered_scores(
				stage_connections[s],
				stage_bits_per_neuron[s],
				stage_neurons_per_cluster[s],
				cache.bitwise_vocab_size[s],
				train,
				eval,
				cache.stage_input_bits[s],
				memory_mode,
				neuron_sample_rate,
				rng_seed.wrapping_add(s as u64),
			);
		}
		else
		{
			stage_bitwise_scores[s] = train_and_get_scores(
				...
				&cache.bitwise_full_train[s],
				&cache.bitwise_full_eval[s],
				memory_mode,
				empty_value,
				neuron_sample_rate,
				rng_seed.wrapping_add(s as u64),
	...
	let unigram_argmax: usize = cache
		.unigram_probs
		.iter()
		.enumerate()
		.max_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap())
		...
		.map(|ex| {
			...
			else
			{
				...
				for gk in 0..k
				{
					...
					if gk == true_group
					{
						log_p_true_raw = log_p;
					}
					if log_p > predicted_lp
					{
						predicted_lp = log_p;
						predicted = gk;
					}
				...
				let p_bi = cache
					...
					+ cache.kn_lambda.get(prev_tok).copied().unwrap_or(0.0)
						* cache
							...
							.get(token_id)
							.copied()
							.unwrap_or(1.0 / cache.vocab_size as f64);
			...
			{
				let p_hier = log_p_hier.exp();
				let p_smooth = (1.0 - label_smoothing) * p_hier + label_smoothing / cache.vocab_size as f64;
				-p_smooth.ln()
			}
			else
			{
			...
			if any_interp
			{
				...
				let s0_correct = if s0_predicted == true_group
				{
					1u32
				}
				else
				...
				let s1_correct = if s1_predicted == true_within
				{
					1u32
				}
				else
			...
			let correct = if s0_predicted == true_group && s1_predicted == true_within
			{
				1u32
			}
			else
	...
	(0..k)
		.map(|g| {
			...
			for (i, &(global_idx, is_in_group)) in included.iter().enumerate()
			{
				...
				packed_input[i * wpe..(i + 1) * wpe].copy_from_slice(
					&cache.bitwise_full_train[s1_stage].packed_input[src_start..src_start + wpe],
				);
...
) -> (f64, f64, f64, f64, f64, f64)
{
	...
	let eval_group_probs: Vec<f32> = if !stage_bitwise_scores_0.is_empty()
	{
		compute_group_softmax_bitwise(
			stage_bitwise_scores_0,
			num_eval,
			cache.bitwise_output_bits[0],
			k,
			&cache.bitwise_token_bits[0],
		)
	}
	else
	{
	...
	let results: Vec<(f64, f64, f64, u32, u32, u32)> = (0..num_eval)
		...
		.map(|ex| {
			...
			for g in 0..k
			{
				...
				let best_j = class_log_probs[..group_size]
					.iter()
					.enumerate()
					.max_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap())
					.map(|(idx, _)| idx)
			...
			{
				if best_predicted_g == true_group && best_predicted_within == true_within
				{
					1u32
				}
				else
			...
			let s0_correct = if best_predicted_g == true_group
			{
				1u32
			}
			else
			...
			let s1_correct = if best_predicted_within == true_within
			{
				1u32
			}
			else
...
) -> (f64, f64, f64, f64, f64, f64)
{
	...
	if cache.stage_is_tiered[0]
	{
		...
		stage_tiered_scores_0 = train_and_get_tiered_scores(
			stage_connections[0],
			stage_bits_per_neuron[0],
			stage_neurons_per_cluster[0],
			cache.bitwise_vocab_size[0],
			train,
			eval,
			cache.stage_input_bits[0],
			memory_mode,
			neuron_sample_rate,
			rng_seed,
		);
	...
	else
	{
		let (scores, clusters, layout) = train_and_get_scores_with_model(
			...
			stage_neurons_per_cluster[0],
			cache.bitwise_output_bits[0],
			&cache.bitwise_full_train[0],
			&cache.bitwise_full_eval[0],
			memory_mode,
			empty_value,
			neuron_sample_rate,
			rng_seed,
	...
	if invalid_mode
	{
		...
		let augmented_subsets =
			build_augmented_selector_subsets(cache, s1_stage, &tgp, s1_output_bits, top_m);

		// Train each group on augmented data, then forward on ALL eval examples
		let all_group_eval_scores: Vec<Vec<f32>> = (0..k)
			.map(|g| {
				if augmented_subsets[g].num_examples == 0
				{
					return Vec::new();
				}
				let (clusters, layout) = train_only(
					...
					memory_mode,
					neuron_sample_rate,
					rng_seed.wrapping_add(1 + g as u64),
					sparse_threshold,
				);
				forward_scores_on_eval(
					...
					s1_output_bits,
					&layout,
					&cache.bitwise_full_eval[s1_stage],
					&clusters,
					memory_mode,
					empty_value,
				)
		...
		return eval_filtered_mixture(
			cache,
			&stage_bitwise_scores_0,
			&stage_tiered_scores_0,
			&all_group_eval_scores,
			s1_output_bits,
			label_smoothing,
			unigram_lambda,
		);
...
) -> Vec<f64>
{
	...
	let connections_flat =
		reorganize_connections_for_gpu(connections, bits_per_neuron, neurons_per_cluster, &groups);
			...
			if let Some(sparse_eval) = get_sparse_metal_evaluator()
			{
				if let Some(export) = memory.export_for_gpu_sparse()
				{
					if let Ok(group_scores) = evaluate_group_sparse_gpu(
						...
						num_eval,
						words_per_example,
						false, // coverage_aware: LM multistage, not the IDS scorer
						memory_mode,
						empty_value,
						0, // run_seed: LM multistage is never QSR/PLN
					)
...
mod cpu_gpu_parity_tests
{
	...
	use crate::adaptive::{
		build_groups, build_neuron_metadata, evaluate_group_metal, evaluate_group_sparse_gpu,
		get_metal_evaluator, get_sparse_metal_evaluator, per_cluster_max_bits,
		reorganize_connections_for_gpu, train_genome_in_slot, GroupMemory,
	};
	...
	fn run_cpu_gpu_parity(bits: usize)
	{
		...
		let train_targets: Vec<i64> = (0..num_train)
			.map(|_| rng.below(num_clusters) as i64)
			.collect();
		let train_negatives: Vec<i64> = train_targets
			.iter()
		...
		let (packed_eval, words_per_example) = ram_core::neuron_memory::pack_packed_to_u64(&eval_input);
		...
		assert_eq!(
			compared,
			num_eval * num_clusters,
			"parity test must cover all scores"
		);
		...
		assert!(
			any_trained,
			"no score deviated from the untrained baseline — test is vacuous"
		);
	}
}
