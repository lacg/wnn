//! Dense RAMLM forward pass on Metal GPU (language modeling).
//!
//! LM-domain ONLY: the dense forward over a large vocabulary (50K clusters),
//! using `ramlm.metal`. The IDS genome-batch evaluators live in
//! `metal_genome_eval.rs`; the shared sparse forward lives in
//! `ram_core::metal_sparse`. Slimmed to LM-only in the 2026-06-19 crate split.
	...
	) -> Result<Vec<f32>, String>
	{
		...
		else
		{
			...
			let thread_group_size =
				MTLSize::new(32.min(num_clusters as u64), 8.min(num_examples as u64), 1);
			encoder.dispatch_threads(grid_size, thread_group_size);
		}
		...
		let results: Vec<f32> = unsafe { std::slice::from_raw_parts(result_ptr, output_size).to_vec() };
