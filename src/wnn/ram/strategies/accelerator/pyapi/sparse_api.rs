...
impl SparseMemory
{
	...
	{
		Self {
			inner: Arc::new(ram_core::sparse_memory::SparseLayerMemory::new(
				num_neurons,
				bits_per_neuron,
			)),
			num_neurons,
	...
	fn write_cell(&self, neuron_idx: usize, address: u64, value: u8, allow_override: bool) -> bool
	{
		self
			.inner
			.write_cell(neuron_idx, address, value, allow_override)
	}
...
{
	let target_bits_flat: Vec<u8> = target_bits_flat
		.as_slice()
		.expect("target_bits_flat must be contiguous")
		.to_vec();
	let connections_flat: Vec<i64> = connections_flat
		.as_slice()
		.expect("connections_flat must be contiguous")
		.to_vec();
