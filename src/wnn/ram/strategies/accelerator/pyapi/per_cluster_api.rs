...
) -> PyResult<Vec<f32>>
{
	...
	for i in 0..num_groups
	{
		...
		let cluster_ids =
			group_cluster_ids_flat[cluster_offset..cluster_offset + cluster_count].to_vec();
...
) -> PyResult<Vec<(f64, f64, f64, f64)>>
{
	...
	let train_negatives_slice = train_negatives.as_slice().map_err(|e| {
		PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
			"Train negatives not contiguous: {}",
			e
		))
	})?;
	...
	let train_input_packed =
		ram_core::packed_bits::PackedBits::from_bool_bytes(train_input_slice, total_input_bits);
	let eval_input_packed =
		ram_core::packed_bits::PackedBits::from_bool_bytes(eval_input_slice, total_input_bits);
