...
impl RAMGatingWrapper
{
	...
	) -> Vec<f32>
	{
		...
		let input_slice = input_bits_flat
			.as_slice()
	...
	) -> usize
	{
		...
		let input_slice = input_bits_flat
			...
			.expect("input_bits_flat must be contiguous");
		let packed = ram_core::packed_bits::PackedBits::from_bool_bytes(input_slice, total_bits);
		let target_bools: Vec<bool> = target_gates_flat
			.as_slice()
		...
		py.allow_threads(|| {
			self
				.inner
				.train_batch(&packed, &target_bools, batch_size, allow_override)
		})
	}
	...
	) -> PyResult<Vec<f32>>
	{
		...
		let input_slice = input_bits_flat
			...
			.expect("input_bits_flat must be contiguous");
		let pb =
			ram_core::packed_bits::PackedBits::from_bool_bytes(input_slice, config.total_input_bits);
		py.allow_threads(|| {
			...
			let evaluator = guard.as_ref().ok_or_else(|| {
				pyo3::exceptions::PyRuntimeError::new_err("Metal gating evaluator not initialized")
			})?;
			evaluator
				.forward_batch(&self.inner, &packed, batch_size, wpe)
				.map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e))
		})
	}
	...
	{
		let input_bools: Vec<bool> = input_bits_flat
			.as_slice()
			.expect("input_bits_flat must be contiguous")
			.iter()
			.map(|&b| b != 0)
		...
		py.allow_threads(|| {
			let config = self.inner.config();
			let (packed, wpe) = ram_core::neuron_memory::pack_bools_to_u64(
				&input_bools,
				batch_size,
			...
			let updated_memory = evaluator
				.train_batch(&self.inner, &packed, &target_bools, batch_size, wpe)
				.map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e))?;

			// Import the updated memory back into the gating model
			self
				.inner
				.import_memory(&updated_memory)
				.map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e))?;
...
#[pyfunction]
pub(crate) fn compute_target_gates<'py>(
	py: Python<'py>,
	targets: Vec<i64>,
	num_clusters: usize,
) -> pyo3::Bound<'py, numpy::PyArray1<u8>>
{
