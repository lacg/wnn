...
impl IDSCacheBuilderWrapper
{
	...
	) -> PyResult<()>
	{
		...
		let pb =
			ram_core::packed_bits::PackedBits::from_packed_bytes(slice.to_vec(), state.total_features());
	...
	fn num_train(&self) -> PyResult<usize>
	{
		self
			...
			.as_ref()
			.map(|s| s.num_train())
			.ok_or_else(|| pyo3::exceptions::PyRuntimeError::new_err("IDSCacheBuilder already finalized"))
	}
