...
impl TieredSparseMemory
{
	...
	{
		Self {
			inner: Arc::new(ram_core::sparse_memory::TieredSparseMemory::new(
				&tier_configs,
				num_clusters,
			)),
			num_clusters,
...
) -> PyResult<SparseGpuCache>
{
	...
	let inner_refs: Vec<Arc<ram_core::sparse_memory::TieredSparseMemory>> =
		memories.iter().map(|m| Arc::clone(&m.inner)).collect();
	...
	py.allow_threads(|| {
		...
		for (group_idx, inner) in inner_refs.iter().enumerate()
		{
			...
			for (local_idx, &(neurons, bits, start_neuron, conn_offset)) in
				export.cluster_infos.iter().enumerate()
			{
			...
			let group_conn_size: u32 = export.cluster_infos.iter().map(|&(n, b, _, _)| n * b).sum();
...
) -> PyResult<Py<numpy::PyArray1<f32>>>
{
	...
	let probs = py
		.allow_threads(|| {
			let (packed_input, wpe) =
				ram_core::neuron_memory::pack_bools_to_u64(&input_bools, num_examples, total_input_bits);
			cache
				.evaluator
...
) -> PyResult<Vec<(String, bool, String, f64, f64)>>
{
	...
	let trainer =
		MarkerTrainer::new().map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e))?;
	...
	let markers_slice =
		unsafe { std::slice::from_raw_parts(markers_buf.contents() as *const u32, total_slots) };
	let keys_slice =
		unsafe { std::slice::from_raw_parts(keys_buf.contents() as *const u64, total_slots) };
	let values_slice =
		unsafe { std::slice::from_raw_parts(values_buf.contents() as *const u32, total_slots) };
	...
	// (also confirms the export is consumable by the eval path).
	{
		...
		for n in 0..num_neurons
		{
			...
			let from_export: Vec<(u64, u8)> = (0..cnt)
				.map(|i| (keys[start + i], values[start + i]))
				.collect();
...
) -> PyResult<Vec<(String, bool, String, f64, f64)>>
{
	...
	for g in 0..num_genomes
	{
		...
		for ex_idx in 0..num_examples
		{
			...
			for n_local in 0..neurons_per_cluster
			{
				...
				if should_skip(n_global, ex_idx)
				{
					continue;
				}
	...
	results.push((
		"marker_train_multicluster_parity".into(),
		ok,
		detail,
		gpu_total_ms,
		cpu_ms,
...
pub(crate) fn run_marker_hashtable_tests() -> PyResult<Vec<(String, bool, String)>>
{
	...
	// basic_write_read
	{
		...
		if !t.write(42, 1, false)
		{
			ok = false;
			why.push_str("write returned false; ");
		}
		if t.read(42) != 1
		{
			ok = false;
			why.push_str(&format!("read(42)={} expected 1; ", t.read(42)));
		}
		if t.read(99) != 2
		{
			ok = false;
			why.push_str(&format!("read(99)={} expected default 2; ", t.read(99)));
		}
		if t.len() != 1
		{
			ok = false;
			why.push_str(&format!("len={} expected 1; ", t.len()));
		}
	...
	// true_wins_over_false
	{
		...
		let mut ok = true;
		let mut why = String::new();
		if !t.write(7, 1, false)
		{
			ok = false;
			why.push_str("initial write TRUE failed; ");
		}
		if t.write(7, 0, false)
		{
			ok = false;
			why.push_str("FALSE-over-TRUE was accepted; ");
		}
		if t.read(7) != 1
		{
			ok = false;
			why.push_str(&format!("read(7)={} expected 1; ", t.read(7)));
		}
	...
	// nudge_clamping
	{
		...
		{
			ok = false;
			why.push_str(&format!("after 1 nudge_true: {} expected 2; ", t.read(11)));
		}
		t.nudge(11, true);
		if t.read(11) != 3
		{
			ok = false;
			why.push_str(&format!("after 2 nudge_true: {} expected 3; ", t.read(11)));
		}
		if t.nudge(11, true)
		{
			ok = false;
			why.push_str("saturated nudge_true reported true; ");
		}
		if t.read(11) != 3
		{
			ok = false;
			why.push_str(&format!("saturated value: {} expected 3; ", t.read(11)));
		}
		t.nudge(11, false);
		if t.read(11) != 2
		{
			ok = false;
			why.push_str(&format!("after nudge_false: {} expected 2; ", t.read(11)));
		}
	...
	// resize_under_load
	{
		...
		for k in 0u64..1000
		{
			if !t.write(k, 1, false)
			{
				ok = false;
				why.push_str(&format!("write {} failed; ", k));
				break;
			}
		}
		if t.len() != 1000
		{
			ok = false;
			why.push_str(&format!("len={} expected 1000; ", t.len()));
		}
		for k in 0u64..1000
		{
			if t.read(k) != 1
			{
				ok = false;
				why.push_str(&format!("read {}={} expected 1; ", k, t.read(k)));
				break;
			}
		}
	...
	// parallel_writes_distinct_keys
	{
		...
		{
			format!("len={} expected 10000; ", t.len())
		};
	...
	// parallel_nudges_same_keys (the contention stress test)
	{
		...
		for k in 0u64..10
		{
			...
			{
				ok = false;
				why.push_str(&format!("key {} read={} expected 3; ", k, t.read(k)));
			}
		}
	...
	// snapshot_sorted
	{
		...
		{
			format!("snap={:?} expected {:?}", snap, expected)
		};
	...
	// parity_with_atomic_random_workload
	{
		...
		{
			format!(
				"DIVERGED: atomic {} vs marker {} entries",
				snap_a.len(),
				snap_m.len()
			)
		};
	...
	#[cfg(target_os = "macos")]
	{
		...
		else
		{
			...
			{
				format!(
					"DIVERGED: metal {} vs heap {} entries",
					snap_metal.len(),
					snap_heap.len()
				)
			};
...
) -> PyResult<Vec<(String, bool, u64, u64, String, f64)>>
{
	...
	{
		let results =
			metal_atomic_test::run_microbench(num_slots, gpu_threads, cpu_threads, iterations)
				.map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e))?;
		Ok(
			results
				.into_iter()
