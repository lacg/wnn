...
impl IDSCacheWrapper
{
	...
	) -> Self
	{
		...
		let train_packed =
			ram_core::packed_bits::PackedBits::from_bool_slice(&train_features, total_features);
		let eval_packed =
			ram_core::packed_bits::PackedBits::from_bool_slice(&eval_features, total_features);
		// Protocol v2: optional val partition, packed like the eval set.
		let val_packed = val_features
			.map(|vf| ram_core::packed_bits::PackedBits::from_bool_slice(&vf, total_features));
		Self {
	...
	) -> PyResult<Self>
	{
		...
		let train_slice = train_features.as_slice().map_err(|e| {
			PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
				"train_features array not contiguous: {}",
				e
			))
		})?;
		let eval_slice = eval_features.as_slice().map_err(|e| {
			PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
				"eval_features array not contiguous: {}",
				e
			))
		})?;
		...
		let train_packed =
			ram_core::packed_bits::PackedBits::from_packed_bytes(train_slice.to_vec(), total_features);
		let eval_packed =
			ram_core::packed_bits::PackedBits::from_packed_bytes(eval_slice.to_vec(), total_features);
		...
		let val_packed = match &val_features
		{
			Some(vf) =>
			{
				let val_slice = vf.as_slice().map_err(|e| {
					PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
						"val_features array not contiguous: {}",
						e
					))
				})?;
	...
	{
		if self.inner.num_genome_clusters() != self.inner.num_classes() || self.inner.num_classes() < 2
		{
