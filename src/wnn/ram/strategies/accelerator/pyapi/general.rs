...
#[pyfunction]
pub(crate) fn fit_platt_scaling_py(scores: Vec<f64>, labels: Vec<i64>) -> PyResult<(f64, f64, f64)> {
    Ok(adaptive::fit_platt_scaling(&scores, &labels))
}

/// Fit Beta calibration. Returns (threshold, a, b, c).
#[pyfunction]
pub(crate) fn fit_beta_calibration_py(scores: Vec<f64>, labels: Vec<i64>) -> PyResult<(f64, f64, f64, f64)> {
    Ok(adaptive::fit_beta_calibration(&scores, &labels))
}

/// Fit empirical threshold. Returns (threshold, n_bins).
#[pyfunction]
pub(crate) fn fit_empirical_threshold_py(scores: Vec<f64>, labels: Vec<i64>) -> PyResult<(f64, usize)> {
...
#[pyfunction]
pub(crate) fn find_optimal_threshold_f1_py(scores: Vec<f64>, labels: Vec<i64>) -> PyResult<(f64, f64, f64)> {
