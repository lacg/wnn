...
impl MetalGroupEvaluator
{
	...
	)
	{
		...
		let all_buffers = BATCHED_SPARSE_BUFFER_POOL.with(|pool| {
			...
			for (idx, group) in sparse_groups.iter().enumerate()
			{
				...
				let conn = get_or_create_buffer(
					&self.device,
					&mut pool.conn_buffers[idx],
					connections_i32,
					current_gen,
				);
				let keys = get_or_create_buffer(
					&self.device,
					&mut pool.keys_buffers[idx],
					group.keys,
					current_gen,
				);
				let values = get_or_create_buffer(
					&self.device,
					&mut pool.values_buffers[idx],
					group.values,
					current_gen,
				);
				let offsets = get_or_create_buffer(
					&self.device,
					&mut pool.offsets_buffers[idx],
					group.offsets,
					current_gen,
				);
				let counts = get_or_create_buffer(
					&self.device,
					&mut pool.counts_buffers[idx],
					group.counts,
					current_gen,
				);
				let cluster_ids = get_or_create_buffer(
					&self.device,
					&mut pool.cluster_ids_buffers[idx],
					cluster_ids_u32,
					current_gen,
				);
				let params = get_or_create_buffer(
					&self.device,
					&mut pool.params_buffers[idx],
					params_slice,
					current_gen,
				);
				...
				{
					Some(get_or_create_buffer(
						&self.device,
						&mut pool.actual_neurons_buffers[idx],
						actual_neurons,
						current_gen,
					))
				}
		...
		for (idx, group) in sparse_groups.iter().enumerate()
		{
			...
			let (
				...
				ref offsets_buffer,
				ref counts_buffer,
				ref cluster_ids_buffer,
				ref params_buffer,
				ref actual_neurons_buf,
			) = all_buffers[idx];
	...
	)
	{
		...
		let (conn_buffer, memory_buffer, cluster_ids_buffer, params_buffer) =
			DENSE_BUFFER_CACHE.with(|cache| {
				...
				let conn = get_or_create_buffer(
					&self.device,
					&mut cache.conn_buffer,
					&connections_i32,
					current_gen,
				);
				let memory = get_or_create_buffer(
					&self.device,
					&mut cache.memory_buffer,
					memory_words,
					current_gen,
				);
				let cluster_ids = get_or_create_buffer(
					&self.device,
					&mut cache.cluster_ids_buffer,
					&cluster_ids_u32,
					current_gen,
				);
				let params = get_or_create_buffer(
					&self.device,
					&mut cache.params_buffer,
					params_slice,
					current_gen,
				);
	...
	) -> Result<(f64, f64, Vec<u32>), String>
	{
		...
		let (targets_buffer, ce_buffer, correct_buffer, predicted_buffer) =
			CE_BUFFER_CACHE.with(|cache| {
				let mut cache = cache.borrow_mut();
				let targets_i32: Vec<i32> = targets.iter().map(|&t| t as i32).collect();
				...
				let ce_buf = if let Some(ref cached) = cache.ce_buffer
				{
					...
					else
					{
						let buf = self
							.device
							.new_buffer(required_ce_bytes, MTLResourceOptions::StorageModeShared);
...
mod tests
{
	...
	fn test_sparse_ce_evaluator_creation()
	{
		if metal_present()
		{
			...
			assert!(
				evaluator.is_ok(),
				"Failed to create sparse CE evaluator: {:?}",
				evaluator.err()
			);
		}
	}
	...
	fn test_qsr_cpu_gpu_coin_parity()
	{
		...
		let packed_input: Vec<u64> = (0..num_examples)
			.map(|e| (0xA5u64).wrapping_mul(e as u64 + 1) & 0xFFFF)
			.collect();
		let connections: Vec<i64> = (0..total_neurons)
			.flat_map(|_| vec![0i64, 1, 2, 3])
			.collect();
		...
		let values: Vec<u8> = (0..total_neurons)
			.map(|i| {
				if i % 2 == 0
				{
					1u8
		...
		let offsets: Vec<u32> = (0..total_neurons).map(|i| i as u32).collect();
		let counts: Vec<u32> = vec![1; total_neurons];
		let targets: Vec<i64> = (0..num_examples)
			.map(|e| (e % num_clusters) as i64)
			.collect();
		let run_seed = 0xC0FFEE_1234_5678u64;

		let (_ce, _acc, gpu_pred) = evaluator
			.compute_ce(
				&packed_input,
				&connections,
				&keys,
		...
		for e in 0..num_examples
		{
			...
			assert_eq!(
				...
				"QSR CPU/GPU prediction mismatch at example {e}: gpu={} cpu={best_c}",
				gpu_pred[e]
			);
		}
	}
