...
pub(crate) fn build_sample_indices(num_examples: usize, sample_size: usize, seed: u64)
	-> Vec<usize>
{
...
) -> Vec<u32>
{
	...
	for &ex in sample_indices
	{
		...
		for (word_i, &word) in packed_input[row_start..row_start + words_per_example]
			.iter()
			.enumerate()
		{
			if word == 0
...
) -> Vec<NeuronStats>
{
	...
	let bit_ones = precompute_bit_ones(
		packed_input,
		words_per_example,
		&sample_indices,
	...
	neuron_info
		...
		.map(|&(global_n, cluster, local_n)| {
			...
			let conn_entropy: Vec<f32> = neuron_conns
				.iter()
				.map(|&conn_idx| {
...
) -> Vec<ClusterStats>
{
	...
	(0..num_clusters)
		...
		.map(|cluster| {
			...
			{
				(0..n_neurons)
					.map(|i| neuron_stats[neuron_base + i].fill_rate)
					.sum::<f32>()
					/ n_neurons as f32
			}
			else
			...
			let majority_votes: Vec<bool> = (0..n_sample)
				.map(|s| {
			...
			if n_sample > 0
			{
				for local_n in 0..n_neurons
				{
					...
					for s in 0..n_sample
					{
						...
						if my_vote != majority_votes[s]
						{
							disagree += 1;
						}
						if my_vote == target_true
						{
							correct += 1;
						}
					}
...
) -> (usize, usize)
{
	...
	let n_sample = sample_indices.len();
	let bit_entropy: Vec<f32> = bit_ones
		.iter()
		.map(|&ones| {
			let p = ones as f32 / n_sample.max(1) as f32;
			if p > 0.0 && p < 1.0
	...
	for n in 0..total_neurons
	{
		...
		if n_bits > config.min_bits
		{
			...
			if let Some((min_idx, &min_entropy)) = stats
				.connection_entropy
				.iter()
				.enumerate()
			...
			&& rng.gen::<f32>() < rate
		{
			...
			{
				if used.contains(&(bit_idx as i64))
				{
					continue;
				}
	...
	if !modifications.is_empty()
	{
		...
		let mods: std::collections::HashMap<usize, &Modification> =
			modifications.iter().map(|(n, m)| (*n, m)).collect();
	...
	{
		eprintln!(
			"[Adapt] Synaptogenesis: pruned={}, grown={} (rate={:.2})",
			pruned, grown, rate
		);
	}
...
) -> usize
{
	...
	for n in 0..total_neurons
	{
		...
		let mut weak_conns: Vec<(usize, f32)> = stats
			.connection_entropy
			.iter()
			.enumerate()
		...
		let existing_bit_vals: Vec<Option<&Vec<u8>>> = (0..n_bits)
			.map(|i| {
				...
				if conn < config.total_input_bits
				{
					bit_vals[conn].as_ref()
				}
				else
				{
					None
		...
		for &(local_idx, _old_entropy) in &weak_conns
		{
			...
			for &(cand_conn, _cand_entropy) in &candidates
			{
				...
				if connections[conn_start..conn_start + n_bits].contains(&cand_conn)
				{
					continue;
				}
				...
				for (si, &ex) in sample_indices.iter().enumerate()
				{
					...
					let row_start = ex * words_per_example;
					let old_bit =
						(packed_input[row_start + old_conn as usize / 64] >> (old_conn as usize % 64)) & 1;
					let new_bit =
						(packed_input[row_start + cand_conn as usize / 64] >> (cand_conn as usize % 64)) & 1;
				...
				if let Some(cand_vals) = &bit_vals[cand_conn as usize]
				{
					for (i, existing) in existing_bit_vals.iter().enumerate()
					{
						...
						{
							continue;
						} // Skip the connection being replaced
	...
	{
		eprintln!(
			"[Adapt] Axonogenesis: rewired={} (rate={:.2})",
			rewired, rate
		);
	}
...
) -> (usize, usize)
{
	...
	for cluster in 0..num_clusters
	{
		...
		{
			(0..n_neurons)
				.map(|i| bits_per_neuron[neuron_base + i])
				.sum::<usize>()
				/ n_neurons
		}
		else
			...
			&& rng.gen::<f32>() < rate
		{
			...
			let neurons_to_add = 1
				.min(max_neurons - n_neurons)
				.min(config.max_neurons_per_pass);
			for _ in 0..neurons_to_add
			{
				mods.push(ClusterMod {
					cluster,
					action: ClusterAction::AddNeuron {
						template_local: best_local,
					},
				});
				added += 1;
			}
	...
	if !mods.is_empty()
	{
		...
		let mut cluster_adds: std::collections::HashMap<usize, Vec<usize>> =
			std::collections::HashMap::new();
		let mut cluster_removes: std::collections::HashMap<usize, Vec<usize>> =
			std::collections::HashMap::new();
		for m in &mods
		{
			match &m.action
			...
			if let Some(add_list) = adds
			{
				for &template_local in add_list
				{
					...
					for i in 0..new_n_bits
					{
						...
						let new_conn = (old_conn + delta)
							.max(0)
							.min(config.total_input_bits as i64 - 1);
						new_connections.push(new_conn);
					}
					kept += 1;
				}
	...
	{
		eprintln!(
			"[Adapt] Neurogenesis: added={}, removed={} (rate={:.2})",
			added, removed, rate
		);
	}
...
) -> (Vec<NeuronStats>, Vec<ClusterStats>)
{
	...
	let conn_entropies: Vec<Vec<f32>> = (0..total_neurons)
		.map(|n| {
			let n_bits = bits_per_neuron[n];
			let conn_start = conn_offsets[n];
			connections[conn_start..conn_start + n_bits]
				.iter()
				.map(|&conn_idx| {
					let ones = bit_ones[conn_idx as usize];
					let p = ones as f32 / n_sample.max(1) as f32;
	...
	// GPU computation
	let (error_counts, filled_counts, cluster_errors, uniqueness_counts, accuracy_counts) = metal
		.compute_all_stats(
			packed_input,
			words_per_example,
			connections,
	...
	// Build NeuronStats
	let neuron_stats: Vec<NeuronStats> = (0..total_neurons)
		.map(|n| {
			let n_bits = bits_per_neuron[n];
	...
	let cluster_stats: Vec<ClusterStats> = (0..num_clusters)
		.map(|c| {
			...
			let uniqueness: Vec<f32> = (0..n_neurons)
				.map(|i| {
			...
			let accuracy: Vec<f32> = (0..n_neurons)
				.map(|i| {
					if cluster_sample_size > 0
					{
...
{
	use crate::metal_stats::{BatchedClusterMeta, BatchedNeuronMeta};
	use ram_core::neuron_memory::{EMPTY_U8, QSR, QUAD_BINARY, QUAD_WEIGHTED};
	...
	(0..num_genomes)
		.map(|g| {
			...
			// Build NeuronStats
			let neuron_stats: Vec<NeuronStats> = (0..(n_end - n_start))
				.map(|local_n| {
					let global_n = n_start + local_n;
					let n_bits = bpn[local_n];
					let total_cells = 1usize << n_bits;
					let fill_rate = result.neuron_filled[global_n].min(total_cells as u32) as f32
						/ total_cells.max(1) as f32;
					let error_rate = if n_sample > 0
					{
			...
			let cluster_stats: Vec<ClusterStats> = (0..num_clusters_per_genome)
				.map(|c| {
					let n_neurons = npc[c];
					...
					{
						(0..n_neurons)
							.map(|i| neuron_stats[neuron_offset + i].fill_rate)
							.sum::<f32>()
							/ n_neurons as f32
					}
					else
...
mod tests
{
	...
	fn test_adaptation_rate_schedule()
	{
		...
		assert!(
			rate_10 > 0.99,
			"rate at gen 10 should be ~1.0, got {}",
			rate_10
		);
		...
		assert!(
			rate_100 > 0.4 && rate_100 < 0.6,
			"rate at gen 100 should be ~0.5, got {}",
			rate_100
		);
		...
		assert!(
			rate_190 < 0.01,
			"rate at gen 190 should be ~0, got {}",
			rate_190
		);
	...
	fn test_neurogenesis_warmup_guard()
	{
		...
		let cluster_stats = vec![
			ClusterStats {
				error_rate: 0.9,
				mean_fill_rate: 0.9,
				neuron_uniqueness: vec![0.5, 0.5],
				neuron_accuracy: vec![0.5, 0.5],
			},
			ClusterStats {
				error_rate: 0.9,
				mean_fill_rate: 0.9,
