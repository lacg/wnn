...
) -> MulticlassMetrics
{
	...
	for c in 0..num_classes
	{
		...
		correct += tp;
		let row: u64 = (0..num_classes)
			.map(|p| confusion[c * num_classes + p])
			.sum();
		let col: u64 = (0..num_classes)
...
) -> Vec<MulticlassModeResult>
{
	...
	if let (Some(vs), Some(vt)) = (val_scores, val_targets)
	{
		...
		let (val_tau, _val_f1) =
			find_optimal_margin_tau(&val_margins, &val_attack, vt, num_classes, benign_class);
	...
	for (mode_name, kind) in [
		("argmax_platt", CalKind::Platt),
		("argmax_beta", CalKind::Beta),
	]
	{
...
) -> Vec<ClassCal>
{
	...
	(0..num_classes)
		.map(|c| {
			...
			let ovr: Vec<i64> = cal_targets
				.iter()
				.map(|&t| (t as usize == c) as i64)
				.collect();
			match kind
			{
				CalKind::Platt =>
...
mod tests
{
	...
	fn calibrated_argmax_matches_argmax_when_separated()
	{
		...
		assert!(
			names.contains(&"argmax_platt") && names.contains(&"argmax_beta"),
			"{names:?}"
		);
		for m in &modes
		{
			...
			{
				assert!(
					(m.metrics.macro_f1 - 1.0).abs() < 1e-9,
					"{}: macro_f1 {} != 1.0",
					m.mode,
					m.metrics.macro_f1
				);
	...
	fn calibrated_argmax_recovers_compressed_class()
	{
		...
		assert!(
			raw.metrics.recall[2] < 1e-9,
			"raw argmax recall[2] = {}",
			raw.metrics.recall[2]
		);
		// Per-class calibration recovers it fully on this synthetic set.
		assert!(
			(platt.metrics.macro_f1 - 1.0).abs() < 1e-6,
			"argmax_platt macro_f1 {} != 1.0",
			platt.metrics.macro_f1
		);
		assert!(
			(beta.metrics.macro_f1 - 1.0).abs() < 1e-6,
			"argmax_beta macro_f1 {} != 1.0",
			beta.metrics.macro_f1
		);
	...
	{
		let scores = vec![0.9, 0.1, 0.2, 0.2, 0.6, 0.5, 0.3, 0.3, 0.4];
