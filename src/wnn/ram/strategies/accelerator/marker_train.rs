...
pub mod metal_impl
{
	...
	impl GpuInfo
	{
		fn from_device(device: &Device) -> Self
		{
			...
			let cores: u32 = if lower.contains("m4 max")
			{
				40
			}
			...
			else if lower.contains("m4 ultra")
			{
				80
			}
	...
	impl MarkerTrainer
	{
		...
		) -> Result<f64, String>
		{
			...
			let num_host_chunks = if examples_per_dispatch >= num_examples_total
...
// re-exported for external callers; not used within this file
...
pub mod batched_path
{
	...
	use crate::adaptive::{
		build_groups, per_cluster_max_bits, reorganize_connections_for_gpu, ConfigGroup, GenomeExport,
		SparseGpuExport,
	};
	...
	) -> Result<Vec<GenomeExport>, String>
	{
		...
		{
			Err(ref msg)
				if msg.starts_with(BUDGET_ERR_PREFIX)
					&& num_genomes == 1
					&& num_clusters == 1
	...
	) -> Result<Vec<GenomeExport>, String>
	{
		...
		let chunk_n = (chunk_budget_bytes / bytes_per_neuron.max(1))
			.max(1)
			.min(n_total);
		...
		if *conn_prefix.last().unwrap() != genomes_connections_flat.len()
		{
			return Err(format!(
				...
				genomes_connections_flat.len(),
				conn_prefix.last().unwrap()
			));
		}
	...
	) -> Result<ChunkedVoteSums, String>
	{
		...
		{
			return Err(
				"eval-in-place requires WNN_ORDER_INDEPENDENT_TRAIN=1 (probe merges OI counters)"
					.to_string(),
			);
		}
		let n_total = genomes_bits_flat.len();
		...
		while start < n_total
		{
			...
			let (markers_buf, keys_buf, values_buf) = tb
				.gpu_table
				.metal_buffers()
	...
	) -> Result<TrainedBatch, String>
	{
		...
		let cluster_capacity: Vec<usize> = cluster_example_count
			.iter()
		...
		let mut neuron_meta: Vec<NeuronTrainMeta> =
			Vec::with_capacity(num_genomes * num_neurons_per_genome);
		for g in 0..num_genomes
		{
			let bpn_start = genome_bpn_offsets[g];
			...
			for c in 0..num_clusters
			{
				...
				for n_local in 0..n_in_cluster
				{
					...
					local_conn_offset += if has_heterogeneous_bpn
					{
						max_bits_in_batch as u32
					}
					else
					{
						bits
		...
		let negs_storage = if train_negatives.is_empty()
		{
			vec![0i64]
		}
		else
		{
			train_negatives.to_vec()
		};
		...
		let params = TrainParams {
			...
			num_negatives: if num_clusters == 1
			{
				0
			}
			else
			{
				num_negatives as u32
			},
	...
	) -> Result<Vec<GenomeExport>, String>
	{
		...
		for g in 0..num_genomes
		{
			...
			let connections_genome: Vec<i64> = if has_heterogeneous_bpn
			{
				...
				let mut unpadded: Vec<i64> = Vec::with_capacity(bpn_slice.iter().sum::<usize>());
				for n in 0..num_neurons_per_genome
				{
	...
	mod chunked_tests
	{
		...
		fn run_multigroup_parity(memory_mode: u8)
		{
			...
			for ex in 0..num_train
			{
				...
				assert_eq!(
					argmax(&sb[ex]),
					targets[ex],
					"batched export failed to separate ex {} (scores {:?})",
					ex,
					sb[ex]
				);
		...
		fn neuron_chunked_matches_unchunked()
		{
			...
			assert!(
				total > 0 && (total as usize) < num_train * n_neurons,
				"sampling did not thin writes: total={}",
				total
			);
			...
			assert_ne!(
				with_offset[0].sparse_exports[0].keys, without_offset[0].sparse_exports[0].keys,
		...
		fn oi_z_parity_with_collisions()
		{
			...
			for (i, run) in runs.iter().enumerate().skip(1)
			{
				...
				assert_eq!(
					a.counts, b.counts,
					"run {} counts differ from z=1 — unmerged duplicates",
					i
				);
				assert_eq!(a.keys, b.keys, "run {} keys differ from z=1", i);
				assert_eq!(
					a.values, b.values,
					"run {} cell values differ from z=1 — lost/split tallies",
					i
				);
			}
			...
			assert!(
				(total as usize) <= 256 * n_neurons,
				"collision regime not reached: {} distinct addresses",
				total
			);
			eprintln!(
		...
		fn budget_eligibility_matches_chunked_regime()
		{
			...
			assert!(!single_genome_exceeds_batch_budget(
				&vec![16; 250],
				100_000,
				0.25
			));
			...
			for ex in 0..num_examples
			{
				...
				for n in 0..n_neurons
				{
					...
					let cell = match sparse.keys[start..start + count].binary_search(&addr)
					{
						...
						{
							hits += 1;
							sparse.values[start + i]
						}
						Err(_) =>
						{
							misses += 1;
							1u8
						} // miss → QUAD_WEAK_FALSE
					};
		...
		fn eval_in_place_vote_parity()
		{
			...
			let packed_train =
				ram_core::packed_bits::PackedBits::from_bool_slice(&train_bools, total_input_bits);
			let targets: Vec<i64> = (0..num_train).map(|ex| (ex % 2) as i64).collect();
			...
			let packed_eval =
				ram_core::packed_bits::PackedBits::from_bool_slice(&eval_bools, total_input_bits);
			...
			let (packed_train_u64, train_words) =
				ram_core::neuron_memory::pack_packed_to_u64(&packed_train);
			let train_buf = device.new_buffer_with_data(
			...
			while start < n_neurons
			{
				...
				let exports =
					export_trained_batch(&tb, chunk_bits, &chunk_neurons).expect("chunk export failed");
			...
			assert!(
				...
				"eval set must exercise both hit and miss paths: hits={} misses={}",
				eval_hits,
				eval_misses
			);
			...
			assert_eq!(
				probe_eval, ref_eval,
				"eval vote sums differ from export-walk reference"
			);
			assert_eq!(
				probe_train, ref_train,
				"train vote sums differ from export-walk reference"
			);
			eprintln!(
			...
			assert!(
				scored.eval_votes.iter().all(|&v| v <= max_vote),
				"eval vote exceeds 4×neurons"
			);
			assert!(
				train_votes.iter().all(|&v| v <= max_vote),
				"train vote exceeds 4×neurons"
			);
		}
		...
		fn bench_prod_genome()
		{
			...
			let sparse_eval =
				ram_core::metal_sparse::MetalSparseEvaluator::new().expect("sparse evaluator");
			let scores = crate::adaptive::evaluate_group_sparse_gpu(
				&sparse_eval,
				&packed_eval_u64,
				&out[0].connections,
				&out[0].sparse_exports[0],
				&out[0].groups[0],
				num_eval,
				eval_words,
		...
		fn bench_chunked_z_sweep()
		{
			...
			for z in [8u32, 64, 256, 1024]
			{
				...
				let total: u32 = out[0]
					.sparse_exports
					.iter()
					.map(|s| s.counts.iter().sum::<u32>())
					.sum();
				eprintln!(
					"[bench] z={:4}  wall={:8.1}ms  writes={}",
					z,
...
pub use batched_path::{
	batched_train_offspring, single_genome_exceeds_batch_budget, train_single_genome_chunked_scored,
	ChunkedVoteSums,
};
