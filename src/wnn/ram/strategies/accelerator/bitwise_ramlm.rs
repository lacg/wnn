...
///
/// Pipeline: Train → Compute stats → Adapt → Re-train (if adapted) → Eval
///
/// Returns: (adapted_bits_per_neuron, adapted_neurons_per_cluster, adapted_connections,
///           ce, accuracy, weighted_bit_accuracy, pruned, grown, neurons_added, neurons_removed)
pub fn train_adapt_eval(
    cache: &BitwiseTokenCache,
    bits_per_neuron: &[usize],
    neurons_per_cluster: &[usize],
    connections: &[i64],
...
) -> (Vec<usize>, Vec<usize>, Vec<i64>, f64, f64, f64, usize, usize, usize, usize) {
    ...
    // Compute cluster_max_bits for storage allocation
    let cluster_max_bits: Vec<usize> = {
        let mut v = Vec::with_capacity(num_clusters);
        let mut off = 0;
        for &nc in neurons_per_cluster {
            let max_b = bits_per_neuron[off..off + nc].iter().copied().max().unwrap_or(0);
            v.push(max_b);
            off += nc;
        }
        v
    ...
    let threshold = match sparse_threshold_override {
        Some(t) => t,
        None => auto_sparse_threshold(&cluster_max_bits, neurons_per_cluster, 20 * 1024 * 1024 * 1024, expected_train),
    };

    let layout = compute_genome_layout(bits_per_neuron, neurons_per_cluster, threshold, expected_train);

    // Allocate storage and train
    let mut cluster_storage: Vec<ClusterStorage> = (0..num_clusters)
        .map(|c| ClusterStorage::new(
    ...
    // Phase 1: Train
    let scores_per_genome = num_eval * num_clusters;
    let mut out_probs = vec![0.0f32; scores_per_genome];
    train_and_forward_into(
        cache, connections, bits_per_neuron, neurons_per_cluster, &layout,
        train_subset, eval_subset, &mut cluster_storage, &mut out_probs,
        memory_mode, neuron_sample_rate, rng_seed,
    );

    // Phase 2: Adaptation
    let mut adapted_bits = bits_per_neuron.to_vec();
    let mut adapted_neurons = neurons_per_cluster.to_vec();
    ...
    if needs_adaptation {
        let mut rng = rand::rngs::StdRng::seed_from_u64(rng_seed.wrapping_add(999));

        for _pass in 0..adapt_config.passes_per_eval {
            // Compute neuron stats from trained storage
            if adapt_config.synaptogenesis_enabled {
                ...
                let (pruned, grown) = adaptation::synaptogenesis_pass(
                    ...
                    train_subset.num_examples, &mut rng,
                );
                total_pruned += pruned;
                total_grown += grown;
            }
            ...
            if adapt_config.neurogenesis_enabled {
                let neuron_stats_for_cluster = adaptation::compute_neuron_stats(
                    &adapted_bits, &adapted_neurons, &adapted_connections,
                    &cluster_storage, &train_subset.packed_input,
                    train_subset.words_per_example, &train_subset.target_bits,
                    train_subset.num_examples, num_clusters,
                ...
                let (added, removed) = adaptation::neurogenesis_pass(
                    ...
                    &cluster_stats, adapt_config, generation, cooldowns, &mut rng,
                );
                total_added += added;
    ...
    let architecture_changed = total_pruned > 0 || total_grown > 0 || total_added > 0 || total_removed > 0;

    // Phase 3: Re-train and eval with adapted architecture (if changed)
    if architecture_changed {
        ...
        let new_threshold = match sparse_threshold_override {
            Some(t) => t,
            None => auto_sparse_threshold(&new_cluster_max_bits, &adapted_neurons, 20 * 1024 * 1024 * 1024, expected_train),
        };

        let new_layout = compute_genome_layout(&adapted_bits, &adapted_neurons, new_threshold, expected_train);
        ...
        out_probs = vec![0.0f32; scores_per_genome];
        train_and_forward_into(
            cache, &adapted_connections, &adapted_bits, &adapted_neurons, &new_layout,
            train_subset, eval_subset, &mut new_storage, &mut out_probs,
            memory_mode, neuron_sample_rate, rng_seed.wrapping_add(1),
        );
    }

    // Phase 4: Compute CE
    let (ce, accuracy) = compute_ce_cpu(
        &out_probs,
        &cache.token_bits,
        &eval_subset.targets,
        num_eval,
    ...
    // Weighted bit accuracy (simplified — use 0.0 for now, can compute later)
    let weighted_bit_acc = 0.0;
