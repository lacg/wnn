...
impl RotorGeometry
{
	...
	{
		let up = [0.0, 0.0, 1.0];
		let mk = |position: [f32; 3], spin: f32| Rotor {
			position,
			axis: up,
			spin,
			k_thrust,
	...
	pub fn canted_hex(arm: f32, k_thrust: f32, k_drag: f32, cant_deg: f32) -> Self
	{
		...
		let rotors = (0..6)
			.map(|i| {
				...
				let axis = [arm_dir[1] * s, -arm_dir[0] * s, c];
				Rotor {
					position: [arm * arm_dir[0], arm * arm_dir[1], 0.0],
					axis,
					spin,
					k_thrust,
					k_drag,
...
mod tests
{
	...
	fn quad_plus_asym_matches_attitude_sim_d3()
	{
		...
		assert_close(
			&geo.body_torque_asym(&pwm, Some(&asym)),
			&oracle,
			1e-6,
			"asym torque",
		);
	...
	fn octo_pinv_is_minimum_norm()
	{
		...
		let wrench = [0.02, -0.015, 0.004, 0.0, 0.0, 6.0];
		let pwm = geo.allocate(wrench, 1e-6);
		let thrusts: Vec<f32> = pwm
			.iter()
			.zip(&geo.rotors)
			.map(|(p, r)| r.k_thrust * p * p)
			.collect();
		...
		let alt_pwm: Vec<f32> = alt
			.iter()
			.zip(&geo.rotors)
			.map(|(t, r)| (t.max(0.0) / r.k_thrust).sqrt())
			.collect();
		...
		if (tau_alt[0] - tau_ref[0]).abs() < 1e-3 && (tau_alt[1] - tau_ref[1]).abs() < 1e-3
		{
			let alt_norm: f32 = alt.iter().map(|t| t * t).sum();
			assert!(
				alt_norm >= norm - 1e-6,
				"pinv allocation must be minimum-norm"
			);
		}
	}
	...
	fn allocation_pinv_matches_allocate()
	{
		...
		for geo in [
			RotorGeometry::octo_x(ARM, KT, KD),
			RotorGeometry::quad_plus(ARM, KT, KD),
		]
		{
			...
			]
			{
				...
				for (i, r) in geo.rotors.iter().enumerate()
				{
					...
					assert!(
						(pwm - via_solve[i]).abs() < 1e-5,
						"rotor {i}: pinv pwm {pwm} vs allocate {}",
						via_solve[i]
					);
				}
			}
