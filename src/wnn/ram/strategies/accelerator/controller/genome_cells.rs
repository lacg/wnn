...
impl NeurCol
{
	...
	pub fn reserve(&mut self, n: usize)
	{
		match self
		{
			NeurCol::W16(v) => v.reserve(n),
			NeurCol::W32(v) => v.reserve(n),
		}
	}
...
impl AddrCol
{
	...
	pub fn reserve(&mut self, n: usize)
	{
		match self
		{
			AddrCol::W32(v) => v.reserve(n),
			AddrCol::W64(v) => v.reserve(n),
		}
	}
	...
	for i in 0..n.len()
	{
		...
		if n.get(i) >= n_neurons
		{
			return Err(PyValueError::new_err(format!(
				"{what} cell neuron out of range"
			)));
		}
		if bits < 64 && (a.get(i) as u128) >= (1u128 << bits)
		{
			return Err(PyValueError::new_err(format!(
				"{what} cell address exceeds 2^bits"
			)));
		}
		if v[i] > 3
		{
			return Err(PyValueError::new_err(format!(
				"{what} cell value not QSR 0..3"
			)));
		}
	}
	Ok(())
...
impl GenomeCells
{
	...
	pub fn py_new(
		state_universe: Vec<(u32, u64)>,
		output_universe: Vec<(u32, u64)>,
		state_values: Vec<u8>,
	...
	#[staticmethod]
	pub fn from_triples(
		state_triples: Vec<Vec<i128>>,
		output_triples: Vec<Vec<i128>>,
	) -> PyResult<Self>
	{
		fn cols(rows: Vec<Vec<i128>>, what: &str) -> PyResult<(Vec<u32>, Vec<u64>, Vec<u8>)>
		{
			...
			for row in rows
			{
				...
				{
					return Err(PyValueError::new_err(format!(
						"{what} triple has {} fields, expected 3",
						row.len()
					)));
				}
				...
				if !(0..=u64::MAX as i128).contains(&row[1])
				{
					return Err(PyOverflowError::new_err(format!(
						"{what} address {} out of u64 range (cells are u64-keyed)",
						row[1]
					)));
				}
	...
	#[getter]
	pub fn handle(slf: PyRef<'_, Self>) -> Py<Self>
	{
		slf.into()
	}
	...
	) -> PyResult<Self>
	{
		...
		Ok(Self {
			sn: NeurCol::from_wide(sn),
			sa: AddrCol::from_wide(sa),
			sv,
			on_: NeurCol::from_wide(on_),
			oa: AddrCol::from_wide(oa),
			ov,
		})
	}

	pub fn clone_cells(&self) -> Self
	{
		self.clone()
	}
	...
	{
		let st = (0..self.sn.len())
			.map(|i| (self.sn.get(i), self.sa.get(i), self.sv[i]))
			.collect();
		let ot = (0..self.on_.len())
			.map(|i| (self.on_.get(i), self.oa.get(i), self.ov[i]))
			.collect();
		(st, ot)
	}
	...
	{
		let (sn, sa, sv) =
			unpack_layer(state_b64, state_n, state_i128, "state").map_err(PyValueError::new_err)?;
		let (on_, oa, ov) =
			unpack_layer(output_b64, output_n, output_i128, "output").map_err(PyValueError::new_err)?;
		Ok(Self {
			sn,
			sa,
			sv,
	...
	pub fn filter_inherited(
		...
		state_neurons: u32,
		state_bits: u32,
		changed_state: Vec<u32>,
		output_neurons: u32,
		output_bits: u32,
		changed_output: Vec<u32>,
	) -> Self
	{
		use std::collections::HashSet;
		...
		for i in 0..self.sn.len()
		{
			...
			{
				out.sn.push(self.sn.get(i));
				out.sa.push(self.sa.get(i));
				out.sv.push(self.sv[i]);
			}
		}
		for i in 0..self.on_.len()
		{
			...
			{
				out.on_.push(self.on_.get(i));
				out.oa.push(self.oa.get(i));
				out.ov.push(self.ov[i]);
			}
		}
		out
	}
...
mod narrow_tests
{
	...
	fn old_digest(sn: &[u32], sa: &[u64], sv: &[u8], on_: &[u32], oa: &[u64], ov: &[u8])
		-> (u64, u64)
	{
		let mut lanes = [0xcbf2_9ce4_8422_2325u64, 0x51_7cc1_b727_220a_95u64];
	...
	fn narrow_storage_engages_for_small_values()
	{
		...
		assert!(
			matches!(g.sn, NeurCol::W16(_)),
			"small neuron ids must store u16"
		);
		assert!(
			matches!(g.sa, AddrCol::W32(_)),
			"b<=32 addresses must store u32"
		);
	...
	fn digest_is_width_invariant()
	{
		...
		.unwrap();
		assert!(
			matches!(gw.sa, AddrCol::W64(_)),
			"precondition: wide storage"
		);
	...
	fn push_promotion_preserves_prefix()
	{
		...
		assert_eq!(
			...
			(7, u32::MAX as u64, u32::MAX as u64 + 1)
		);
