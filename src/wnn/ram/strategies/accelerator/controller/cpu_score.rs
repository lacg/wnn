...
) -> [f64; 15]
{
	...
	if let Some(rows) = geometry
	{
		...
		sim
			.set_rotor_asym_core(rotor_asym.map(|a| a.to_vec()))
			.expect("validated rotor_asym");
	}
	...
	for ep in 0..num_eps
	{
		...
		{
			sim
				.set_translation_core(s1.mass[ep])
		...
		let mut ep_trace: Vec<f64> = if trace.is_some()
		{
		...
		for _t in 0..steps
		{
			...
			{
				c.set_vertical_obs(
					s1.collective_pwm(ep, gravity, k_thrust),
			...
			if let Some(ab) = alloc
			{
				...
				{
					let r = ((pwm[m] - neutral) * residual_scale).clamp(-residual_clamp, residual_clamp);
		...
		sum_effort += if ep_steps > 0
		{
			ep_effort / ep_steps as f64
		}
		else
		{
			0.0
		};
		sum_poserr += if ep_steps > 0
		{
			ep_sum_poserr / ep_steps as f64
		}
		else
		{
			0.0
		};
		sum_alterr += if ep_steps > 0
		{
			ep_sum_alterr / ep_steps as f64
		}
		else
		{
			0.0
