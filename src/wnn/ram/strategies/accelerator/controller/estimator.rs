...
impl MahonyEstimatorRs
{
	...
	pub fn new(dt: f64, kp: f64, ki: f64) -> PyResult<Self>
	{
		...
		Ok(Self {
			inner: MahonyFilter::new(dt, kp, ki),
		})
	}
...
mod tests
{
	...
	fn tracks_clean_sim_attitude()
	{
		...
		for _ in 0..1000
		{
			...
			let dot = (qe[0] as f64) * (qt[0] as f64)
				+ (qe[1] as f64) * (qt[1] as f64)
				+ (qe[2] as f64) * (qt[2] as f64)
				+ (qe[3] as f64) * (qt[3] as f64);
