...
impl AttitudePidFirmwareRs
{
	...
	) -> Self
	{
		...
		let filt = |c: f64| {
			if c > 0.0
			{
				Some(Lpf2p::new(attitude_hz as f64, c))
			}
			else
			{
				None
	...
	fn update_cascade(&mut self, q: [f64; 4], gyro: [f64; 3], target_rpy: [f64; 3]) -> [f64; 3]
	{
...
mod tests
{
	...
	fn lpf2p_matches_firmware_coefficients()
	{
		...
		assert!(
			(f.b0 - 0.027859766117136024).abs() < 1e-15,
			"b0 {:.17}",
			f.b0
		);
		assert!(
			(f.b1 - 0.05571953223427205).abs() < 1e-15,
			"b1 {:.17}",
			f.b1
		);
		assert!(
			(f.b2 - 0.027859766117136024).abs() < 1e-15,
			"b2 {:.17}",
			f.b2
		);
		assert!(
			(f.a1 - (-1.475480443592646)).abs() < 1e-15,
			"a1 {:.17}",
			f.a1
		);
