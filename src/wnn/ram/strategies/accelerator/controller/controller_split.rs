...
) -> Vec<Conflict>
{
	...
	{
		buckets
			.entry(&keys_flat[i * words_per_key..(i + 1) * words_per_key])
			.or_default()
			.push(i);
	}
	let mut conflicts: Vec<Conflict> = buckets
...
{
	let words = state_in_len.div_ceil(32);
	let n_rec = if state_in_len == 0
	{
		0
	}
	else
	{
		bits.len() / state_in_len
	};
...
mod packed_key_tests
{
	...
	fn assert_same(a: &(Vec<Conflict>, usize), b: &(Vec<Conflict>, usize), what: &str)
	{
		...
		for (i, (x, y)) in a.0.iter().zip(b.0.iter()).enumerate()
		{
			assert_eq!(
				...
				"{what}: conflict {i} instances differ"
			);
			assert_eq!(x.out_in, y.out_in, "{what}: conflict {i} out_in differs");
			assert!(
				(x.spread - y.spread).abs() < 1e-6,
				"{what}: conflict {i} spread differs"
			);
		}
	}
		...
		]
		{
			...
			for &tau in &[0.05f32, 0.2]
			{
				for &tmin in &[1usize, 32]
				{
					...
					let want =
						scan_conflicts_coarse_reference(&out_ins, &pwms, tau, bpf, nf, frame_bits, tmin);
					assert_same(
						&got,
						&want,
						&format!("nf={nf} bpf={bpf} ns={ns} tau={tau} tmin={tmin}"),
					);
				}
			}
		}
	...
	fn fixture_produces_real_conflicts()
	{
		...
		assert!(
			!c.is_empty(),
			"fixture produced no conflicts — equivalence test is vacuous"
		);
	...
	fn pack_sif_round_trips_every_bit()
	{
		...
		]
		{
			...
			assert_eq!(
				packed.len(),
				n_rec * sil.div_ceil(32),
				"sil={sil}: wrong word count"
			);
			for r in 0..n_rec
			{
				for b in 0..sil
