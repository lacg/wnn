...
) -> usize
{
	...
	for _ in 0..steps
	{
		...
		{
			c.set_vertical_obs(
				s.cfg.collective_pwm(ep, s.gravity, s.k_thrust),
...
) -> (Vec<f64>, Vec<f64>)
{
	...
	(
		s_act
			.iter()
			.map(|&a| binary_entropy(a as f64 / n))
			.collect(),
		o_act
			.iter()
			.map(|&a| binary_entropy(a as f64 / n))
...
mod tests
{
	...
	fn stage1_vertical_channel_changes_the_recorded_universe()
	{
		...
		let record = |s1: Option<&RecorderStage1<'_>>| {
			...
			record_address_universe(
				&mut c, &mut sim, &mut d, &init_q, &init_om, [0.0; 3], 400, s1,
			)
		};
	...
	fn binary_entropy_endpoints_and_peak()
	{
		...
		assert!(
			(binary_entropy(0.5) - 1.0).abs() < 1e-12,
			"p=0.5 must be exactly 1 bit"
		);
		// symmetric
