	...
	uint3 tid [[thread_position_in_grid]])
{
	...
	if (c >= P.num_conflicts || ai >= P.num_bits || bi >= P.num_bits) return;
	uint oidx = (c * P.num_bits + ai) * P.num_bits + bi;
	if (ai == bi) { out_corr[oidx] = 0.0f; return; }

	uint n = conf_inst_count[c];
	if (n == 0u) { out_corr[oidx] = 0.0f; return; }
	...
	for (uint k = 0u; k < n; k++) { sx += (ca[k] - cb[k]); sy += y[k]; }
	float nf = float(n);
	float mx = sx / nf;
	float my = sy / nf;
	...
	float corr;
	if (sxx <= 1e-9f || syy <= 1e-9f) corr = 0.0f;
	else corr = sxy / (sqrt(sxx) * sqrt(syy));
	out_corr[oidx] = corr;
}
