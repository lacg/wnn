...
{
	uint rng = seed + step * 2654435761u + axis * 1000003u + channel * 97780813u + k * 668265263u;
	if (rng == 0u)
		rng = 1u;
	rng ^= rng << 13;
	rng ^= rng >> 17;
...
{
	if (gamma == 1.0f)
		return t; // exact no-op, matches Rust
											 ...
											 constant Params &P, float dt)
{
	...
	if (wrap)
		error = pid_wrap(error);
	st[i_integ] += error * dt;
	if (i_limit != 0.0f)
		st[i_integ] = clamp(st[i_integ], -i_limit, i_limit);
	...
	if (P.pidfw_filter_on != 0u && lpf_d1 != nullptr)
	{
		...
		if (!isfinite(d0))
			d0 = deriv;
		float o = d0 * P.pidfw_lpf[0] + (*lpf_d1) * P.pidfw_lpf[1] + (*lpf_d2) * P.pidfw_lpf[2];
		*lpf_d2 = *lpf_d1;
		*lpf_d1 = d0;
		deriv = o;
	}
		...
		thread float *dhat, thread float *dhat_last_gyro, thread bool &dhat_have_last)
{
	...
	if (P.num_features > NUM_FEATURES)
	{
		...
		if (P.obs_tilt_i != 0u)
		{
			...
			sensors[fi++] = integ[ii] * P.integral_scale;
			ii++;
		}
		if (P.obs_peraxis_p != 0u)
		{
			sensors[fi++] = roll_err;
			sensors[fi++] = pitch_err;
			if (P.obs_peraxis_yaw != 0u)
			{
				sensors[fi++] = yaw_err;
			} // drop yaw when ref unobservable
		}
		...
		if (P.obs_collective_cmd != 0u)
			sensors[fi++] = P.vert_collective_cmd;
		if (P.obs_alt_err != 0u)
			sensors[fi++] = P.vert_alt_err;
		if (P.obs_vz != 0u)
			sensors[fi++] = P.vert_vz;
		// STAGE 2 horizontal: both axes together, error before velocity — the
		// canonical tail order of controller.rs compute_features.
		if (P.obs_pos_err_xy != 0u)
		{
			sensors[fi++] = P.horiz_err_x;
			sensors[fi++] = P.horiz_err_y;
		}
		if (P.obs_vel_xy != 0u)
		{
			sensors[fi++] = P.horiz_vx;
			sensors[fi++] = P.horiz_vy;
		}
	}
}
		...
		thread uchar *new_state) // OUT [n_state]
{
	...
	bool shift = (filled == 0u) || (P.frame_stride <= 1u) || (pushes % P.frame_stride == 0u);
	pushes += 1u;
	if (shift)
	{
		if (filled == P.window)
		...
		uint2 tid [[thread_position_in_grid]])
{
	...
	if (g >= P.num_genomes || e >= P.num_episodes)
		return;
	...
	for (uint m = 0u; m < MAX_ROTORS; m++)
		pwm_acc[m] = (P.decouple_outputs != 0u && m >= 1u) ? 0.0f : 0.5f;
	...
	for (uint m = 0u; m < MAX_ROTORS; m++)
		last_pwm[m] = (P.decouple_outputs != 0u && m >= 1u) ? 0.0f : 0.5f;
	// DOB Fix A (12/08/2026): the pwm the PLANT received last step — dhat trim
	...
	for (uint m = 0u; m < MAX_ROTORS; m++)
		pwm_applied[m] = (P.decouple_outputs != 0u && m >= 1u) ? 0.0f : 0.5f;
	...
	uint ep_seed = dist_on
										 ? dist_hash(P.dist_seed, P.dist_ep_offset + e, 0u, DIST_CH_EP_SEED, 0u)
										 : 0u;
	...
	for (uint _i = 0u; _i < (uint)PIDFW_STATE; _i++)
		pid_i[_i] = 0.0f;
	...
	for (uint t = 0u; t < P.steps; t++)
	{
		...
		bool bad = false;
		if (!isfinite(omega.x) || !isfinite(omega.y) || !isfinite(omega.z))
			bad = true;
		if (fabs(omega.x) > 50.0f || fabs(omega.y) > 50.0f || fabs(omega.z) > 50.0f)
			bad = true;
		...
		sensors[1] = omega.y;
		sensors[2] = omega.z;
		sensors[3] = -grav_body.x;
		sensors[4] = -grav_body.y;
		sensors[5] = -grav_body.z;
		sensors[6] = P.target0;
		sensors[7] = P.target1;
		sensors[8] = P.target2;
		...
		if (dist_on)
		{
			...
			if (dist_delay > 0u || P.dist_dropout_prob > 0.0f)
			{
				...
				if (P.dist_dropout_prob > 0.0f)
				{
					...
					if (!frozen)
						starting = dist_uniform(ep_seed, t, 0u, DIST_CH_DROPOUT, 0u) < P.dist_dropout_prob;
				}
				...
				if ((frozen || starting) && imu_cache_valid)
					for (uint i = 0u; i < 6u; i++)
						sensors[i] = imu_cache[i];
				else
					for (uint i = 0u; i < 6u; i++)
						sensors[i] = post_lat[i];
		...
		else
		{
			...
			for (uint m = 0u; m < P.num_motors; m++)
			{
				...
				for (uint l = 0u; l < P.levels; l++)
				{
					...
					if (ctrl_fire_bit(qv, P.memory_mode))
					{
						if (prev_zero && seen_one)
							mono_step += 1.0f;
						seen_one = true;
						prev_zero = false;
					}
		...
		else
		{
			...
			if (dist_on)
			{
				th0 = P.k_thrust * P.dist_motor_asym0 * p0 * p0;
				th1 = P.k_thrust * P.dist_motor_asym1 * p1 * p1;
				th2 = P.k_thrust * P.dist_motor_asym2 * p2 * p2;
				th3 = P.k_thrust * P.dist_motor_asym3 * p3 * p3;
			}
			else
			{
				th0 = P.k_thrust * p0 * p0;
				th1 = P.k_thrust * p1 * p1;
				th2 = P.k_thrust * p2 * p2;
				th3 = P.k_thrust * p3 * p3;
			}
		...
		if (P.translation_on != 0u)
		{
			...
			if (dist_on)
			{
				a0 = P.dist_motor_asym0;
				a1 = P.dist_motor_asym1;
				a2 = P.dist_motor_asym2;
				a3 = P.dist_motor_asym3;
			}
		...
		float dt = P.dt;
		float3 k1o, k2o, k3o, k4o;
		float4 k1q, k2q, k3q, k4q;
		...
		if (dist_on)
		{
			...
			for (uint a3 = 0u; a3 < 3u; a3++)
			{
				if (P.dist_gust_sigma > 0.0f)
				{
					...
					gust[a3] += -gust[a3] / P.dist_gust_tau_c * dt + P.dist_gust_sigma * sqrt_dt * xi;
				}
		...
		{
			init_err = err;
			band_rel = 0.05f * init_err;
		}
		iae += err * P.dt;
		ise += err * err * P.dt;
		itae += t_s * err * P.dt;
		if (!rise_done && err <= 0.10f * init_err)
		{
			rise_s = t_s;
			rise_done = true;
		}
		if (err >= band_abs)
			last_exc_abs = t_s + P.dt;
		if (err >= band_rel)
			last_exc_rel = t_s + P.dt;
...
{
	if (target_levels == 0u || target_levels >= levels)
		return uint2(level_idx, levels);
...
inline bool ctrl_output_target_bit(float d, uint level_idx, uint levels, uint output_decode)
{
	if (output_decode == WNN_DECODE_ANTAGONIST)
	{
		...
		float net = d - 0.5f;
		if (level_idx < half_l)
			return net > 0.0f && (uint)(net * (float)levels) > level_idx;
		...
		uint gid [[thread_position_in_grid]])
{
	...
	for (uint ej = 0u; ej < E; ej++)
	{
		...
		uint filled = 0u;
		uint pushes = 0u; // frame-stride tick source (twin of controller.rs frame_pushes)
		for (uint k = 0u; k < MAX_INTEGRALS; k++)
			integ[k] = 0.0f;
		...
		for (uint t = 0u; t < T; t++)
		{
			...
			uint s4 = (sbase + t) * 4u;
			float sensors[MAX_FEATURES];
			sensors[0] = gyros[s3 + 0];
			sensors[1] = gyros[s3 + 1];
			sensors[2] = gyros[s3 + 2];
			sensors[3] = accels[s3 + 0];
			sensors[4] = accels[s3 + 1];
			sensors[5] = accels[s3 + 2];
			...
			for (uint n = 0u; n < P.n_state; n++)
				if (ctrl_fire_bit((uint)new_state[n], P.memory_mode))
				{
					state_active = true;
					break;
				}
		...
		uint2 tid [[thread_position_in_grid]])
{
	...
	if (g >= P.num_genomes || ej >= ep_count[g])
		return;
	uint ep = ep_base[g] + ej;
	...
	for (uint t = 0u; t < T; t++)
	{
		...
		{
			uint feat = within / P.bpf, b = within % P.bpf;
			if (sensors[feat] >= thresholds[feat * P.bpf + b])
				set_packed_bit(rec_out_ins, ob, within);
		}
		for (uint n = 0u; n < P.n_state; n++)
			if (ctrl_fire_bit((uint)new_state[n], P.memory_mode))
				set_packed_bit(rec_out_ins, ob, P.frame_bits + n);
		...
		for (uint n = 0u; n < P.n_state; n++)
			if (ctrl_fire_bit((uint)prev_state[n], P.memory_mode))
				set_packed_bit(rec_state_ins, sb, P.sensor_total + n);

		rec_pwm[rec * 4 + 0] = pid_pwms[s4 + 0];
		rec_pwm[rec * 4 + 1] = pid_pwms[s4 + 1];
		rec_pwm[rec * 4 + 2] = pid_pwms[s4 + 2];
		rec_pwm[rec * 4 + 3] = pid_pwms[s4 + 3];
		...
		uint key_words, uint cap, thread const ulong *key)
{
	...
	for (uint probe = 0u; probe < cap; probe++)
	{
		...
		if (m == MARKER_FINAL)
		{
			if (keys_equal(keys + (ulong)slot * key_words, key, key_words))
				return slot;
		}
		else if (m == MARKER_EMPTY)
		{
			...
			for (uint retry = 0u; retry < 4u; retry++)
			{
				...
				if (expected != MARKER_EMPTY)
					break;
			}
			if (won)
			{
				for (uint w = 0u; w < key_words; w++)
					keys[(ulong)slot * key_words + w] = key[w];
				atomic_store_explicit(&markers[slot], MARKER_FINAL, memory_order_relaxed);
				return slot;
			}
			if (expected == MARKER_FINAL && keys_equal(keys + (ulong)slot * key_words, key, key_words))
				return slot;
			m = expected;
		}
		if (m != MARKER_EMPTY && m != MARKER_FINAL)
		{
			...
			for (uint w = 0u; w < 64u; w++)
			{
				...
				if (resolved == MARKER_FINAL || resolved == MARKER_EMPTY)
					break;
			}
			if (resolved == MARKER_FINAL && keys_equal(keys + (ulong)slot * key_words, key, key_words))
				return slot;
		}
		idx = (idx + 1u) & mask;
	}
		...
		uint i [[thread_position_in_grid]])
{
	...
	for (uint w = 0u; w < MAX_KEY_WORDS; w++)
		key[w] = 0ul;
	...
	for (uint f = 0u; f < P.num_features; f++)
	{
		...
		for (uint j = 0u; j < P.k; j++)
		{
			...
			if (get_packed_bit(rec_out_ins, ob, base + idx) != 0u)
				key[kb >> 6u] |= (1ul << (kb & 63u));
			kb++;
		}
	}
	...
	{
		if (get_packed_bit(rec_out_ins, ob, s) != 0u)
			key[kb >> 6u] |= (1ul << (kb & 63u));
		kb++;
	}
		...
		uint2 tid [[thread_position_in_grid]])
{
	...
	if (c >= P.num_conflicts || bi >= P.num_bits)
		return;
	uint oidx = c * P.num_bits + bi;
	out_correct[oidx] = 0xFFFFFFFFu;
	out_lag[oidx] = 0u;
	out_high[oidx] = 0u;
	...
	for (uint lag = 1u; lag <= max_lag; lag++)
	{
		...
		{
			if (step_of[conf_inst[base + j]] < lag)
				ok = false;
		}
		if (!ok)
			continue;
	...
	if (found)
	{
		out_correct[oidx] = best_correct;
		out_lag[oidx] = best_lag;
		out_high[oidx] = best_high;
	}
}
		...
		uint2 tid [[thread_position_in_grid]])
{
	...
	for (uint j = 0u; j < n; j++)
	{
		...
		for (uint lag = 0u; lag <= max_lag; lag++)
		{
			...
			{
				uint rec = ep_start[ep_of[i]] + (step_of[i] - lag);
				if (get_packed_bit(state_ins, rec * P.state_words, b) != 0u)
					cnt += 1u;
			}
		}
		...
		uint r [[thread_position_in_grid]])
{
	...
	ulong rmask[8];
	ulong clear = 0ul;
	for (uint j = 0u; j < P.num_rel; j++)
	{
		rmask[j] = 1ul << (P.sbpn - 1u - rel_pos[j]);
		clear |= rmask[j];
	}
	...
	for (uint combo = 0u; combo < ncombo; combo++)
	{
		...
		if (slot != 0xFFFFFFFFu)
			atomic_store_explicit(&values[slot], (uint)combo_vals[combo], memory_order_relaxed);
	}
}
		...
		uint a [[thread_position_in_grid]])
{
	...
	bool on;
	if (dn_b == 1u && self_b == 1u && upper_b == 0u)
		on = false; // decrement (top unwinds)
	else if (self_b == 1u)
		on = true; // hold
	else
		on = (up_b == 1u && lower_b == 1u); // increment
		...
		uint slot_off, uint slot_cap, ulong addr)
{
	...
	for (uint probe = 0u; probe < slot_cap; probe++)
	{
		...
		uint m = markers[slot];
		if (m == MARKER_EMPTY)
			return WNN_CELL_EMPTY; // open-addressing miss
		if (m == MARKER_FINAL && keys[slot] == addr)
			return values[slot] & 0xFFu;
		idx = (idx + 1u) & mask;
	}
	return WNN_CELL_EMPTY;
}
		...
		uint i [[thread_position_in_grid]])
{
	...
	if (slot != 0xFFFFFFFFu)
		atomic_store_explicit(&values[slot], (uint)cell_vals[i], memory_order_relaxed);
}
		...
		uint gid [[thread_position_in_grid]])
{
	...
	{
		if (cm_genome[i] != g || cm_neuron[i] != n)
			continue;
		uint slot = find_or_claim_slot(st_markers, st_keys, off, cap, cm_addr[i]);
		if (slot == 0xFFFFFFFFu)
			continue; // region full — cap is sized to prevent this
		bool target_true = cm_target[i] != 0u;
		if (quad)
			slot_nudge(st_values, slot, target_true);
		else
			slot_set_direct(st_values, slot, target_true);
		writes += 1u;
	}
	// Per-genome diagnostic; each thread owns a distinct neuron so this accumulates.
	if (writes > 0u)
		atomic_fetch_add_explicit((device atomic_uint *)&out_writes[g], writes, memory_order_relaxed);
}
...
inline ulong ctrl_projected_address(device const int *conn, device const uchar *input_bits, uint n_bits)
{
	...
	{
		if (input_bits[(uint)conn[k]] != 0u)
			proj |= (1ul << (ulong)(n_bits - 1u - k));
	}
	return proj;
}
...
{
	if (cnt == k_top && (rank > r[cnt - 1u] || (rank == r[cnt - 1u] && addr >= a[cnt - 1u])))
		return;
		...
		uint n [[thread_position_in_grid]])
{
	...
	for (uint h = 0u; h <= n_bits && h <= CTRL_MAX_RADIUS; ++h)
	{
		...
		while (more)
		{
			ulong addr = proj;
			for (uint j = 0u; j < h; ++j)
				addr ^= (1ul << (ulong)(n_bits - 1u - combo[j]));
			...
			examined += 1u;
			if (examined >= CTRL_MAX_REACH_SCAN)
			{
				h = n_bits + 1u;
				break;
			}
			...
			while (i > 0u)
			{
				...
				{
					combo[i] += 1u;
					for (uint j = i + 1u; j < h; ++j)
						combo[j] = combo[j - 1u] + 1u;
					more = true;
					break;
				}
			}
		...
		uint t [[thread_position_in_grid]])
{
	...
	uint beam_n = 1u;
	for (uint w = 0u; w < CTRL_BITS_WORDS; ++w)
		cur[w] = 0u; // all unset
	for (uint j = 0u; j < min(n_imm, tib); ++j)
		ctrl_bits_set(cur, j, ib[j] != 0u);
	cost[0] = 0u;
	...
	for (uint n = 0u; n < num_neurons; ++n)
	{
		...
		for (uint pi = 0u; pi < beam_n; ++pi)
		{
			...
			for (uint ai = 0u; ai < kc; ++ai)
			{
				...
				if (n_cnt == beam_width && cst >= n_cost[n_cnt - 1u])
					continue;
				uint pos = n_cnt < beam_width ? n_cnt : beam_width - 1u;
				while (pos > 0u && n_cost[pos - 1u] > cst)
				{
					n_cost[pos] = n_cost[pos - 1u];
					n_par[pos] = n_par[pos - 1u];
					n_addr[pos] = n_addr[pos - 1u];
					pos -= 1u;
				}
				n_cost[pos] = cst;
				n_par[pos] = pi;
