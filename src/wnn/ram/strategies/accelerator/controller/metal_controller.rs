...
pub struct RotorGpu
{
	px: f32,
	py: f32,
	pz: f32, // position, body frame (m)
	ax: f32,
	ay: f32,
	az: f32,       // unit thrust axis, body frame
...
pub fn build_rotor_table(
	rows: &[[f32; 9]],
	rotor_asym: Option<&[f32]>,
) -> Result<Vec<RotorGpu>, String>
{
	if rows.is_empty() || rows.len() > MAX_ROTORS_GPU
	{
		return Err(format!(
			"geometry needs 1..={MAX_ROTORS_GPU} rotors, got {}",
			rows.len()
		));
	}
...
impl ControllerRolloutEvaluator
{
	...
	fn geometry_pipeline(&self, n: u32) -> Result<ComputePipelineState, String>
	{
		...
		fcv.set_constant_value_at_index(
			&has_geometry as *const bool as *const _,
			MTLDataType::Bool,
			0,
		);
		fcv.set_constant_value_at_index(&n as *const u32 as *const _, MTLDataType::UInt, 1);
		let func = self
			.library
			.get_function("controller_rollout", Some(fcv))
	...
	) -> Result<Vec<Vec<f64>>, String>
	{
		...
		{
			return Err(
				"score_controllers_metal: the controller has stage-1 vertical \
		...
		if let Some(ab) = alloc_baseline
		{
			...
			{
				return Err(
					"score_controllers_metal: alloc_baseline requires residual \
				            (scale/clamp) — pass residual cfg."
						.to_string(),
				);
			}
			if ab.num_rotors() != num_motors
			{
				return Err(format!(
					"score_controllers_metal: alloc baseline has {} rotors but controllers \
					 emit num_motors={} PWMs — they must match.",
					ab.num_rotors(),
					num_motors
				));
			}
			...
			if rotors.len() != num_motors
			{
				return Err(format!(
					"score_controllers_metal: geometry has {} rotors but controllers emit \
					 num_motors={} PWMs — they must match.",
					rotors.len(),
					num_motors
				));
			}
		...
		let alloc_blob = alloc_baseline
			.map(|ab| ab.to_gpu_blob())
			.unwrap_or_default();
		...
		while chunk_start < num_episodes
		{
			...
			let chunk_params = RolloutParams {
				num_genomes: g as u32,
				num_episodes: chunk_ep_count as u32,
				steps: steps as u32,
				num_motors: num_motors as u32,
				levels: levels as u32,
				n_state: n_state as u32,
				sbpn: sbpn as u32,
				obpn: obpn as u32,
				bpf: bpf as u32,
				window: window as u32,
				frame_bits: frame_bits as u32,
				sensor_total: sensor_total as u32,
				...
				target1: target[1],
				target2: target[2],
				delta_control: if delta_control { 1 } else { 0 },
				delta_max,
				delta_leak,
				delta_gamma,
				num_features: num_features as u32,
				obs_tilt_p: obs_tilt_p as u32,
				obs_tilt_i: obs_tilt_i as u32,
				obs_peraxis_p: obs_peraxis_p as u32,
				obs_peraxis_i: obs_peraxis_i as u32,
				obs_peraxis_yaw: obs_peraxis_yaw as u32,
				obs_yaw_err: obs_yaw_err as u32,
				obs_yaw_err_i: obs_yaw_err_i as u32,
				...
				pidfw_on: if residual.and_then(|r| r.cascade).is_some()
				{
					1
				}
				else
				...
				pidfw_att: residual
					.and_then(|r| r.cascade)
					.map_or([0.0; 12], |c| c.att),
				pidfw_rate: residual
					.and_then(|r| r.cascade)
					.map_or([0.0; 12], |c| c.rate),
				pidfw_out_limit_n: residual
					.and_then(|r| r.cascade)
					.map_or(0.0, |c| c.out_limit_n),
				pidfw_hover_n: residual.and_then(|r| r.cascade).map_or(0.0, |c| c.hover_n),
				pidfw_k_thrust: residual.and_then(|r| r.cascade).map_or(0.0, |c| c.k_thrust),
				pidfw_decimation: residual
					.and_then(|r| r.cascade)
					.map_or(1, |c| c.decimation.max(1)),
				pidfw_lpf: residual.and_then(|r| r.cascade).map_or([0.0; 5], |c| c.lpf),
				pidfw_filter_on: residual.and_then(|r| r.cascade).map_or(0, |c| c.filter_on),
				...
				dhat_on: if controllers[0].dhat_params().is_some()
				{
					1
				}
				else
				...
				dhat_ff: if controllers[0].dhat_ff_params().0
				{
					1
				}
				else
			...
			let reward =
				unsafe { std::slice::from_raw_parts(b_reward.contents() as *const f32, n_out_chunk) };
			let sumerr =
				unsafe { std::slice::from_raw_parts(b_sumerr.contents() as *const f32, n_out_chunk) };
			let stepsv =
				unsafe { std::slice::from_raw_parts(b_steps.contents() as *const u32, n_out_chunk) };
			let divv = unsafe { std::slice::from_raw_parts(b_div.contents() as *const u32, n_out_chunk) };
			let jerkv =
				unsafe { std::slice::from_raw_parts(b_jerk.contents() as *const f32, n_out_chunk) };
			let monov =
				unsafe { std::slice::from_raw_parts(b_mono.contents() as *const f32, n_out_chunk) };
			let steadyv =
				unsafe { std::slice::from_raw_parts(b_steady.contents() as *const f32, n_out_chunk) };
			let risev =
				unsafe { std::slice::from_raw_parts(b_rise.contents() as *const f32, n_out_chunk) };
			let settleabv =
				unsafe { std::slice::from_raw_parts(b_settleab.contents() as *const f32, n_out_chunk) };
			let settlerev =
				unsafe { std::slice::from_raw_parts(b_settlere.contents() as *const f32, n_out_chunk) };
			let itaev =
				unsafe { std::slice::from_raw_parts(b_itae.contents() as *const f32, n_out_chunk) };
			...
			let effortv =
				unsafe { std::slice::from_raw_parts(b_effort.contents() as *const f32, n_out_chunk * 3) };
			for gi in 0..g
			{
...
) -> PyResult<Vec<Vec<f64>>>
{
	...
	let stage1_cfg = if translation
	{
		Some(crate::stage1::Stage1Cfg {
			...
			init_vz,
			mass: ep_mass,
			collective_frac: ep_collective_frac,
			lambda_pos,
			init_x,
			init_y,
		})
	...
	let evaluator =
		ControllerRolloutEvaluator::new().map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e))?;
...
) -> Option<usize>
{
	...
	for _ in 0..cap
	{
		...
		if markers[slot] == MARKER_FINAL_U32 && keys[slot] == addr
		{
			return Some(slot);
		}
		idx = ((idx as u64 + 1) & mask) as usize;
...
impl SlotTable
{
	...
	) -> Self
	{
		...
		let (mut off, mut cap) = (
			Vec::with_capacity(g * num_neurons),
			Vec::with_capacity(g * num_neurons),
		);
		let mut total: u64 = 0;
...
pub struct TrainBatch<'a>
{
	...
	pub vert_obs: &'a [f32],
	pub horiz_obs: &'a [f32],
	// obs_pwm replay fix (13/09/2026): per-step pwm FEATURE values (*4,
	// accumulator − anchor) the student's compute_features produced at rollout
	// (ReplayObs.pwm_dev twin). The kernels feed them to derive_features as-is.
	pub pwm_dev: &'a [f32],
	pub selective: bool,
...
fn replay_obs_batch_guard(batch: &TrainBatch, site: &str)
{
	...
	assert!(
		batch.pwm_dev.len() == total_steps * 4,
		"{site}: replay pwm-feature buffer misaligned (pwm_dev {} floats for {} steps) — \
...
impl ControllerTrainer
{
	pub fn new() -> Result<Self, String>
	{
		...
		let sep_func = sep_lib
			.get_function("controller_sep_bidir", None)
			.map_err(|e| format!("kernel controller_sep_bidir not found: {e}"))?;
		let sep_bidir_pipeline = device
			.new_compute_pipeline_state_with_function(&sep_func)
		...
		Ok(Self {
			...
			sep_counts_pipeline,
			sep_bidir_pipeline,
			plant_table_pipeline,
			plant_bidir_pipeline,
			mht_populate_pipeline,
			mht_probe_pipeline,
			state_commit_pipeline,
			nudge_dist_pipeline,
			proj_addr_pipeline,
			cand_rank_pipeline,
			phase1_pipeline,
	...
	) -> Result<Vec<Vec<Option<Vec<bool>>>>, String>
	{
		...
		for l in layers
		{
			...
			for _ in 0..l.num_inst
			{
				ib_rep.extend_from_slice(&ib);
			}
			...
			for (i, b) in [
				&b_k, &b_v, &b_of, &b_cn, &b_co, &b_ib, &b_tb, &b_oa, &b_or, &b_oc, &b_p1,
			]
			.iter()
			.enumerate()
			{
			...
			let tg = self
				...
				.min(all_neurons as u64)
				.max(1);
			enc.dispatch_threads(
				MTLSize::new(all_neurons as u64, 1, 1),
				MTLSize::new(tg, 1, 1),
			);
			enc.end_encoding();
			...
			let p2: [u32; 4] = [
				l.num_inst as u32,
				l.neurons_per_inst as u32,
				l.n_bits as u32,
				k_top as u32,
			];
			let q2: [u32; 2] = [l.total_input_bits as u32, l.n_immutable_bits as u32];
			let b_p2 = self.device.new_buffer_with_data(
				p2.as_ptr() as *const _,
				16,
				MTLResourceOptions::StorageModeShared,
			);
			let b_q2 = self.device.new_buffer_with_data(
				q2.as_ptr() as *const _,
				8,
				MTLResourceOptions::StorageModeShared,
			);
			...
			for (i, b) in [
				&b_oa, &b_or, &b_oc, &b_co, &b_ibr, &b_scr, &b_ob, &b_ok, &b_p2, &b_q2,
			]
			.iter()
			.enumerate()
			{
				enc2.set_buffer(i as u64, Some(b), 0);
			}
			let tg2 = self
				.beam_pipeline
				.max_total_threads_per_threadgroup()
				.min(l.num_inst as u64)
				.max(1);
			enc2.dispatch_threads(
				MTLSize::new(l.num_inst as u64, 1, 1),
				MTLSize::new(tg2, 1, 1),
			);
			enc2.end_encoding();
		...
		Ok(
			held
				.iter()
				.map(|(b_ob, b_ok, n_inst, tib, _)| {
					let ok = unsafe { std::slice::from_raw_parts(b_ok.contents() as *const u32, *n_inst) };
					let bits =
						unsafe { std::slice::from_raw_parts(b_ob.contents() as *const u8, n_inst * tib) };
					(0..*n_inst)
						.map(|i| {
							if ok[i] == 0
							{
	...
	) -> Result<Vec<Option<Vec<bool>>>, String>
	{
		...
		enc2.dispatch_threads(
			MTLSize::new(num_motors as u64, 1, 1),
			MTLSize::new(num_motors as u64, 1, 1),
		);
		enc2.end_encoding();
		cmd.commit();
		...
		{
			return Err(format!(
				"solve_qsr_reachable_motors failed: {:?}",
				cmd.status()
			));
		}
		let ok = unsafe { std::slice::from_raw_parts(b_ok.contents() as *const u32, num_motors) };
		let bits = unsafe {
			std::slice::from_raw_parts(b_ob.contents() as *const u8, num_motors * total_input_bits)
		};
		Ok(
			(0..num_motors)
				.map(|m| {
					...
					{
						Some(
							bits[m * total_input_bits..(m + 1) * total_input_bits]
								.iter()
								.map(|&b| b != 0)
								.collect(),
						)
	...
	) -> Result<Option<Vec<bool>>, String>
	{
		...
		let tg1 = self
			.phase1_pipeline
			.max_total_threads_per_threadgroup()
			.min(num_neurons as u64)
			.max(1);
		enc.dispatch_threads(
			MTLSize::new(num_neurons as u64, 1, 1),
			MTLSize::new(tg1, 1, 1),
		);
		enc.end_encoding();
		...
		enc2.set_compute_pipeline_state(&self.beam_pipeline);
		for (i, b) in [
			&b_oa, &b_or, &b_oc, &b_co, &b_ib, &b_scr, &b_ob, &b_ok, &b_p2, &b_q2,
		]
		.iter()
		.enumerate()
		{
		...
		let bits =
			unsafe { std::slice::from_raw_parts(b_ob.contents() as *const u8, total_input_bits) };
	...
	) -> Result<Vec<Vec<(u64, u8)>>, String>
	{
		...
		let b_par = self.device.new_buffer_with_data(
			...
			mem::size_of::<StateCommitParams>() as u64,
			MTLResourceOptions::StorageModeShared,
		);
		...
		let bufs: [&Buffer; 11] = [
			&b_cg, &b_cn, &b_ca, &b_ct, &b_mk, &b_ky, &b_vl, &b_off, &b_cap, &b_par, &b_w,
		];
		for (i, b) in bufs.iter().enumerate()
		{
			enc.set_buffer(i as u64, Some(b), 0);
		...
		{
			return Err(format!(
				"controller_state_commit failed: {:?}",
				cmd.status()
			));
		}

		let mk =
			unsafe { std::slice::from_raw_parts(b_mk.contents() as *const u32, table.markers.len()) };
		let ky = unsafe { std::slice::from_raw_parts(b_ky.contents() as *const u64, table.keys.len()) };
		let vl =
			unsafe { std::slice::from_raw_parts(b_vl.contents() as *const u32, table.values.len()) };
		Ok(
			(0..total)
				.map(|gn| {
	...
	) -> Result<Vec<Vec<(u64, u8)>>, String>
	{
		...
		let p = TrainParams {
			...
			target0: batch.target_rpy[0],
			target1: batch.target_rpy[1],
			target2: batch.target_rpy[2],
			...
			dhat_b: controllers[0].dhat_params().map_or([0.0; 3], |(b, _)| b),
			dhat_l_gain: controllers[0].dhat_params().map_or(0.05, |(_, g)| g),
			out_full_window: if controllers[0].output_full_window_pub()
			{
				1
			}
			else
		...
		let b_par = self.device.new_buffer_with_data(
			&p as *const _ as *const _,
			mem::size_of::<TrainParams>() as u64,
			MTLResourceOptions::StorageModeShared,
		);
		...
		let b_pa = self.buf(batch.pwm_dev); // obs_pwm replay fix (buffer 25)
		...
		for gn in 0..g * num_out
		{
			...
			{
				if mk[s] == MARKER_FINAL_U32
				{
					entries.push((ky[s], (vl[s] & 0xFF) as u8));
				}
			}
	...
	) -> Result<RecordBuffers, String>
	{
		...
		let b_tg = self.buf(batch.targets);
		let b_pp = self.buf(batch.pid_pwms);
		let b_ro = self.buf(&rec_out);
		let b_rs = self.buf(&rec_state);
		let b_rp = self.buf(&rec_pwm);
		let b_par = self.device.new_buffer_with_data(
			&p as *const _ as *const _,
			mem::size_of::<TrainParams>() as u64,
			MTLResourceOptions::StorageModeShared,
		...
		let b_pa = self.buf(batch.pwm_dev); // obs_pwm replay fix (buffer 22)
	...
	) -> Result<Vec<(Vec<bool>, Vec<bool>, [f32; 4])>, String>
	{
		...
		let ro = unsafe {
			std::slice::from_raw_parts(
				rb.b_ro.contents() as *const u32,
				rb.total_steps * rb.out_words,
			)
		};
		let rs = unsafe {
			std::slice::from_raw_parts(
				rb.b_rs.contents() as *const u32,
				rb.total_steps * rb.state_words,
			)
		};
		let rp =
			unsafe { std::slice::from_raw_parts(rb.b_rp.contents() as *const f32, rb.total_steps * 4) };
		let unpack = |buf: &[u32], base_w: usize, len: usize| -> Vec<bool> {
			(0..len)
				.map(|pos| (buf[base_w + (pos >> 5)] >> (pos & 31)) & 1 != 0)
				.collect()
		};
	...
	) -> Result<(Vec<ScanConflict>, usize), String>
	{
		...
		for k in (1..=bpf).rev()
		{
			...
			let p = ScanParams {
				...
				out_input_len: out_input_len as u32,
				out_words: out_words as u32,
				frame_bits: frame_bits as u32,
				bpf: bpf as u32,
				num_features: num_features as u32,
				k: k as u32,
				key_words: key_words as u32,
				slot_capacity: cap as u32,
			};
			let b_par = self.device.new_buffer_with_data(
				&p as *const _ as *const _,
				mem::size_of::<ScanParams>() as u64,
				MTLResourceOptions::StorageModeShared,
			);
			...
			{
				return Err(format!(
					"controller_scan: hash table full at k={k} (cap={cap})"
				));
			}
			...
			let mut conflicts: Vec<ScanConflict> = buckets
				...
				.filter_map(|(_, idxs)| {
					...
					(spread > tau).then(|| ScanConflict {
						out_in: out_ins.map_or_else(Vec::new, |oi| {
							crate::controller_split::coarse_key(&oi[idxs[0]], k, bpf, num_features, frame_bits)
						}),
						instances: idxs,
						spread,
					})
	...
	) -> Result<(Vec<ScanConflict>, usize), String>
	{
		...
		let rp =
			unsafe { std::slice::from_raw_parts(rb.b_rp.contents() as *const f32, rb.total_steps * 4) };
		let pwms: Vec<[f32; 4]> = (0..rb.total_steps)
			.map(|r| [rp[r * 4], rp[r * 4 + 1], rp[r * 4 + 2], rp[r * 4 + 3]])
			.collect();
		self.scan_buffer(
			&rb.b_ro,
			rb.total_steps,
	...
	) -> Result<Vec<Option<crate::controller_split::Separator>>, String>
	{
		...
		let p = SepParams {
			num_conflicts: c as u32,
			num_bits: b as u32,
			state_words: state_words as u32,
		};
		let b_par = self.device.new_buffer_with_data(
			&p as *const _ as *const _,
			mem::size_of::<SepParams>() as u64,
			MTLResourceOptions::StorageModeShared,
		);
		...
		let bufs: [&Buffer; 14] = [
			&b_base, &b_cnt, &b_inst, &b_lab, &b_epof, &b_stof, &b_epst, b_st, &b_cb, &b_ml, &b_og,
			&b_ol, &b_oh, &b_par,
		];
	...
	pub fn accumulator_search(
		...
		Vec<(
			Option<crate::controller_split::Accumulator>,
			Option<crate::controller_split::BidirAccumulator>,
		)>,
		String,
	>
	{
		...
		let state_packed = state_ins_flat; // already in kernel word layout
		let b_state = self.buf(&state_packed);
		self.accumulator_search_buffer(
			&b_state,
			state_words,
			conflicts,
			scalars,
	...
	>
	{
		...
		let b_par = self.device.new_buffer_with_data(
			&p as *const _ as *const _,
			mem::size_of::<CountParams>() as u64,
			MTLResourceOptions::StorageModeShared,
		);
		...
		let bufs: [&Buffer; 12] = [
			&b_base, &b_cnt, &b_inst, &b_epof, &b_stof, &b_epst, b_st, &b_cb, &b_ml, &b_bb, &b_co, &b_par,
		];
		for (i, bf) in bufs.iter().enumerate()
		{
			enc.set_buffer(i as u64, Some(bf), 0);
		...
		let co =
			unsafe { std::slice::from_raw_parts(b_co.contents() as *const f32, total_counts.max(1)) };
		...
		let bidir_corr: Vec<f32> = if do_bidir
		{
			...
			for (i, bf) in bufs2.iter().enumerate()
			{
				enc2.set_buffer(i as u64, Some(bf), 0);
			}
			let g = MTLSize::new(c as u64, b as u64, b as u64);
			let tg = MTLSize::new(
				4u64.min(c as u64).max(1),
				4u64.min(b as u64).max(1),
				4u64.min(b as u64).max(1),
			);
			...
			if do_bidir
			{
				for ai in 0..b
				{
					for bi in 0..b
					{
						...
						{
							bid = Some(BidirAccumulator {
								up: candidate_bits[ai],
								dn: candidate_bits[bi],
								corr,
							});
						}
					}
	...
	) -> Vec<(u64, u8)>
	{
		...
		let p = PlantParams {
			...
			state_words: state_words as u32,
			slot_cap: cap as u32,
			num_rel: rel_pos.len() as u32,
		};
		let b_par = self.device.new_buffer_with_data(
			&p as *const _ as *const _,
			mem::size_of::<PlantParams>() as u64,
			MTLResourceOptions::StorageModeShared,
		);
		...
		let tw = self
			.plant_table_pipeline
			.max_total_threads_per_threadgroup()
			.min(num_records.max(1) as u64)
			.max(1);
		enc.dispatch_threads(
			MTLSize::new(num_records.max(1) as u64, 1, 1),
			MTLSize::new(tw, 1, 1),
		);
		enc.end_encoding();
		cmd.commit();
	...
	) -> Result<(Option<usize>, Vec<(u64, u8)>), String>
	{
		...
		let conns: Vec<i32> = sc[c * sbpn..(c + 1) * sbpn]
			.iter()
			.map(|&x| x as i32)
			.collect();
		let state_words = (sil + 31) / 32;
		let num_records = if state_words == 0
		{
			0
		}
		else
		{
			state_ins_flat.len() / state_words
		};
		...
		let combo_vals: Vec<u8> = (0..4)
			...
			.collect();
		let entries = self.plant_table_dispatch(
			&packed,
			&conns,
			sbpn,
			state_words,
	...
	) -> Result<(Option<Vec<usize>>, Vec<usize>, Vec<Vec<(u64, u8)>>), String>
	{
		...
		for k in 0..chain.len()
		{
			...
			let tp = match pos(conns_i64, trigger)
			{
				Some(p) => p as u32,
				None => return Ok((None, written, per_neuron)),
			};
			let sp = match pos(conns_i64, sensor_window + c)
			{
				Some(p) => p as u32,
				None => return Ok((None, written, per_neuron)),
			};
			...
			else
			{
				...
				let lp = match pos(conns_i64, sensor_window + chain[k - 1])
				{
					Some(p) => p as u32,
					None => return Ok((None, written, per_neuron)),
				};
			...
			per_neuron.push(self.plant_table_dispatch(
				&packed,
				&conns,
				sbpn,
				state_words,
	...
	) -> Result<(Option<Vec<usize>>, Vec<Vec<(u64, u8)>>), String>
	{
		...
		let b_par = self.device.new_buffer_with_data(
			&p as *const _ as *const _,
			mem::size_of::<BidirPlantParams>() as u64,
			MTLResourceOptions::StorageModeShared,
		);
		let cmd = self.queue.new_command_buffer();
		let enc = cmd.new_compute_command_encoder();
	...
	) -> Result<(i64, Vec<usize>), String>
	{
		...
		let sampled =
			crate::controller::subsample_instances(instances, crate::controller::SPLIT_INST_CAP);
		...
		let max_lag = sampled
			...
			.min()
			.unwrap_or(0)
			.min(crate::controller::SPLIT_LAG_CAP);

		// TYPE-1: discriminative_walk → set/hold latch.
		let seps = self.sep_walk(
			&[sampled.clone()],
			&[labels],
			ep_of,
			step_of,
		...
		{
			let (n_opt, cells) =
				self.plant_latch(controller, state_ins_flat, sil, s.bit, s.high_on, used)?;
			if let Some(n) = n_opt
			{
				for (addr, val) in cells
				{
					controller.plant_state_cell(n, addr, val);
				}
				return Ok((1, vec![n]));
			}
		}
		...
		let do_bidir = sbpn >= 5;
		let accs = self.accumulator_search(
			&[sampled],
			&[scalar],
			ep_of,
			step_of,
		...
		if do_bidir
		{
			...
			{
				let (lv_opt, per_neuron) =
					self.plant_counter_bidir(controller, b.up, b.dn, n_state, used)?;
				if let Some(levels) = lv_opt
				{
					for (k, &nn) in levels.iter().enumerate()
					{
						for &(addr, val) in &per_neuron[k]
						{
							controller.plant_state_cell(nn, addr, val);
						}
					}
					return Ok((3, levels));
				}
		...
		{
			let (chain_opt, written, per_neuron) =
				self.plant_counter(controller, state_ins_flat, sil, a.bit, n_state, used)?;
	...
	) -> Result<(usize, usize, usize, Vec<usize>), String>
	{
		...
		let (sc, _oc, _se, _oe) = controller.gpu_export();
		let mut candidate_bits: Vec<usize> = sc
			.iter()
			.map(|&x| x as usize)
		...
		let epl: Vec<usize> = (0..ne)
			.map(|j| (batch.step_count[e0 + j] as usize).div_ceil(action_repeat))
			.collect();
		...
		for round in 0..max_rounds
		{
			...
			for n in 0..num_out
			{
				for &(addr, val) in &cells[n]
				{
					controller.set_output_cell(n, addr, val);
				}
			}
		}
	...
	pub fn mht_lookup_parity(&self, cells: &[(u64, u8)], queries: &[u64]) -> Result<usize, String>
	{
		...
		let p = MhtParams {
			slot_cap: cap as u32,
			num_cells: nc as u32,
			sorted_count: nc as u32,
			num_q: nq as u32,
		};
		let b_par = self.device.new_buffer_with_data(
			&p as *const _ as *const _,
			mem::size_of::<MhtParams>() as u64,
			MTLResourceOptions::StorageModeShared,
		);
		...
		for (i, bf) in [&b_ca, &b_cv, &b_mk, &b_ky, &b_vl, &b_par]
			.iter()
			.enumerate()
		{
			enc.set_buffer(i as u64, Some(bf), 0);
		...
		for (i, bf) in [
			&b_sk, &b_sv, &b_mk, &b_ky, &b_vl, &b_q, &b_ob, &b_om, &b_par,
		]
		.iter()
		.enumerate()
		{
...
pub fn run_controller_train_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_train_parity".to_string(),
			true,
	...
	for &selective in &[false, true]
	{
		match controller_train_parity_once(selective)
		{
			...
			Err(e) => results.push((
				format!("controller_train_parity(selective={selective})"),
				false,
				e,
			)),
		}
...
struct ParityFixture
{
	...
	vert_obs: Vec<f32>,
	horiz_obs: Vec<f32>,
	pwm_dev: Vec<f32>, // obs_pwm replay fix: zeros (fixtures run obs_pwm OFF)
...
fn build_parity_fixture_mode(
	seed_salt: u64,
	memory_mode: u8,
	output_decode: Option<u8>,
) -> Result<ParityFixture, String>
{
	...
	let thresholds: Vec<f32> = (0..num_features * bpf)
		.map(|_| xf(&mut rng) - 0.5)
		.collect();
	...
	let c = WnnController::new(
		...
		0.05,
		false,
		0.30, // dhat_b/ff: parity fixtures keep the observer OFF
		false,
		false,
		false, // stage-1 vertical channel OFF
		false,
	...
	let (mut gyros, mut accels, mut targets, mut pids) =
		(Vec::new(), Vec::new(), Vec::new(), Vec::new());
	let (mut step_base, mut step_count) = (Vec::new(), Vec::new());
	let (mut cpu_g, mut cpu_a, mut cpu_t, mut cpu_p) =
		(Vec::new(), Vec::new(), Vec::new(), Vec::new());
	let mut sbase = 0u32;
	for _ in 0..e_count
	{
		step_base.push(sbase);
		step_count.push(t_steps as u32);
		sbase += t_steps as u32;
		let (mut eg, mut ea, mut et, mut ep): (
			Vec<[f32; 3]>,
			Vec<[f32; 3]>,
			Vec<[f32; 3]>,
			Vec<[f32; 4]>,
		...
		for _ in 0..t_steps
		{
			...
			gyros.extend_from_slice(&gy);
			accels.extend_from_slice(&ac);
			targets.extend_from_slice(&tg);
			pids.extend_from_slice(&pw);
			eg.push(gy);
			ea.push(ac);
			et.push(tg);
			ep.push(pw);
	...
	let cpu_obs: Vec<ReplayObs> = cpu_g
		.iter()
		.map(|g| ReplayObs { vert: vec![[0.0; 3]; g.len()], horiz: vec![[0.0; 4]; g.len()], pwm_dev: vec![] })
		.collect();
	Ok(ParityFixture {
		c,
		num_out,
...
fn controller_train_parity_once(selective: bool) -> Result<(usize, usize, usize), String>
{
	...
	for n in 0..num_out
	{
		...
		for &(a, _) in gpu_entries
		{
			all.insert(a);
		}
		for &(a, _) in &cpu_entries
		{
			all.insert(a);
		}
		addrs += all.len();
...
pub fn run_controller_train_seeded_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_train_seeded_parity".to_string(),
			true,
			"skipped: no Metal device".to_string(),
		));
		return results;
	...
	for &selective in &[false, true]
	{
		match controller_train_seeded_parity_once(selective)
		{
			...
			Err(e) => results.push((
				format!("controller_train_seeded_parity(selective={selective})"),
				false,
				e,
			)),
		}
...
fn controller_train_seeded_parity_once(
	selective: bool,
...
pub fn run_controller_beam_solve_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_beam_solve_parity".to_string(),
			true,
			"skipped: no Metal device".to_string(),
		));
		return results;
	...
	let t = match ControllerTrainer::new()
	{
		...
		Err(e) =>
		{
			results.push(("controller_beam_solve_parity".to_string(), false, e));
			return results;
		}
	};
	const K_TOP: usize = 4;
	for &mode in &[2u8, 0u8, 3u8]
	{
		...
		let f = match build_parity_fixture_mode(0xBEA3_0000u64, mode, None)
		{
			...
			Err(e) =>
			{
				results.push((tag, false, format!("fixture: {e}")));
				continue;
			}
		};
		...
		for i in 0..INST
		{
			for j in 0..tib
			{
				input_bits[i * tib + j] = (next() % 2) as u8;
			}
			for j in 0..n_state
			{
				target_bits[i * n_state + j] = (next() % 2) as u8;
			}
		}
		...
		for i in 0..INST
		{
			...
			for (bi, b) in [
				&b_k, &b_v, &b_of, &b_cn, &b_co, &b_ib, &b_tb, &b_oa, &b_or, &b_oc, &b_p,
			]
			.iter()
			.enumerate()
			{
				enc.set_buffer(bi as u64, Some(b), 0);
			}
			enc.dispatch_threads(
				MTLSize::new(n_state as u64, 1, 1),
				MTLSize::new(n_state as u64, 1, 1),
			);
			enc.end_encoding();
			cmd.commit();
		...
		let b_q = t.device.new_buffer_with_data(
			q.as_ptr() as *const _,
			8,
			MTLResourceOptions::StorageModeShared,
		);
		...
		for (bi, b) in [
			&b_ta, &b_tr, &b_tc, &b_co, &b_ib, &b_scr, &b_ob, &b_ok, &b_p, &b_q,
		]
		.iter()
		.enumerate()
		{
		...
		for i in 0..INST
		{
			let ibits: Vec<bool> = input_bits[i * tib..(i + 1) * tib]
				.iter()
				.map(|&b| b != 0)
				.collect();
			let tbits: Vec<bool> = target_bits[i * n_state..(i + 1) * n_state]
				.iter()
				.map(|&b| b != 0)
				.collect();
			let want = crate::controller_training::solve_partial_connectivity_qsr_reachable(
				|n| f.c.state_entries(n),
				ram_core::neuron_memory::EMPTY_U8,
				sconn,
				n_state,
				n_bits,
				tib,
			...
			match (want, gok[i] == 1)
			{
				(Some(w), true) =>
				{
					...
					if w != g
					{
						...
						if first.is_empty()
						{
							...
							first = format!(
								"inst {i}: first bit diff at {k} (cpu={} gpu={})",
								w[k], g[k]
							);
						}
					}
				...
				(w, ok) =>
				{
					...
					{
						first = format!(
							"inst {i}: solvability differs — cpu={} gpu={ok}",
							w.is_some()
						);
					}
				}
...
pub fn run_controller_phase1_topk_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_phase1_topk_parity".to_string(),
			true,
			"skipped: no Metal device".to_string(),
		));
		return results;
	...
	let t = match ControllerTrainer::new()
	{
		...
		Err(e) =>
		{
			results.push(("controller_phase1_topk_parity".to_string(), false, e));
			return results;
		}
	};
	...
	for &mode in &[2u8, 0u8, 3u8]
	{
		for &k_top in &[1usize, 4]
		{
			...
			let ga =
				unsafe { std::slice::from_raw_parts(b_oa.contents() as *const u64, n_state * k_top) };
			...
			results.push((
				...
				if bad == 0
				{
					format!("{n_state} neurons, k_top={k_top}: selected address sequences identical")
				}
				else
				{
					format!("{bad}/{n_state} neurons differ; first {first}")
				},
			));
		}
	}
...
pub fn run_controller_candidate_rank_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_candidate_rank_parity".to_string(),
			true,
			"skipped: no Metal device".to_string(),
		));
		return results;
	...
	let t = match ControllerTrainer::new()
	{
		...
		Err(e) =>
		{
			results.push(("controller_candidate_rank_parity".to_string(), false, e));
			return results;
		}
	};
	...
	for (i, b) in [&b_c, &b_t, &b_m, &b_a, &b_p, &b_o, &b_n]
		.iter()
		.enumerate()
	{
		enc.set_buffer(i as u64, Some(b), 0);
	...
	for i in 0..n
	{
		...
		if want != got[i]
		{
			...
			{
				first = format!(
					"i={i} cell={} tgt={} mode={} cpu={want} gpu={}",
	...
	results.push((
		"candidate_rank_values".to_string(),
		bad == 0,
		if bad == 0
		{
			format!("{n} random candidates agree")
		}
		else
		{
			format!("{bad}/{n} differ; first {first}")
		},
	));
	...
	for &sat in &[0.0f64, 0.37, 7.5, 14.999, 15.0]
	{
		...
		by_f64.sort_by(|&a, &b| {
			let ca = 7.0f64
				* crate::cell_mode::nudge_distance(cells[a], targets[a] != 0, modes[a]) as f64
				+ (addrs[a] ^ projs[a]).count_ones() as f64
				+ sat;
			let cb = 7.0f64
				* crate::cell_mode::nudge_distance(cells[b], targets[b] != 0, modes[b]) as f64
				+ (addrs[b] ^ projs[b]).count_ones() as f64
				+ sat;
	...
	results.push((
		"candidate_rank_orders_like_f64_cost".to_string(),
		order_bad == 0,
		if order_bad == 0
		{
			format!("integer key reproduces the f64 ordering of {n} candidates at 5 saturation values")
		}
		else
		{
			format!("{order_bad}/5 saturations reorder; {order_first}")
		},
	));
	results
}
...
pub fn run_controller_projected_address_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_projected_address_parity".to_string(),
			true,
			"skipped: no Metal device".to_string(),
		));
		return results;
	...
	let t = match ControllerTrainer::new()
	{
		...
		Err(e) =>
		{
			results.push(("controller_projected_address_parity".to_string(), false, e));
			return results;
		}
	};
	...
	for &n_bits in &[4usize, 12, 24, 30]
	{
		for (label, fill) in [("random", 2u8), ("all_zero", 0), ("all_one", 1)]
		{
			...
			for (i, b) in [&b_c, &b_i, &b_o, &b_p].iter().enumerate()
			{
				enc.set_buffer(i as u64, Some(b), 0);
			}
			enc.dispatch_threads(
			...
			for n in 0..num_neurons
			{
				...
				if want != got[n]
				{
					...
					if first.is_empty()
					{
						first = format!("neuron {n}: cpu={want:#x} gpu={:#x}", got[n]);
					}
				}
			}
			results.push((
				...
				if bad == 0
				{
					format!("{num_neurons} neurons agree at n_bits={n_bits} ({label})")
				}
				else
				{
					format!("{bad}/{num_neurons} differ; first {first}")
				},
			));
		}
	}
...
pub fn run_controller_nudge_distance_parity_test() -> Vec<(String, bool, String)>
{
	...
	let t = match ControllerTrainer::new()
	{
		...
		Err(e) =>
		{
			results.push(("controller_nudge_distance_parity".to_string(), false, e));
			return results;
		}
	};
	let (mut cells, mut targets, mut modes) = (Vec::new(), Vec::new(), Vec::new());
	...
	enc.dispatch_threads(
		...
		MTLSize::new(n.min(48) as u64, 1, 1),
	);
	enc.end_encoding();
	cmd.commit();
	cmd.wait_until_completed();
	...
	results.push((
		...
		if bad.is_empty()
		{
			format!("all {} (cell,target,mode) combinations agree", cells.len())
		}
		else
		{
			format!(
...
pub fn run_controller_state_commit_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_state_commit_parity".to_string(),
			true,
			"skipped: no Metal device".to_string(),
		));
		return results;
	...
	let trainer = match ControllerTrainer::new()
	{
		...
		Err(e) =>
		{
			results.push(("controller_state_commit_parity".to_string(), false, e));
			return results;
		}
	};
	for &mode in &[2u8, 0u8, 3u8]
	{
		...
		let table = SlotTable::build(
			&[commits.len() as u64],
			n_state,
			Some(std::slice::from_ref(&sexp)),
		);
		let gpu = match trainer.state_commit(&table, &commits, 1, n_state, mode)
		{
			...
			Err(e) =>
			{
				results.push((tag, false, format!("dispatch: {e}")));
				continue;
			}
		};
		...
		for n in 0..n_state
		{
			...
			for addr in 0u64..24
			{
				...
				let have = got
					.get(&addr)
					.copied()
					.unwrap_or(ram_core::neuron_memory::EMPTY_U8);
				checked += 1;
				...
				let (wf, hf) = (
					crate::cell_mode::cell_weight(want, mode),
				...
				if wf != hf
				{
					...
					{
						first =
							format!("neuron {n} addr {addr}: cpu cell={want} w={wf} vs gpu cell={have} w={hf}");
					}
				}
			}
		}
		results.push((
			...
			{
				format!(
					"{} commits, {checked} (neuron,addr) cell reads identical across {n_state} neurons",
					commits.len()
				)
			}
			else
			{
				format!("{mism}/{checked} cell reads differ; first {first}")
			},
		));
	}
	results
...
pub fn run_controller_bptt_window_parity_test() -> Vec<(String, bool, String)>
{
	...
	for &(mode, decode) in &[(2u8, None::<u8>), (0u8, None), (3u8, Some(1u8))]
	{
		...
		let requested = std::env::var("WNN_CONTROLLER_GPU_SOLVE")
			.map(|v| v == "1")
			.unwrap_or(false);
		...
		results.push((
			...
			d_rev != d1,
			format!(
				"forward {d1:#018x} vs reversed {d_rev:#018x} — these MUST differ: \
...
pub fn run_controller_split_train_loop_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_split_train_loop_parity".to_string(),
			true,
			"skipped: no Metal device".to_string(),
		));
		return results;
...
fn controller_split_train_loop_parity_once(
	selective: bool,
	coarse_target: usize,
	salt: u64,
	memory_mode: u8,
...
) -> Result<(usize, usize, usize, usize, usize), String>
{
	...
	let trainer = ControllerTrainer::new()?;
	let batch = TrainBatch {
		ep_base: &f_gpu.ep_base,
		ep_count: &f_gpu.ep_count,
		step_base: &f_gpu.step_base,
		step_count: &f_gpu.step_count,
		gyros: &f_gpu.gyros,
		accels: &f_gpu.accels,
	...
	let mut s_mism = 0usize;
	let mut s_addr = 0usize;
	for n in 0..n_state
	{
		let mut all: std::collections::BTreeSet<u64> = std::collections::BTreeSet::new();
		for (a, _) in c_cpu.state_entries(n)
		{
			all.insert(a);
		}
		for (a, _) in c_gpu.state_entries(n)
		{
			all.insert(a);
		}
		s_addr += all.len();
		for a in all
		{
			if c_cpu.state_cell(n, a) != c_gpu.state_cell(n, a)
			{
				s_mism += 1;
			}
		}
	...
	let mut o_mism = 0usize;
	let mut o_addr = 0usize;
	for n in 0..num_out
	{
		let mut all: std::collections::BTreeSet<u64> = std::collections::BTreeSet::new();
		for (a, _) in c_cpu.output_entries(n)
		{
			all.insert(a);
		}
		for (a, _) in c_gpu.output_entries(n)
		{
			all.insert(a);
		}
		o_addr += all.len();
		for a in all
		{
			if c_cpu.output_cell(n, a) != c_gpu.output_cell(n, a)
			{
				o_mism += 1;
			}
		}
...
pub fn run_controller_record_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_record_parity".to_string(),
			true,
			"skipped: no Metal device".to_string(),
		));
		return results;
...
fn controller_record_parity_once() -> Result<(usize, usize, usize, usize), String>
{
	...
	// CPU reference.
	let (cpu_out_ins, cpu_pwms, cpu_state_flat, state_len) = c.split_record_pub(
		f.cpu_g.clone(),
		f.cpu_a.clone(),
		f.cpu_t.clone(),
		f.cpu_p.clone(),
	...
	{
		return Err(format!(
			"record count mismatch: gpu={records} cpu={}",
			cpu_out_ins.len()
		));
	}
	...
	for r in 0..records
	{
		...
		if (0..state_len)
			.any(|b| g_state[b] != crate::controller_split::sif_bit(&cpu_state_flat, r, state_len, b))
		{
			mism_state += 1;
		}
		for m in 0..4
		{
			if (g_pwm[m] - cpu_pwms[r][m]).abs() > 1e-6
			{
				mism_pwm += 1;
				break;
			}
...
pub fn run_controller_scan_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_scan_parity".to_string(),
			true,
			"skipped: no Metal device".to_string(),
		));
		return results;
	...
	for (case, label) in [(0u32, "single_word"), (1u32, "multi_word")]
	{
		match controller_scan_parity_once(case)
		{
			Ok((cpu_k, gpu_k, n_conf, mism)) =>
			{
				...
				results.push((
					format!("controller_scan_parity({label})"),
					ok,
					format!("cpu_k={cpu_k}, gpu_k={gpu_k}, conflicts={n_conf}, mismatches={mism}"),
				));
			}
...
fn controller_scan_parity_once(case: u32) -> Result<(usize, usize, usize, usize), String>
{
	...
	let trainer = ControllerTrainer::new()?;
	let (gpu_conf, gpu_k) = trainer.scan(
		&out_ins,
		&pwms,
		tau,
		bpf,
	...
	let cpu_set: std::collections::HashMap<Vec<usize>, (u32, Vec<bool>)> = cpu_conf
		.iter()
		.map(|c| (c.instances.clone(), (c.spread.to_bits(), c.out_in.clone())))
		.collect();
...
pub fn run_controller_sep_walk_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_sep_walk_parity".to_string(),
			true,
			"skipped: no Metal device".to_string(),
		));
		return results;
	...
	match controller_sep_walk_parity_once()
	{
		Ok((n_conf, n_some, mism)) =>
		{
			results.push((
				"controller_sep_walk_parity".to_string(),
				mism == 0,
				format!("conflicts={n_conf}, separators_found={n_some}, mismatches={mism}"),
			));
		}
...
pub fn run_controller_accumulator_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_accumulator_parity".to_string(),
			true,
			"skipped: no Metal device".to_string(),
		));
		return results;
...
fn controller_accumulator_parity_once() -> Result<(usize, usize, usize, usize, usize), String>
{
	...
	let trainer = ControllerTrainer::new()?;
	let gpu = trainer.accumulator_search(
		&conflicts,
		&scalars,
		&ep_of,
		&step_of,
...
pub fn run_controller_plant_latch_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_plant_latch_parity".to_string(),
			true,
			"skipped: no Metal device".to_string(),
		));
		return results;
	...
	for high_on in [false, true]
	{
		match controller_plant_latch_parity_once(high_on)
		{
			...
			{
				results.push((
					format!("controller_plant_latch_parity(high_on={high_on})"),
					mism == 0,
					format!("addresses={addrs}, cpu_nonempty_cells={cpu_cells}, mismatches={mism}"),
				));
			}
			Err(e) => results.push((
				format!("controller_plant_latch_parity(high_on={high_on})"),
...
fn controller_plant_latch_parity_once(high_on: bool) -> Result<(usize, usize, usize), String>
{
	...
	let c = WnnController::new(
		...
		0.05,
		false,
		0.30, // dhat_b/ff: observer OFF for fixtures (bit-identical anchor)
		false,
		false,
		false, // stage-1 vertical channel OFF
		false,
	...
	let sif_bools: Vec<bool> = (0..num_records * state_input_len)
		.map(|_| xf(&mut rng) < 0.5)
		.collect();
	...
	let (gpu_n, gpu_cells) =
		trainer.plant_latch(&c, &sif, state_input_len, trigger, high_on, &used)?;
	...
	{
		return Err(format!(
			"planted-neuron mismatch: gpu={gpu_n:?} cpu={cpu_n:?}"
		));
	}
...
pub fn run_controller_plant_counter_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_plant_counter_parity".to_string(),
			true,
			"skipped: no Metal device".to_string(),
		));
		return results;
	...
	match controller_plant_counter_parity_once()
	{
		Ok((levels, addrs, mism)) =>
		{
			results.push((
				"controller_plant_counter_parity".to_string(),
				mism == 0,
				format!("chain_levels={levels}, addresses={addrs}, mismatches={mism}"),
			));
		}
...
fn controller_plant_counter_parity_once() -> Result<(usize, usize, usize), String>
{
	...
	let set = |sc: &mut [i64], c: usize, vals: &[i64]| {
		...
		for i in vals.len()..sbpn
		{
			sc[c * sbpn + i] = (60 + c * sbpn + i) as i64;
		} // fillers (<152, ≠5)
	};
	set(
		&mut state_conns,
		0,
		&[trigger as i64, (sensor_window) as i64],
	); // n0: trig, self144
	set(
		&mut state_conns,
		1,
		&[
			trigger as i64,
			(sensor_window) as i64,
			(sensor_window + 1) as i64,
		],
	); // n1: trig, lower144, self145
	set(
		&mut state_conns,
		2,
		&[
			trigger as i64,
			(sensor_window + 1) as i64,
			(sensor_window + 2) as i64,
		],
	); // n2: trig, lower145, self146
	for c in 3..n_state
	{
		for i in 0..sbpn
		{
	...
	let trainer = ControllerTrainer::new()?;
	let used = vec![false; n_state];
	let (gpu_chain, _written, gpu_cells) =
		trainer.plant_counter(&c, &sif, state_input_len, trigger, n_state, &used)?;

	let cpu_chain = c.split_install_counter_pub(trigger, n_state, &sif, state_input_len);
	if gpu_chain != cpu_chain
	{
		return Err(format!(
			"chain mismatch: gpu={gpu_chain:?} cpu={cpu_chain:?}"
		));
	}
...
pub fn run_controller_plant_bidir_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_plant_bidir_parity".to_string(),
			true,
			"skipped: no Metal device".to_string(),
		));
		return results;
	...
	match controller_plant_bidir_parity_once()
	{
		Ok((levels, addrs, mism)) =>
		{
			results.push((
				"controller_plant_bidir_parity".to_string(),
				mism == 0,
				format!("levels={levels}, addresses={addrs}, mismatches={mism}"),
			));
		}
...
fn controller_plant_bidir_parity_once() -> Result<(usize, usize, usize), String>
{
	...
	for k in 0..n_levels
	{
		...
		let vals = [
			...
			dn as i64,
			lower as i64,
			(sensor_window + k) as i64,
			upper as i64,
		];
		for (i, &v) in vals.iter().enumerate()
		{
			state_conns[k * sbpn + i] = v;
		}
		for i in 5..sbpn
		{
			state_conns[k * sbpn + i] = (60 + k * sbpn + i) as i64;
		}
	}
	...
	{
		return Err(format!(
			"levels mismatch: gpu={gpu_levels:?} cpu={cpu_levels:?}"
		));
	}
...
pub fn run_controller_mht_lookup_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_mht_lookup_parity".to_string(),
			true,
			"skipped: no Metal device".to_string(),
		));
		return results;
	...
	match controller_mht_lookup_parity_once()
	{
		Ok((nc, nq, mism)) =>
		{
			results.push((
				"controller_mht_lookup_parity".to_string(),
				mism == 0,
				format!("cells={nc}, queries={nq}, mismatches={mism}"),
			));
		}
...
fn controller_mht_lookup_parity_once() -> Result<(usize, usize, usize), String>
{
	...
	for _ in 0..300
	{
		queries.push(xs(&mut rng) % (1u64 << 20));
	}
...
pub fn run_controller_record_and_scan_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_record_and_scan_parity".to_string(),
			true,
			"skipped: no Metal device".to_string(),
		));
		return results;
...
fn controller_record_and_scan_parity_once() -> Result<(usize, usize, usize, usize), String>
{
	...
	let (gpu_conf, gpu_k) =
		trainer.record_and_scan(&f.c, &batch, tau, bpf, num_features, frame_bits, target_min)?;
	...
	let cpu_set: std::collections::HashMap<Vec<usize>, u32> = cpu_conf
		...
		.collect();
	let gpu_set: std::collections::HashMap<Vec<usize>, u32> = gpu_conf
		.iter()
		.map(|c| (c.instances.clone(), c.spread.to_bits()))
		.collect();
	let mut mism = 0usize;
...
pub fn run_controller_record_search_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_record_search_parity".to_string(),
			true,
			"skipped: no Metal device".to_string(),
		));
		return results;
	...
	match controller_record_search_parity_once()
	{
		Ok((n, mism_sep, mism_acc)) =>
		{
			...
			results.push((
				"controller_record_search_parity".to_string(),
				ok,
				format!("conflicts={n}, sep_mismatch={mism_sep}, accum_mismatch={mism_acc}"),
			));
		}
...
{
	use crate::controller_split::{
		detect_accumulator, detect_accumulator_bidir, discriminative_walk,
	};
	...
	let (step_base, step_count): (Vec<usize>, Vec<usize>) = (
		f.step_base.iter().map(|&x| x as usize).collect(),
		f.step_count.iter().map(|&x| x as usize).collect(),
	);
	...
	for ep in 0..e_count
	{
		...
		for s in 0..step_count[ep]
		{
			ep_of[step_base[ep] + s] = ep;
			step_of[step_base[ep] + s] = s;
		}
	}

	// CPU split_record state_ins (== GPU b_rs by P2a record parity) — the reference.
	let mut c = f.c;
	let (_oi, _pw, cpu_state_flat, cpu_sil) = c.split_record_pub(
		f.cpu_g.clone(),
		f.cpu_a.clone(),
		f.cpu_t.clone(),
		f.cpu_p.clone(),
	...
	let (mut conflicts, mut labels, mut scalars, mut max_lags) =
		(Vec::new(), Vec::new(), Vec::new(), Vec::new());
	for _ in 0..12
	{
		let ep = (xs(&mut rng) % e_count as u64) as usize;
		...
		let insts: Vec<usize> = (0..m)
			.map(|_| step_base[ep] + 5 + (xs(&mut rng) % (t as u64 - 5)) as usize)
			.collect();
	...
	let gpu_sep = trainer.sep_walk_buffer(
		&rb.b_rs,
		state_words,
		&conflicts,
		&labels,
	...
	let gpu_acc = trainer.accumulator_search_buffer(
		&rb.b_rs,
		state_words,
		&conflicts,
		&scalars,
	...
	for ci in 0..n
	{
		let cpu_sep = discriminative_walk(
			&conflicts[ci],
			&labels[ci],
			&ep_of,
			&step_of,
		...
		let cpu_acc = detect_accumulator(
			&conflicts[ci],
			&scalars[ci],
			&ep_of,
			&step_of,
		...
		let cpu_bid = detect_accumulator_bidir(
			&conflicts[ci],
			&scalars[ci],
			&ep_of,
			&step_of,
...
pub fn run_controller_resolve_conflict_parity_test() -> Vec<(String, bool, String)>
{
	...
	{
		results.push((
			"controller_resolve_conflict_parity".to_string(),
			true,
			"skipped: no Metal device".to_string(),
		));
		return results;
...
fn controller_resolve_conflict_parity_once() -> Result<(i64, i64, bool, usize), String>
{
	...
	let mut c_rec = f.c;
	let (out_ins, pwms, state_flat, sil) = c_rec.split_record_pub(
		f.cpu_g.clone(),
		f.cpu_a.clone(),
		f.cpu_t.clone(),
		f.cpu_p.clone(),
	...
	let (conflicts, _k) = crate::controller_split::scan_conflicts_coarse(
		&out_ins,
		&pwms,
		0.05,
		bpf,
...
/// pwm feature from the `pwm_dev` buffer (buffer 22) exactly as the CPU
/// split_record applies it from ReplayObs.pwm_dev. Same shape as the vertical
...
fn controller_record_obs_pwm_parity_once() -> Result<(usize, usize, usize, bool), String>
{
	...
	for fi in 9..13
	{
		for b in 0..bpf
		{
			thresholds[fi * bpf + b] = -0.15 + 0.1 * b as f32; // deviation ladder −.15 −.05 .05 .15
		}
	}
	let state_conns: Vec<i64> = (0..n_state * sbpn)
		.map(|_| (xs(&mut rng) % total_state_in as u64) as i64)
	...
	{
		let n = g.len().max(1);
		let pwm_dev: Vec<[f32; 4]> = (0..g.len())
			.map(|t| {
				let u = t as f32 / n as f32;
				[-0.2 + 0.4 * u, 0.2 - 0.4 * u, 0.2 * ((t % 5) as f32 - 2.0) / 2.0, -0.1 + 0.3 * ((t % 3) as f32) / 2.0]
			})
			.collect();
		for v in &pwm_dev
		{
		...
		cpu_obs.push(ReplayObs { vert: vec![[0.0; 3]; g.len()], horiz: vec![[0.0; 4]; g.len()], pwm_dev });
	}
	...
	let const_obs: Vec<ReplayObs> = f
		.cpu_g
		.iter()
		.map(|g| ReplayObs { vert: vec![[0.0; 3]; g.len()], horiz: vec![[0.0; 4]; g.len()], pwm_dev: vec![[0.0; 4]; g.len()] })
		.collect();
	let mut c2 = c.clone();
...
fn controller_record_vertical_obs_parity_once() -> Result<(usize, usize, usize, bool), String>
{
	...
	for g in &f.cpu_g
	{
		...
		cpu_obs.push(ReplayObs { vert, horiz: vec![[0.0; 4]; g.len()], pwm_dev: vec![] });
	}
	let (tau, target_min) = (0.05f32, 2usize);
	let trainer = ControllerTrainer::new()?;
	let batch = TrainBatch {
	...
	let const_obs: Vec<ReplayObs> = f
		.cpu_g
		.iter()
		.map(|g| ReplayObs { vert: vec![[0.7, 0.0, 0.0]; g.len()], horiz: vec![[0.0; 4]; g.len()], pwm_dev: vec![] })
		.collect();
	let mut c2 = c.clone();
	let (rows_const, ..) = c2.split_record_pub(
		f.cpu_g.clone(), f.cpu_a.clone(), f.cpu_t.clone(), f.cpu_p.clone(), Some(&const_obs),
...
mod tests
{
		...
		($name:ident, $sweep:path) => {
			...
			fn $name()
			{
				...
				assert!(
					...
					concat!(
						stringify!($sweep),
						" returned no cases — a sweep that ",
	...
	parity_sweep_test!(
		parity_controller_train_seeded,
		run_controller_train_seeded_parity_test
	);
	parity_sweep_test!(
		parity_split_train_loop,
	...
	parity_sweep_test!(
		...
		run_controller_nudge_distance_parity_test
	);
	parity_sweep_test!(
		parity_projected_address,
		run_controller_projected_address_parity_test
	);
	parity_sweep_test!(
		parity_candidate_rank,
		run_controller_candidate_rank_parity_test
	);
	...
	fn coalescer_never_crosses_results_between_threads()
	{
		...
		let jobs: Vec<OwnedLayer> = (0..N)
			.map(|j| {
				...
				OwnedLayer {
					keys: sexp.keys.clone(),
					values: sexp.values.clone(),
					offsets: sexp.offsets.clone(),
					counts: sexp.counts.clone(),
					conns: sconn.to_vec(),
					num_inst: 1,
					neurons_per_inst: n_state,
					n_bits,
					total_input_bits: tib,
		...
		let expect: Vec<SolveOut> = jobs
			.iter()
			.map(|l| {
				solve_coalescer()
					.solve(&t, vec![l.clone()], 4, 2)
					.expect("serial solve")
			})
			.collect();
	...
	fn bench_solve_paths()
	{
		...
		for _ in 0..REPS
		{
			for m in 0..m_cnt
			{
				...
				for nn in 0..lv
				{
					let mut e = f.c.state_entries(m * lv + nn);
					e.sort_unstable();
					o.push(k.len() as u32);
					c.push(e.len() as u32);
					for (a, val) in e
				...
				let _ = t.solve_qsr_reachable(
					&k,
					&v,
					&o,
					&c,
					&sconn[m * lv * n_bits..(m + 1) * lv * n_bits],
					lv,
					n_bits,
					tib,
					&ib,
		...
		for _ in 0..REPS
		{
			...
			let _ = t.solve_qsr_reachable_motors(
				&k, &v, &o, &c, sconn, m_cnt, lv, n_bits, tib, &ib, &tb, 0, 4, 2,
			);
		}
		...
		println!(
			"  GPU per-motor (x{m_cnt})  {per_motor:9.3}   {:.2}x",
			cpu / per_motor
		);
		println!(
			"  GPU batched          {batched:9.3}   {:.2}x   ({:.2}x vs per-motor)\n",
			cpu / batched,
			per_motor / batched
		);
	}
	...
	fn state_layer_slot_table_seeds_and_round_trips()
	{
		...
		assert!(
			planted > 0,
			"fixture planted no state cells — the test would be vacuous"
		);
		...
		assert_eq!(
			t.off.len(),
			n_state,
			"one region per (genome, state neuron)"
		);
		for gn in 0..n_state
		{
			let c = t.cap[gn];
			assert!(
				c.is_power_of_two(),
				"cap {c} must be a power of two — the probe uses cap-1 as a mask"
			);
			assert!(
				t.off[gn] as usize + c as usize <= t.markers.len(),
				"region {gn} overruns the table"
			);
			if gn > 0
			{
				assert!(
					t.off[gn] >= t.off[gn - 1] + t.cap[gn - 1],
					"regions {} and {gn} overlap",
					gn - 1
				);
			}
		}
		...
		for n in 0..n_state
		{
			...
			for i in 0..ec
			{
				...
				assert_eq!(
					t.values[slot] & 0xFF,
					val as u32,
					"state cell (n={n}, addr={addr:#x}) seeded with the wrong value"
				);
				found += 1;
			}
		}
		...
		assert_eq!(
			total, planted,
			"entries() must enumerate exactly the planted cells"
		);
	...
	parity_sweep_test!(
		parity_plant_counter,
		run_controller_plant_counter_parity_test
	);
	...
	parity_sweep_test!(
		parity_record_and_scan,
		run_controller_record_and_scan_parity_test
	);
	parity_sweep_test!(
		parity_record_search,
		run_controller_record_search_parity_test
	);
	parity_sweep_test!(
		parity_resolve_conflict,
		run_controller_resolve_conflict_parity_test
	);
	...
	use crate::controller::{
		compute_reward, monotonicity_violations_core, yaw_from_quat_rs, AttitudeSim,
	};
	...
	{
		let (levels, bpf, window, n_state, sbpn, obpn) =
			(4usize, 3usize, 2usize, 8usize, 8usize, 8usize);
		let num_features = 9usize;
		let frame_bits = num_features * bpf;
		...
		let state_connections: Vec<i64> = (0..n_state * sbpn)
			.map(|_| rng.gen_range(0..state_in) as i64)
			.collect();
		let num_out = num_motors * levels;
		let out_in = frame_bits + n_state;
		let output_connections: Vec<i64> = (0..num_out * obpn)
			.map(|_| rng.gen_range(0..out_in) as i64)
			.collect();
		let mut c = WnnController::new_core(
			...
			0.15,
			0.98,
			1.0, // delta-control off (absolute PWM)
			false,
			false,
			false,
			false,
			false, // H2 obs extras off
			false,
			false,
			false,
			0.99,
			...
			SIM_DT,
			false,
			1, // decouple off, action_repeat 1
			memory_mode,
			None, // output_decode: mode default (anchor)
			None,
			0.05,
			false,
			0.30, // dhat_b/ff: observer OFF (anchor)
			false,
			false,
			false, // stage-1 vertical channel OFF
			false,
		...
		if plant
		{
			let cell_hi = if crate::cell_mode::is_quad(memory_mode)
			{
				4u8
			}
			else
	...
	fn test_episodes(seed: u64, n: usize) -> (Vec<f32>, Vec<f32>)
	{
		...
		for _ in 0..n
		{
			...
			let norm = (ax[0] * ax[0] + ax[1] * ax[1] + ax[2] * ax[2])
				.sqrt()
				.max(1e-6);
			...
			for _ in 0..3
			{
				w0.push(rng.gen_range(-1.0f32..1.0));
			}
		}
		(q0, w0)
	}
	...
	) -> [f64; 5]
	{
		...
		for ep in 0..num_eps
		{
			...
			sum_err += mean_err;
			sum_jerk += if ep_jerk_count > 0
			{
				ep_jerk / ep_jerk_count as f64
			}
			else
			{
				0.0
			...
			sum_mono += mono_last;
			if !diverged && mean_err <= stable_thresh
			{
				n_stable += 1;
			}
		}
	...
	fn bl_pidfw_cfg(decimation: u32) -> PidFwCfg
	{
		...
		let lpf = bl_pidfw_rs(decimation)
			.rate_lpf_coeffs()
			.expect("filter on");
		PidFwCfg {
			att: [
				6.0, 3.0, 0.0, 0.34906585, 6.0, 3.0, 0.0, 0.34906585, 6.0, 1.0, 0.35, 6.28318531,
			],
			rate: [
				0.0349711022,
				0.0699422043,
	...
	fn gpu_pidfw_cascade_matches_cpu_twin()
	{
		...
		for decimation in [2u32, 10u32]
		{
			...
			let gpu = ev
				.score(
					...
					num_eps,
					steps,
					(SIM_DT, BL_ARM, BL_KT, BL_INERTIA, SIM_G),
					BL_KD,
					0.0,
					[0.0, 0.0, 0.0],
					None,
			...
			// larger.
			for (i, name) in ["reward", "err", "stable", "jerk", "mono"]
				.iter()
				.enumerate()
			{
				assert_rel_close(
			...
			assert!(
				gpu[0][1] > 1e-4,
				"vacuous rollout at dec={decimation} (err={})",
				gpu[0][1]
			);
		}
		...
		assert_eq!(
			gpu[0][2], 1.0,
			"cascade failed to stabilize (stable={})",
			gpu[0][2]
		);
	}
	...
	fn assert_rel_close(gpu: f64, cpu: f64, rel: f64, abs_floor: f64, what: &str)
	{
		...
		assert!(
			(gpu - cpu).abs() <= tol,
			"{what}: gpu={gpu} cpu={cpu} (|Δ|={} > tol={tol})",
			(gpu - cpu).abs()
		);
	}
	...
	{
		let tilt = [0.8f32, -1.2, 0.5, -0.3, 1.0, -0.7, 0.2, -0.9].map(|d: f32| d.to_radians());
	...
	fn rotor_table_builder_validates_and_bakes()
	{
		...
		assert!(
			build_rotor_table(&rows, Some(&[1.0, 1.0])).is_err(),
			"asym len mismatch"
		);
		...
		assert!(
			(t[0].k_thrust - SIM_KT * 0.9).abs() < 1e-6,
			"asym must bake: {}",
			t[0].k_thrust
		);
	}
	...
	fn gpu_octo_geometry_matches_cpu_step_n_hover()
	{
		...
		let rows_gpu = ev
			.score(
				...
				num_eps,
				steps,
				(SIM_DT, SIM_ARM, SIM_KT, SIM_INERTIA, SIM_G),
				SIM_KD,
				0.0,
				[0.0, 0.0, 0.0],
				None,
		...
		assert!(
			oracle[1] > 1e-3,
			"hover rollout has no attitude error (err={})",
			oracle[1]
		);
	}
	...
	fn gpu_octo_geometry_matches_cpu_step_n_closed_loop()
	{
		...
		assert!(
			oracle[3] > 1e-4,
			"closed-loop rollout is trivially constant (jerk={})",
			oracle[3]
		);
		assert!(
			oracle[1] > 1e-3,
			"closed-loop rollout has no attitude error (err={})",
			oracle[1]
		);
		...
		assert!(
			cpu_row[5] > 1e-4,
			"steady vacuous (regressed to hardcoded 0?): {}",
			cpu_row[5]
		);
	...
	fn gpu_dhat_feature_matches_cpu_closed_loop()
	{
		...
		assert!(
			oracle[3] > 1e-4,
			"trivially constant rollout (jerk={})",
			oracle[3]
		);
		...
		assert_eq!(
			c_off.obs_params().0,
			9,
			"control arm must be the 9-feature anchor"
		);
		let off = cpu_oracle_quad(&mut c_off, &q0, &w0, num_eps, steps);
		assert!(
			(off[1] - oracle[1]).abs() > 1e-6,
	...
	fn parity_motor_lag_quad()
	{
		...
		assert!(
			(gpu_lag[0][1] - gpu_off[0][1]).abs() > 1e-6,
	...
	fn parity_stage1_vertical_channel()
	{
		...
		let s1 = crate::stage1::Stage1Cfg {
			...
			mass: (0..num_eps)
				.map(|e| 0.0393 * (0.9 + 0.04 * e as f32))
				.collect(),
		...
		let mut s1_noalt = crate::stage1::Stage1Cfg {
			target_altitude: s1.target_altitude,
			lambda_alt: 0.0,
			init_z: s1.init_z.clone(),
			init_vz: s1.init_vz.clone(),
			mass: s1.mass.clone(),
			collective_frac: s1.collective_frac.clone(),
			lambda_pos: 0.0,
			init_x: vec![],
			init_y: vec![],
		};
		...
		assert!(
			(gpu_on[0][0] - gpu_noalt[0][0]).abs() > 1e-6,
		...
		assert!(
			(gpu_on[0][0] - gpu_heavy[0][0]).abs() > 1e-6,
	...
	fn parity_stage2_horizontal_channel()
	{
		...
		assert_rel_close(
			...
			5e-2,
			1e-3,
			"stage-2 alt_err_m parity",
		);
		assert!(
			cpu[14] > 0.0,
			"the altitude-error metric is zero — it is not wired"
		);
		assert!(
			cpu[14] < cpu[13],
			"alt_err ({}) must be strictly below the 3-D pos_err ({}) with x/y \
			 displaced — the row is carrying the wrong quantity",
			cpu[14],
			cpu[13]
		);
		...
		let mut s2_nopos = crate::stage1::Stage1Cfg {
			target_altitude: s2.target_altitude,
			lambda_alt: s2.lambda_alt,
			init_z: s2.init_z.clone(),
			init_vz: s2.init_vz.clone(),
			mass: s2.mass.clone(),
			collective_frac: s2.collective_frac.clone(),
			lambda_pos: 0.0,
			init_x: s2.init_x.clone(),
			init_y: s2.init_y.clone(),
		};
		let gpu_nopos = ev
			.score(
				&[&c],
		...
		assert!(
			(gpu_on[0][0] - gpu_nopos[0][0]).abs() > 1e-6,
			"lambda_pos=2 scored the same reward as lambda_pos=0 ({} vs {}) — the \
			 shader is ignoring the stage-2 reward term",
			gpu_on[0][0],
			gpu_nopos[0][0]
		);
	...
	fn parity_frame_stride()
	{
		...
		let sbpn = 10usize;
		let state_conns: Vec<i64> = (0..n_state * sbpn)
			.map(|_| rng.gen_range(0..sensor_total) as i64)
			.collect();
		let output_connections: Vec<i64> = (0..num_out * obpn)
			.map(|_| rng.gen_range(0..(frame_bits + n_state)) as i64)
			.collect();
		...
		let bit = |n: usize, a: u64| -> u8 {
			let h =
				(n as u64).wrapping_mul(0x9E37_79B9_7F4A_7C15) ^ a.wrapping_mul(0xD1B5_4A32_D192_ED03);
			((h >> 33) & 1) as u8
		};
		let fill = |c: &mut WnnController| {
			for n in 0..num_out
			{
				for a in 0u64..(1u64 << obpn)
				{
					let _ = c.write_output_cell_internal(n, a, bit(n, a));
				}
			}
			for n in 0..n_state
			{
				for a in 0u64..(1u64 << sbpn)
				{
					let _ = c.write_state_cell_internal(n, a, bit(n + 1000, a));
				}
			}
		};
		...
		assert!(
			(cpu10[0] - cpu1[0]).abs() > 1e-9 || (cpu10[1] - cpu1[1]).abs() > 1e-9,
	...
	fn parity_target_levels()
	{
		...
		for n in 0..num_out
		{
			...
			for a in all
			{
				if *gpu_map.get(&a).unwrap_or(&2u8) != c.output_cell(n, a)
				{
					mismatches += 1;
				}
			}
		...
		'outer: for n in 0..num_out
		{
			...
			for &(a, _) in &c2.output_entries(n)
			{
				all.insert(a);
			}
			for a in all
			{
				if c.output_cell(n, a) != c2.output_cell(n, a)
				{
					differs = true;
					break 'outer;
				}
		...
		assert!(
			differs,
			"T=4 trained cells identical to legacy T=8 — the mapping never reached a trainer"
		);
	}
	...
	fn arm_d_trainers_use_full_window()
	{
		...
		let output_conns: Vec<i64> = (0..num_out * obpn)
			.map(|_| (xs(&mut rng) % sensor_total as u64) as i64)
			.collect();
		assert!(
			output_conns.iter().any(|&c| (c as usize) >= frame_bits),
		...
		assert_eq!(
			...
			"arm D output input must be the whole {window}-frame window ({sensor_total} bits), \
			 got {} — the trainer is still sizing it frame-only",
			out_ins[0].len()
		);
	...
	fn parity_output_full_window()
	{
		...
		assert!(
			output_connections
				.iter()
				.any(|&c| (c as usize) < (window - 1) * frame_bits),
			"fixture must include historical-frame indices"
		);
		let build = |conns: Vec<i64>, full: bool| {
			WnnController::new_core(
				num_motors,
				levels,
				bpf,
		...
		let writes: Vec<(usize, u64)> = (0..num_out * 6)
			.map(|_| {
				(
					wrng.gen_range(0..num_out),
					wrng.gen::<u64>() & ((1 << obpn) - 1),
				)
			})
			.collect();
		for &(n, a) in &writes
		{
			let _ = c.write_output_cell_internal(n, a, 1);
		}
		...
		let folded: Vec<i64> = output_connections
			.iter()
			.map(|&cn| ((window - 1) * frame_bits + (cn as usize % frame_bits)) as i64)
			.collect();
		let mut cf = build(folded, true);
		for &(n, a) in &writes
		{
			let _ = cf.write_output_cell_internal(n, a, 1);
		}
		...
		assert!(
			(cpu[0] - cpu_folded[0]).abs() > 1e-9 || (cpu[1] - cpu_folded[1]).abs() > 1e-9,
	...
	fn test_controller_stage2(seed: u64) -> WnnController
	{
		...
		WnnController::new_core(
			...
			true,
			true,
			true, // stage-1 vertical channel ON
			true,
			true,  // stage-2 horizontal channel ON
	...
	fn test_controller_dhat(seed: u64, dhat_b: Option<[f64; 3]>, ff: bool) -> WnnController
	{
		...
		for (n, a, v) in state_cells
		{
			let _ = c.write_state_cell_internal(n, a, v);
		}
		for (n, a, v) in out_cells
		{
			let _ = c.write_output_cell_internal(n, a, v);
		}
		c
	}
	...
	fn gpu_alloc_residual_zero_equals_teacher()
	{
		...
		assert_eq!(
			gpu[0][2], 1.0,
			"alloc-LQR baseline failed to stabilize (stable={})",
			gpu[0][2]
		);
	...
	fn gpu_alloc_residual_matches_cpu_scorer()
	{
			...
			.enumerate()
		{
			assert_rel_close(
				...
				2e-2,
				2e-3,
				&format!("alloc-residual {name}"),
			);
		}
		assert!(
			cpu[3] > 1e-4,
			"residual never moved the PWM (jerk={})",
			cpu[3]
		);
	}
	...
	fn geometry_guards_fail_loudly()
	{
		...
		assert!(
			...
			.is_err(),
			"rotor/motor mismatch must be refused"
		);
		...
		let residual = ResidualCfg {
			scale: 1.0,
			clamp: 0.4,
			pid: [1.2, 0.0, 0.3, 0.5, 0.6, 0.0, 0.2, 0.5, 0.5, 0.4],
			cascade: None,
		};
		assert!(
			ev.score(
			...
			.is_err(),
			"residual + N≠4 geometry must be refused"
		);
	}
}
...
pub(crate) fn gpu_solver() -> Option<&'static ControllerTrainer>
{
	GPU_SOLVER
		.get_or_init(|| {
			if std::env::var("WNN_CONTROLLER_GPU_SOLVE")
				.map(|s| s == "1")
				.unwrap_or(false)
			{
				match ControllerTrainer::new()
				{
					...
					Err(e) =>
					{
						eprintln!("[controller] GPU solve requested but unavailable: {e}");
						None
					}
				}
			}
...
impl SolveCoalescer
{
	...
	) -> Result<SolveOut, String>
	{
		...
		let lead = {
			let mut g = self
				.m
				.lock()
				.map_err(|e| format!("coalescer poisoned: {e}"))?;
			g.queue.push(Job {
				layers,
				topk,
				mode,
		...
		rx.recv()
			.map_err(|e| format!("coalescer sender dropped before delivering: {e}"))
	}
	...
	fn dispatch_batch(t: &ControllerTrainer, batch: Vec<Job>)
	{
		...
		for j in batch
		{
			groups.entry((j.topk, j.mode)).or_default().push(j);
		}
			...
			for j in &jobs
			{
				for l in &j.layers
				{
					flat.push(SolveLayer {
						...
						conns: &l.conns,
						num_inst: l.num_inst,
						neurons_per_inst: l.neurons_per_inst,
						n_bits: l.n_bits,
						total_input_bits: l.total_input_bits,
