...
pub fn run_microbench(
	...
	results.push(run_gpu_only_increment(
		&device, &command_queue, &inc_pipeline,
		num_slots, gpu_threads, iterations,
	)?);
	...
	results.push(run_claim_contention(
		&device, &command_queue, &claim_pipeline,
		num_slots, claim_threads,
	)?);
...
fn run_gpu_only_increment(
	...
	unsafe { std::ptr::write_bytes(ptr, 0, num_slots); }

	// Params buffers
	let n_slots_u32 = num_slots as u32;
	let iters_u32 = iterations as u32;
	let n_buf = device.new_buffer_with_data(
		&n_slots_u32 as *const _ as *const _, 4, MTLResourceOptions::StorageModeShared,
	);
	let i_buf = device.new_buffer_with_data(
		&iters_u32 as *const _ as *const _, 4, MTLResourceOptions::StorageModeShared,
	);
...
fn run_concurrent_increment(
	...
	let atomic_slice: &[AtomicU64] = unsafe {
		std::slice::from_raw_parts(ptr as *const AtomicU64, num_slots)
	};
	...
	Ok(MicrobenchResult {
		...
		details: format!(
			...
			gpu_threads, cpu_threads, iterations, expected, total,
...
fn run_claim_contention(
	...
	unsafe { std::ptr::write_bytes(success_ptr, 0, claim_threads); }
