...
impl MetalSparseEvaluator
{
	...
	pub fn new() -> Result<Self, String>
	{
		...
		let sparse_forward_per_example_pipeline = device
			...
			.map_err(|e| {
				format!(
					"Failed to create sparse forward per-example pipeline: {}",
					e
				)
			})?;
	...
	) -> Result<Vec<f32>, String>
	{
		...
		let cluster_info_structs: Vec<ClusterInfo> = cluster_infos
			.iter()
			.map(|&(n, b, s, c)| ClusterInfo {
...
mod tests
{
	...
	fn test_sparse_evaluator_creation()
	{
		if metal::Device::system_default().is_some()
		{
			...
			assert!(
				evaluator.is_ok(),
				"Failed to create sparse evaluator: {:?}",
				evaluator.err()
			);
		}
	}
