...
class ArithmeticOp(IntEnum):
    """Arithmetic operations for computed FFN."""
    INCREMENT = 0      # Add 1
    DECREMENT = 1      # Subtract 1
    ADD = 2            # Add constant
    SUBTRACT = 3       # Subtract constant
    ADD_MOD = 4        # Add constant with modulo
    SUBTRACT_MOD = 5   # Subtract constant with modulo
    ROT13 = 6          # ROT13 cipher (add 13 mod 26)
    NEGATE = 7         # Bitwise NOT
...
class ComputedArithmeticFFN(Module):
    ...
    def __init__(
        ...
        self.modulo = modulo
        self.max_value = (1 << input_bits) - 1  # Max representable value
    ...
    def _apply_operation(self, value: int) -> int:
        """Apply the arithmetic operation to a value."""
        if self.operation == ArithmeticOp.INCREMENT:
            result = value + 1
        elif self.operation == ArithmeticOp.DECREMENT:
            result = value - 1
        elif self.operation == ArithmeticOp.ADD:
            result = value + self.constant
        elif self.operation == ArithmeticOp.SUBTRACT:
            result = value - self.constant
        elif self.operation == ArithmeticOp.ADD_MOD:
            result = (value + self.constant) % self.modulo
        elif self.operation == ArithmeticOp.SUBTRACT_MOD:
            result = (value - self.constant) % self.modulo
        elif self.operation == ArithmeticOp.ROT13:
            # ROT13: add 13 mod 26 (for alphabet encoding where A=0, Z=25)
            result = (value + 13) % 26
        elif self.operation == ArithmeticOp.NEGATE:
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = x.squeeze()
        value = _bits_to_int(x)
        result = self._apply_operation(value)
        return _int_to_bits(result, self.input_bits)
