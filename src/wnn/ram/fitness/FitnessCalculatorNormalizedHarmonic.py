...
Normalizes CE and accuracy to [0, 1], combines using weighted harmonic mean.
Penalizes imbalance more strongly than arithmetic mean.
"""
...
class FitnessCalculatorNormalizedHarmonic(FitnessCalculator):
	"""
	Fitness = weighted harmonic mean of normalized CE and accuracy.

	HM = (w_ce + w_acc) / (w_ce / norm_ce + w_acc / norm_acc)
	...
	def fitness(self, metrics_list: list[Metrics]) -> list[float]:
		...
		return [
			w_total / (self.weight_ce / (norm_ce[i] + eps) + self.weight_acc / (norm_acc[i] + eps))
			for i in range(n)
		]
