...
<!-- Added 20/06/2026. WILL BE WIPED if this file is regenerated by
     scripts/build_xds_5tables.py — re-add after any regen. -->

## ⭐ CURRENT 96b BEST-FPR (20/06/2026) — e9639 beats the paper best-FPR row

**e9639** = experiment 9639 / flow 4173 = `XDS-cicids-random-96b-Wa-C35-500n34b-OI-r94345`
(best_fpr genome × `empirical_cumulative` threshold):

| | F1 (%) | FPR (%) | Acc (%) | encoding |
|---|---|---|---|---|
| **e9639** (96b-Wa, emp-cumul) | **99.37** | **0.053** | **99.61** | 96-bit |
| Paper Best-FPR row (`tab:cicids`) | 98.58 | 0.05 | 99.12 | 16-bit |
| **Δ (e9639 − paper)** | **+0.79** | +0.003 | **+0.49** | — |

At matched FPR (both 0.05% at display precision) e9639 wins **+0.79pp F1** and **+0.49pp Acc**.
Caveats: (1) FPR is 0.053 vs the paper's 0.050 — tied at 2-dp, microscopically higher strictly,
so it's "matched-FPR + better F1/Acc", not a strict ≤-FPR domination; (2) different config — paper
best-FPR is the **16b** cohort (FPR-weighted fitness), e9639 is **96b-Wa** (CE/acc-heavy, FPR from
threshold calibration). Lines up with the paper footnote that the best-FPR row will be re-measured
at the width-sweep winner. **Promote to the paper table IF 96b wins the width sweep.** Single genome
(one seed of the 96b-Wa n=5 tail still completing).
<!-- =========================================================================== -->
