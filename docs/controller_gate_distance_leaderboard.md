...
```
markers with a headline held-out : 209
  altitude regimen (alt= present) : 120
  attitude-only                   : 89
  state-neuron count unreadable   : 0  (no .out on disk)
  h743 keys counted exactly       : 19  (experiments/h743_keys.json)
  h743 fits, exact / bound        : 14 / 47
```
