...
```
markers with a headline held-out : 250
  altitude regimen (alt= present) : 166
  attitude-only                   : 84
  state-neuron count unreadable   : 0  (no .out on disk)
  h743 keys counted exactly       : 56  (experiments/h743_keys.json)
  h743 fits, exact / bound        : 33 / 50
