...
| 1 | Generalize AttitudeSim step to RotorGeometry (feature-flagged; quad preset must stay bit-identical — parity test) + Metal shader N-rotor port | controller crate + shaders | DISCUSS FIRST (live hot path) — CPU ✅ (step_n, cd6deec5) · Metal step 1 ✅ (function-constant kernel, 2af70830) · Metal step 2 ✅ (specialized pipeline cache + RotorGpu upload + `score_controllers_metal(geometry=, rotor_asym=)` + CPU/GPU parity suite; wheel NOT installed until seed-pairs drain) |
