...
| 2 | Residual injection point + teacher (allocator-aware LQR) + DAGGER plumbing | controller crate + wnn/control | after Phase 1 parity — step 1 ✅ (`AllocLqrRs` teacher: torque-space closed-form CARE + damped-pinv allocation, τ_max 0.144 N·m quad-equivalent, f_hover = Σk·0.25; 5 tests incl. nominal-allocator-vs-perturbed-vehicle bounded stabilization; ABI 5 installed). REMAINING: step 2 = N-motor residual composition (CPU compose + in-kernel allocator-LQR baseline via precomputed B⁺ matvec) · step 3 = N-motor DAGGER/record/train plumbing (kernels are quad-hardcoded) · step 4 = phased_ga geometry CLI |
