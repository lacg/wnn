...
BOTH directions, at the evaluator's OWN packing site:
  ...
  (d) AXIS F merge: cf21 + motor_lag_s=0.15 packs BOTH af_k_thrust 0.2 and
      motor_lag_s 0.15; lag 0 passes no kwarg; both ctor sites splat
      episode_config.motor_lag_kwargs().
...
def test_cf21_with_motor_lag_packs_both() -> None:
	# AXIS F merged with the airframe fix (19/09/2026): the plant kwargs and the
	# motor lag ride the SAME ctor call — neither may shadow the other. The Rust
	# field is f32, hence the tolerance.
	af = Airframe.preset("cf21_brushless")
	cfg, kw = capture_trainer_cfg(EpisodeConfig(airframe=af, motor_lag_s=0.15))
	check("cf21 + lag: af_k_thrust still cf21's 0.2", cfg.af_k_thrust, 0.2, tol=1e-6)
	check("cf21 + lag: motor_lag_s packs 0.15", cfg.motor_lag_s, 0.15, tol=1e-6)
	check("cf21 + lag: motor_lag_s is an explicit kwarg", "motor_lag_s" in kw, True)
	cfg0, kw0 = capture_trainer_cfg(EpisodeConfig(airframe=af))
	check("cf21, lag 0: no motor_lag_s kwarg (bit-identical call)", "motor_lag_s" in kw0, False)
	check("cf21, lag 0: packed motor_lag_s is the Rust default 0.0", cfg0.motor_lag_s, 0.0, tol=0.0)
