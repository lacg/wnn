#!/usr/bin/env python
"""Reproduce the BINARY grid winner (sn=16 b=30, ~90% during-search stable) and run
the HONEST held-out on it — NOTHING ELSE. No GA-Neurons, no GA-Memory.

Mirror of scripts/ternary_grid_winner_holdout.py for the BINARY arm (mode 3, antagonist
E/I motor decode). The BINARY NEURONS stage is serial-by-OOM-budget (warm BINARY cells
~7GB/genome blow past _eval_batch_size's 10GB budget → batch=1 → serial → hours/days), so
BINARY gets the same grid-winner-held-out treatment TERNARY got instead of the full GA.

The grid is deterministic given seeds/args, so re-running ONLY stage-0 grid with the
identical CLI of c10_gran_binary_20260715_binqsr_v3_s2000p50 regenerates the winner
(sn=16 b=30, err 2.87°/stable 90% during-search → GRID WINNER).
...
(K=1, undertrained) — that diverges (an artifact, not a real result). Production never
hits that path because real held-out winners (MEMORY stage) carry cells → score-only. We
replicate the score-only semantics here.
"""
import math
import time
...
# EXACT CLI of the binqsr_v3 BINARY arm (run_gran_5arm_capped.sh, STEPS=2000 POP=50).
ARGV = [
...
SAVE = "/Users/lacg/wnn/logs/controller/binary_grid_winner.yaml.gz"  # canonical (_ctl_load / --seed-winner)
...
def main():
	...
	print(f"\n{'#'*72}\n# BINARY grid-winner HONEST held-out (grid-only; NO GA)\n{'#'*72}")
	t0 = time.time()

	# STAGE 0 — deterministic grid → regenerates the sn=16 b=30 winner.
	...
	# lost and is loadable by _ctl_load / --seed-winner. Seeds/thresholds are NOT stored
	# (deterministic from --base-seed; thresholds are PID-fit + arch-independent).
	_save_winner(SAVE, args, winner_spec, winner, seed_population, m0)
	sn = getattr(winner_spec, 'state_neurons', '?')
	b = getattr(winner_spec, 'state_bits', '?')
	print(f"\n[grid done {dt0:.0f}s] winner sn={sn} b={b} during-search: "
	...
	print(f"\n{bar}\n  HONEST HELD-OUT [GRID-WINNER sn={sn} b={b} BINARY] — train seed {s.train} "
