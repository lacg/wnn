#!/bin/bash
# Yaw-anchor + DECOUPLED OUTPUTS (H3) test. The connectivity diag showed the coupled anchor
# over-wires the OUTPUT layer to yaw_err (2.14x) and drives the SHARED motors → yaw action
# bleeds into roll/pitch → the held-out failures are roll/pitch blow-ups. --decouple-outputs
# gives orthogonal control banks [T, τ_roll, τ_pitch, τ_yaw] so yaw_err can dominate the YAW
# torque bank ONLY and physically can't perturb roll/pitch. S16 recipe (.25/.35/.20/.15/.05)
# + --obs-yaw-err + --decouple-outputs, folds=3 — A/B vs coupled anchor (70.5%) and S16 (87.2%).
# Args: SEED. Writes YAWDEC_seed{SEED}/.
set -u
cd /Users/lacg/wnn
SEED="${1:-20260609}"
...
DIR=logs/controller/YawDecouple_20260626/YAWDEC_seed${SEED}; mkdir -p "$DIR"
echo "[yaw-dec] $(date '+%Y-%m-%d %H:%M:%S') START obs_yaw_err + decouple-outputs seed=$SEED"
$PY -u -m wnn.control.phased_ga \
  --grid-state-neurons 8 12 16 --grid-bits 24 30 --levels 16 --bits-per-feature 8 \
  --no-delta-control --decouple-outputs \
...
echo "{\"yaw_dec_done\":true,\"seed\":$SEED,\"ts\":\"$(date '+%Y-%m-%dT%H:%M:%S')\"}" > /tmp/wnn_yaw_dec_seed${SEED}_done.json
echo "[yaw-dec] $(date '+%Y-%m-%d %H:%M:%S') COMPLETE seed=$SEED"
