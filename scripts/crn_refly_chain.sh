...
#
# THE QUESTION. The CRN fitness landed mid bits-round-2 (03/09 21:05 EDT), so
# every CRN-vs-rotation pair banked so far ALSO differs by seed (b24: 0.1972 at
# s31337002 rotation vs 0.1271 at s31337003 CRN). This flies the SAME shape at
# the SAME seed with only the scorer changed: b=24 n=256 gamma=1 seed 31337002,
# tagged ..._s31337002_crn so the rotation-era marker is untouched.
#
...
TAG="SL_C_b${BITS}n${NEURONS}_${AIRFRAME}_${DIST}_g10_s${SEED}"
MARKER="${LADDERMARK}/${TAG}_crn.json"
CONTROL="${LADDERMARK}/${TAG}.json"
...
log "########## ARMED — CRN re-fly of ${TAG} (control: rotation-era marker $(basename "$CONTROL")) ##########"
[ -f "$CONTROL" ] || { log "ABORT — control marker $CONTROL missing; nothing to pair against."; exit 1; }
[ -f "$MARKER" ] && { log "marker $MARKER already exists — nothing to do."; exit 0; }
...
log "===== LAUNCHING ${TAG}_crn via $CHAIN ====="
...
if [ ! -f "$MARKER" ]; then
	log "ABORT — ladder exited but $MARKER is MISSING (watchdog kill / crash / no MEMORY triple). A run needs a human."
	exit 1
fi
log "marker banked: $MARKER"
...
grep -E "${TAG}(_crn)?$" docs/controller_gate_distance_leaderboard.md | while IFS= read -r row; do log "  $row"; done
log "########## DONE ##########"
