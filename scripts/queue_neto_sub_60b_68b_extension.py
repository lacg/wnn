"""Add 60b and 68b × 2 seeds each to the in-progress neto-subsample bit sweep.

64b r1812 hit 91.09% F1 / 12.28% FPR / 95.59% Acc (best across the sweep
so far), and 72b r201 lands within seed-noise of it (91.01%). Need points
ON BOTH SIDES of 64b to confirm whether 60b-72b is a plateau or whether
64b is a local peak.

Total: 4 flows. Cost: ~6h wall (each ~90min). Queued with status=queued
so they cut in front of the still-pending 24b/48b/4b flows (higher ids
win under ORDER BY id DESC).
...
SEEDS = [201, 202]
BIT_WIDTHS = [60, 68]  # interpolate between 32 (done) and 64/72 (done)

# Clone r1812 — the best-performing 64b flow we have.
...
for n_bits in BIT_WIDTHS:
	for seed in SEEDS:
		...
		body = {
			"name": name,
			"description": (
				f"Bit-width interpolation between 32b/64b/72b on neto-subsample "
				f"(1.43M, 46f). ids_n_bits={n_bits}, seed={seed}. Confirms whether "
				f"60-72b is a plateau or 64b is a local peak."
			),
...
# Flip pending → queued so the worker picks them up.
print()
