...
LOG="/private/tmp/calib_levels_sweep.log"
VP="/Volumes/20260401-WDBlack-SN850X-2TB/wnn/venv/bin/python"
OUTDIR="logs/controller/calib_sweep"
MARKDIR="experiments/calib_sweep_markers"
...
log() { echo "[calib] $(date -u +%FT%TZ) $*" >> "$LOG"; }
...
for cell in $CELLS; do
	...
	run_controller_arm "$tag" \
		...
		"\"arm\":\"CALIBxLEVELS\",\"teacher\":\"${TEACHER}\",\"calib_tilt\":${calib},\"levels\":${levels},\"airframe\":\"${AIRFRAME}\",\"disturbance\":\"${DIST}\",\"obs_dhat\":false,\"mode\":\"BINARY\",\"state_neurons\":0,\"neurons_gens\":${NEURONS_GENS},\"seed\":${seed},\"code\":\"post-4514d5c9\"" \
		-- \
		--levels "$levels" --threshold-calib-tilt "$calib" \
...
log "NEXT: per (calib, levels), compare HEADLINE steady vs the SAME-SEED calib-30 control (L16: CMT_lqi 0.53/0.81 · L64: ALP 0.31/0.91). Bar: beat BOTH seeds without losing stable. Then read the 2x2: does perception alone move it, and does it COUPLE with the finer action alphabet?"
