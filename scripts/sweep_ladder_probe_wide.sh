...
LOG="/private/tmp/sweep_ladder_probe_wide.log"
VP="/Volumes/20260401-WDBlack-SN850X-2TB/wnn/venv/bin/python"
OUTDIR="logs/controller/sweep_ladder"
MARKDIR="experiments/sweepladder_markers"
AIRFRAME="cf21_brushless"
DIST="L4C"
SEEDS="${SL_SEEDS:-31337002}"   # probe is seed-1 only; no cull, no seed-2 round
BITS="${SL_BITS:-40 48 64}"   # WIDE PROBE: sparse points above the ladder
...
# ===== WIDE PROBE: sparse widths above the ladder, seed 1 only, both arms =====
# No cull, no later-seed round: this probe exists ONLY to answer whether the
# gate-distance curve keeps falling past b=36 or turns. Read it against the
# ladder's own b=32/34/36 points, which were flown identically.
#
# b=64 is the SUBSTRATE CEILING: controller cells are keyed by u64
# (compute_address_sparse -> u64), so 1<<bits stops fitting above 64.
# Smoke b=64 before trusting the full probe (see docs / the arming note).
log "########## ARMED — WIDE PROBE bits=[$BITS] seed=[$SEEDS] arms=[gate|desir] ##########"
for seed in $SEEDS; do
	for b in $BITS; do
		run_pair "$seed" "$b"
	done
done
log "########## WIDE PROBE COMPLETE — read against the ladder's b=32/34/36 on gate-distance ##########"
