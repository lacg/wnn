"""Low-FPR weight sweep on 96b — 4 variants × 8 seeds = 32 flows, alternating.

Hypothesis tested: high FPR weight in fitness pulls GA toward the
best_fpr-pathology zone (90% of best_fpr genomes are #5/5 worst test FPR).
Demoting FPR weight should let best_fitness converge toward the
low-FPR zone occupied by best_ce / best_acc / best_f1 (each ~30% win rate).

Variants (all sum to 1.00):
  F40  (0.30/0.20/0.40/0.10)  - F1-leading, FPR de-prioritized
  C35  (0.35/0.30/0.30/0.05)  - CE-leading, FPR ≈ silenced (most extreme)
  DDR  (0.30/0.30/0.25/0.15)  - data-driven (mirrors win-rate ordering)
  TRI  (0.30/0.30/0.30/0.10)  - tri-balanced 0.30s, FPR low

Alternation: by 4 flows in, n=1 of each; by 8 flows, n=2; etc.
DDR has highest priority within each seed (runs first).

NOTE: these will queue with HIGHER ids than remaining F1H/CE40 flows,
so the worker will run them FIRST. F1H/CE40 will resume after the new 32.

Per CLAUDE.md Rule 2: dashboard POST /api/flows.
"""

import json
import sqlite3
import sys
import time
from pathlib import Path

import requests
import urllib3

urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

DASHBOARD = "https://localhost:3000"
DB_PATH = Path(__file__).resolve().parents[1] / "db" / "wnn.db"

# (label, ce, acc, f1, fpr) — ordered for alternation; DDR last so it
# gets highest id per seed and runs first.
VARIANTS = [
	("TRI",  0.30, 0.30, 0.30, 0.10),
	("F40",  0.30, 0.20, 0.40, 0.10),
	("C35",  0.35, 0.30, 0.30, 0.05),
	("DDR",  0.30, 0.30, 0.25, 0.15),
]
SEEDS = list(range(201, 209))  # 8 seeds matching prior sweep

con = sqlite3.connect(str(DB_PATH))
row = con.execute("SELECT config_json FROM flows WHERE id = 1812").fetchone()
con.close()
if not row:
	print("ERROR: r1812 base config not found.")
	sys.exit(1)
base_params = dict(json.loads(row[0])["params"])

experiments = [
	{"name": "Grid Search (neurons x bits)", "experiment_type": "grid_search", "phase_type": "grid_search"},
	{"name": "GA Neurons", "experiment_type": "ga", "phase_type": "ga_neurons"},
]

# Create order: low id first. For each seed iterate VARIANTS in declared order
# (TRI → F40 → C35 → DDR), so DDR gets highest id and runs first within each seed.
create_order = []
for seed in SEEDS:
	for variant in VARIANTS:
		create_order.append((variant, seed))

created = []
for (label, ce, acc, f1, fpr), seed in create_order:
	# Verify weights sum to 1
	if abs((ce + acc + f1 + fpr) - 1.0) > 1e-9:
		print(f"ERROR: weights for {label} sum to {ce+acc+f1+fpr}, not 1.0")
		sys.exit(3)
	params = dict(base_params)
	params["ids_n_bits"] = 96
	params["seed"] = seed
	params["fitness_weight_ce"] = ce
	params["fitness_weight_acc"] = acc
	params["fitness_weight_f1"] = f1
	params["fitness_weight_fpr"] = fpr
	name = f"WSWEEP-96b-{label}-r{seed}"
	body = {
		"name": name,
		"description": (
			f"Low-FPR weight sweep. variant={label} "
			f"(ce={ce}, acc={acc}, f1={f1}, fpr={fpr}), seed={seed}."
		),
		"config": {"template": "ids-binary-2-phase", "params": params},
		"experiments": experiments,
	}
	print(f"Creating: {name}...")
	resp = requests.post(f"{DASHBOARD}/api/flows", json=body, verify=False, timeout=30)
	if resp.status_code in (200, 201):
		fid = resp.json()["id"]
		created.append((fid, label, seed, name))
		print(f"  ✓ id={fid}")
		time.sleep(1)
	else:
		print(f"  ✗ Failed ({resp.status_code}): {resp.text[:200]}")
		sys.exit(2)

print()
print("Flipping pending → queued...")
for fid, label, seed, name in created:
	requests.post(f"{DASHBOARD}/api/flows/{fid}/restart", json={}, verify=False, timeout=15)
	time.sleep(0.8)

print()
print("Final state (id-DESC = run order; first 12 only):")
con = sqlite3.connect(str(DB_PATH))
for fid, label, seed, name in sorted(created, key=lambda x: -x[0])[:12]:
	r = con.execute("SELECT id, name, status FROM flows WHERE id = ?", (fid,)).fetchone()
	exps = con.execute("SELECT COUNT(*) FROM experiments WHERE flow_id = ?", (fid,)).fetchone()[0]
	flag = "✓" if exps == 2 and r[2] in ("queued", "running") else "✗"
	print(f"  {flag} id={r[0]}  status={r[2]:<10}  experiments={exps}  {r[1]}")
con.close()
