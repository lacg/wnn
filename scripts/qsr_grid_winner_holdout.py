#!/usr/bin/env python
"""Reproduce the QSR grid winner and run the HONEST held-out on it — NOTHING ELSE.
No GA-Neurons, no GA-Memory.

Mirror of scripts/binary_grid_winner_holdout.py for the QSR arm (mode 4, stochastic QUAD
read: per-timestep coin with E[fire] = the QUAD weight). QSR is a LIGHT mode (QUAD-like
cells that consolidate), so unlike BINARY it does NOT hit the _eval_batch_size batch=1
serial trap — a full GA run is feasible later. We use the grid-winner-held-out path here
for consistency with the TERNARY/BINARY arms and to keep the box free for the priority
IDS drain.
...
# EXACT CLI of the binqsr_v3 QSR arm (run_gran_5arm_capped.sh, STEPS=2000 POP=50).
ARGV = [
...
SAVE = "/Users/lacg/wnn/logs/controller/qsr_grid_winner.yaml.gz"  # canonical (_ctl_load / --seed-winner)
...
def main():
	...
	print(f"\n{'#'*72}\n# QSR grid-winner HONEST held-out (grid-only; NO GA)\n{'#'*72}")
	t0 = time.time()
	...
	# SAVE immediately (canonical schema-2 yaml.gz, cells packed).
	...
	print(f"\n{bar}\n  HONEST HELD-OUT [GRID-WINNER sn={sn} b={b} QSR] — train seed {s.train} "
