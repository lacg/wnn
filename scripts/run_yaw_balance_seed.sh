...
# → robust. This forces S16-like even wiring via --feature-balance-ratio 1.5 (no feature >1.5x
...
DIR=logs/controller/YawBalance_20260627/YAWBAL_seed${SEED}; mkdir -p "$DIR"
echo "[yaw-bal] $(date '+%Y-%m-%d %H:%M:%S') START obs_yaw_err + balance-ratio 1.5 seed=$SEED"
$PY -u -m wnn.control.phased_ga \
  --grid-state-neurons 8 12 16 --grid-bits 24 30 --levels 16 --bits-per-feature 8 \
  --no-delta-control \
  --obs-yaw-err --feature-balance-ratio 1.5 --integral-leak 0.99 --integral-scale 1.0 \
...
echo "{\"yaw_bal_done\":true,\"seed\":$SEED,\"ts\":\"$(date '+%Y-%m-%dT%H:%M:%S')\"}" > /tmp/wnn_yaw_bal_seed${SEED}_done.json
echo "[yaw-bal] $(date '+%Y-%m-%d %H:%M:%S') COMPLETE seed=$SEED"
